...
// Split migration into smaller chunks to execute separately
const migrations = [
  {
...
async function executeSql(sql) {
  ...
  const options = {
    hostname: 'lzlrojoaxrqvmhempnkn.supabase.co',
    port: 443,
    path: '/rest/v1/rpc',
    method: 'POST',
    headers: {
...
async function applyMigrations() {
  console.log('🚀 Applying nutrition system migrations...\n');
  
  // Since direct SQL execution isn't available, let's save the migration
  // and provide instructions for manual execution
  
  const fullMigration = migrations.map(m => m.sql).join('\n\n');
...
-- RLS Policies for coaches/staff
...
`;
  ...
  fs.writeFileSync(migrationPath, fullMigration + additionalSql);
  
  console.log(`✅ Migration SQL saved to: ${migrationPath}\n`);
  console.log('📋 To apply this migration:\n');
  console.log('1. Go to Supabase Dashboard: https://supabase.com/dashboard/project/lzlrojoaxrqvmhempnkn');
  console.log('2. Navigate to SQL Editor');
  console.log('3. Copy and paste the migration from the file above');
  ...
  for (const migration of migrations) {
    try {
      const { data, error } = await supabase
        .from(migration.name.replace('_', '_'))
        .select('*')
        .limit(1);
      
      if (error) {
        console.log(`❌ ${migration.name}: Not created yet`);
      } else {
