-- Complete Facebook Integration Fix
-- Run this entire script at once

-- 1. First drop the table if it exists (to start fresh)
DROP TABLE IF EXISTS facebook_lead_forms CASCADE;

-- 2. Create facebook_lead_forms table with proper structure
...
-- 3. Create facebook_ad_accounts if it doesn't exist
...
-- 4. Add columns to facebook_pages if they don't exist
DO $$ 
BEGIN
...
-- 5. Add columns to facebook_integrations if they don't exist
DO $$ 
BEGIN
...
-- 7. Create helper function to get user organization
...
-- 8. Create view for user accessible organizations
...
-- 11. Create RLS policies for facebook_lead_forms
...
-- 12. Create RLS policies for facebook_ad_accounts
