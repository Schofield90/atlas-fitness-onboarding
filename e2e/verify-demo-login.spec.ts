...
test.describe("Demo Account Login Verification", () => {
  test("should successfully login with demo credentials", async ({ page }) => {
    ...
    // Navigate to gym owner login page (not admin /signin)
    await page.goto("https://login.gymleadhub.co.uk/login", {
      waitUntil: "load",
      timeout: 60000,
    });
    console.log("✅ Navigated to gym owner login page");
    ...
    // Take a screenshot for verification (before assertions)
    await page.screenshot({ path: "demo-login-success.png", fullPage: true });
    console.log("📸 Screenshot saved: demo-login-success.png");

    // Verify we're not on the login page anymore (but owner-login is acceptable intermediate step)
    const isOnLoginForm =
      currentUrl.includes("/login") && !currentUrl.includes("/owner-login");
    expect(isOnLoginForm).toBe(false);

    // Check for common dashboard indicators or intermediate pages
    const isDashboard =
    ...
    if (isDashboard) {
      console.log("✅ Successfully navigated past login form");
    }
  ...
  test("should display user email or name after login", async ({ page }) => {
    ...
    const hasUserEmail =
      (await page.locator("text=/test@test.co.uk/i").count()) > 0;
