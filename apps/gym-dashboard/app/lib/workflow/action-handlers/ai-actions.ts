...
export async function aiGenerateAction(
  ...
  try {
    ...
    const systemPrompt = parameters.systemPrompt
      ? interpolateValue(parameters.systemPrompt, context)
      : null;
...
export async function aiAnalyzeAction(
  ...
  try {
    ...
    switch (analysisType) {
      case "sentiment":
        systemPrompt =
          "You are a sentiment analysis expert. Analyze the sentiment of the provided text and return a JSON object with sentiment (positive/negative/neutral), confidence (0-1), and key phrases.";
        prompt = `Analyze the sentiment of this text: ${JSON.stringify(data)}`;
        break;

      case "lead_scoring":
        systemPrompt =
          "You are a lead scoring expert for a fitness business. Analyze the lead data and return a JSON object with score (0-100), factors (array of scoring factors), and recommended_actions (array of next steps).";
        prompt = `Score this lead based on their likelihood to convert: ${JSON.stringify(data)}`;
        break;

      case "content_moderation":
        systemPrompt =
          "You are a content moderation expert. Analyze the text for inappropriate content and return a JSON object with is_appropriate (boolean), categories (array of any issues found), and confidence (0-1).";
        prompt = `Moderate this content: ${JSON.stringify(data)}`;
        break;

      case "data_extraction":
        systemPrompt =
          "You are a data extraction expert. Extract structured information from the provided text and return it as a JSON object.";
      ...
      case "classification":
        systemPrompt =
          "You are a classification expert. Classify the provided data into the appropriate category.";
