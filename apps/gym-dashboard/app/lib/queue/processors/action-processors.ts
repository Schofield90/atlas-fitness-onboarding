...
export async function processNodeExecution(job: Job<NodeExecutionData>) {
  const { nodeId, node, workflowId, executionId, organizationId, context } =
    job.data;

  console.log(
    `Executing node ${nodeId} (${node.type}) for workflow ${workflowId}`,
  );
...
function replaceVariables(template: string, context: any): string {
  ...
  return template
    ...
    .replace(
      /{{firstName}}/g,
      lead.first_name || lead.name?.split(" ")[0] || "there",
    )
    ...
    .replace(
      /{{organizationName}}/g,
      context.organizationName || "Atlas Fitness",
    );
}
