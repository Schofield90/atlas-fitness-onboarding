...
async function generateWeeklyBriefData() {
  ...
  try {
    ...
    const signupChange =
      newSignupsLast > 0
        ? ((newSignupsThis - newSignupsLast) / newSignupsLast) * 100
        : 0;
    ...
    const userChange =
      newUsersLast > 0
        ? ((newUsersThis - newUsersLast) / newUsersLast) * 100
        : 0;
    ...
    const currentMRR = (allOrgs || []).reduce((total, org) => {
      const price =
        tierPricing[org.subscription_tier as keyof typeof tierPricing] || 0;
      return total + price;
    }, 0);
    ...
    return {
      kpis: {
        ...
        newSignups: {
          ...
          change: signupChange,
          trend: signupChange >= 0 ? ("up" as const) : ("down" as const),
        },
        activeUsers: {
...
async function getTopPerformingTenants() {
  try {
    ...
    return (orgs || []).slice(0, 5).map((org) => ({
      ...
      name: org.name,
      revenue:
        tierRevenue[org.subscription_tier as keyof typeof tierRevenue] || 29,
