...
export async function GET(request: NextRequest) {
  try {
    ...
    const includeCircuitBreakers =
      searchParams.get("include_circuit_breakers") === "true";
    ...
    if (includeTrends) {
      responseData.trends = await errorMonitor.getErrorTrends(
        timeRange as any,
        targetOrgId,
      );
    }
    ...
    if (includeCircuitBreakers) {
      responseData.circuitBreakers =
        globalRecoveryManager.getCircuitBreakerStatus();
...
function calculateAdditionalStats(errorData: any[]) {
  ...
  errorData.forEach((error) => {
    ...
    if (error.status_code) {
      stats.statusCodeBreakdown[error.status_code] =
        (stats.statusCodeBreakdown[error.status_code] || 0) + 1;
    }
    ...
    // Track error codes
    if (error.error_code) {
      errorCodeCounts[error.error_code] =
        (errorCodeCounts[error.error_code] || 0) + 1;
    }
  });
