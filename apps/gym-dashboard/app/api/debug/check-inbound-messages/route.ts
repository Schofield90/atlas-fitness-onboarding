  ...
  try {
    ...
    const analysis = {
      sms: {
        ...
        inbound:
          smsLogs?.filter((log) => log.status === "received").length || 0,
        outbound: smsLogs?.filter((log) => log.status === "sent").length || 0,
        hasInboundMessages:
          smsLogs?.some((log) => log.status === "received") || false,
        error: smsError,
      },
      whatsapp: {
        total: whatsappLogs?.length || 0,
        inbound:
          whatsappLogs?.filter((log) => log.status === "received").length || 0,
        outbound:
          whatsappLogs?.filter((log) => log.status === "sent").length || 0,
        hasInboundMessages:
          whatsappLogs?.some((log) => log.status === "received") || false,
        error: whatsappError,
      },
    };
    ...
    return NextResponse.json({
      ...
      webhookInfo: {
        ...
        checkWebhook:
          "Go to Twilio Console > Phone Numbers > Your Number > Messaging > Webhook",
      },
      troubleshooting: {
        noInboundMessages:
          !analysis.sms.hasInboundMessages &&
          !analysis.whatsapp.hasInboundMessages,
        possibleIssues: [
