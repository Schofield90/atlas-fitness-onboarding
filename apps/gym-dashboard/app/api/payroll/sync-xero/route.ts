...
export async function POST(request: NextRequest) {
  try {
    ...
    const { organizationId, action, ...actionData } = body;
    ...
    switch (action) {
      ...
      case "sync_single_employee":
        ...
        const singleSyncResult = await employeeSync.syncEmployeeToXero(
      ...
      case "resolve_conflict":
        if (!actionData.employeeId || !actionData.resolution) {
          return NextResponse.json(
            {
              success: false,
              error: "Employee ID and resolution method are required",
            },
            { status: 400 },
          );
        }
      ...
      case "sync_batch_to_xero":
        ...
        const batchSyncResult = await payrollService.syncToXero(
          actionData.batchId,
        );
        return NextResponse.json({
