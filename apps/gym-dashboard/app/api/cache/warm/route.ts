...
export async function POST(request: NextRequest) {
  try {
    ...
    const serviceWarming = {
      organization: async () => {
        if (!userId)
          throw new Error("userId required for organization cache warming");
      ...
      return NextResponse.json(
        {
          ...
          message:
            'Available services: organization, analytics, leads, bookings, or "all"',
        },
        { status: 400 },
      );
    }
    ...
    logger.info(
      `Cache warming completed for org ${orgId}: ${successCount}/${results.length} services warmed in ${totalDuration}ms`,
    );
          ...
          availableServices: [
            {
              name: "organization",
              description:
                "Organization settings, permissions, and feature flags",
              priority: 1,
              estimatedTime: 500,
          ...
          strategies: [
            {
              name: "sequential",
              description:
                "Warm services one by one (recommended for production)",
              pros: [
            ...
            },
            {
              name: "parallel",
              description:
                "Warm all services simultaneously (faster but resource intensive)",
...
function generateWarmingRecommendations(orgId: string): string[] {
  ...
  if (hour >= 9 && hour <= 17) {
    recommendations.push(
      "Consider warming during evening hours (after 6 PM) to avoid peak usage",
    );
  } else {
    recommendations.push(
      "Current time is optimal for cache warming (off-peak hours)",
    );
  }
