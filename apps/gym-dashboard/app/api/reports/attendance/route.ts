...
export async function GET(request: NextRequest) {
  try {
    ...
    const noShowCount =
      bookings?.filter((b) => !b.checked_in && b.status === "confirmed")
        .length || 0;
    ...
    bookings?.forEach((booking) => {
      ...
      if (booking.checked_in) classTypeStats[className].attended++;
        ...
        classTypeStats[className].cancelled++;
      else if (!booking.checked_in && booking.status === "confirmed")
        classTypeStats[className].noShow++;
    });
    ...
    bookings?.forEach((booking) => {
      if (booking.checked_in) {
        const date = new Date(
          booking.class_session?.start_time || booking.created_at,
    ...
    const dailyData = Object.entries(dailyAttendance)
      .map(([date, count]) => ({
        date,
        attendance: count,
      }))
    ...
    return NextResponse.json({
      summary: {
        ...
        attendanceRate:
          totalBookings > 0
            ? ((attendedCount / totalBookings) * 100).toFixed(1)
            : 0,
        dateRange: {
          start: startDate,
          end: endDate,
