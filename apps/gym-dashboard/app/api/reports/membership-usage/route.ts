...
export async function GET(request: NextRequest) {
  try {
    ...
    const pausedMemberships =
      memberships?.filter((m) => m.status === "paused").length || 0;
    const cancelledMemberships =
      memberships?.filter((m) => m.status === "cancelled").length || 0;
    ...
    const membershipUsage =
      memberships?.map((membership) => {
        const customerBookings =
          bookings?.filter(
            (b) => b.customer_id === membership.customer?.id && b.checked_in,
          ).length || 0;
    ...
    memberships?.forEach((membership) => {
      ...
      if (membership.status === "active")
        planDistribution[planName].activeCount++;
      planDistribution[planName].revenue +=
        (membership.membership_plan?.price_pennies || 0) / 100;
    ...
    Object.values(planDistribution).forEach((plan) => {
      plan.avgUsage =
        plan.count > 0 ? (plan.totalUsage / plan.count).toFixed(1) : 0;
    ...
    memberships?.forEach((membership) => {
      const month = new Date(membership.created_at).toLocaleDateString(
        "en-GB",
        {
          year: "numeric",
          month: "short",
    ...
    const trend = Object.values(monthlyTrend).sort(
      ...
      insights: {
        averageUsageRate:
          membershipUsage
            ...
            membershipUsage.filter((m) => m.usageRate !== "Unlimited").length ||
          0,
        mostPopularPlan:
          Object.values(planDistribution).sort(
