...
async function main() {
  ...
  console.log(`   Checks performed: ${metricsBefore.checksPerformed} → ${metricsAfter.checksPerformed}`);
  console.log(`   Tasks queued: ${metricsBefore.tasksQueued} → ${metricsAfter.tasksQueued}`);
  console.log('');
  ...
  console.log('   Last check:', status.metrics.lastCheckTime?.toISOString() || 'Never');
  console.log('   Next check:', status.metrics.nextCheckTime?.toISOString() || 'Unknown');
  console.log('');
