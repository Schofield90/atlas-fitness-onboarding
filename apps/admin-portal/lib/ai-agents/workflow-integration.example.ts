...
import type { EnhancedWorkflow, WorkflowNode, WorkflowEdge } from '@/app/lib/workflow/types';
  ...
  nodes: [
    ...
    },
    {
      ...
      data: {
        ...
        categories: ['billing', 'technical', 'general', 'urgent', 'cancellation'],
      },
    },
    {
      id: 'condition_urgent',
 ...
 */
export async function getAgentExecutionLogs(workflowId: string, executionId?: string) {
  ...
  const response = await fetch(`/api/workflows/actions/execute-agent?${params}`);
  const result = await response.json();
