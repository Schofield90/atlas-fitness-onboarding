...
// Read the nutrition migration SQL
const migrationPath = path.join(__dirname, 'supabase/migrations/20250110_nutrition_system_complete.sql');

const migrationSQL = `
-- Complete the nutrition system tables that haven't been created yet
...
-- Meal substitutions suggested by AI or coach
CREATE TABLE IF NOT EXISTS meal_substitutions (
  ...
  suggested_by VARCHAR(20) CHECK (suggested_by IN ('ai', 'coach', 'client')),
...
-- Coach-client communication table
CREATE TABLE IF NOT EXISTS nutrition_coach_messages (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  client_id UUID NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
  coach_id UUID REFERENCES auth.users(id),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  
  message_type VARCHAR(20) CHECK (message_type IN ('coach_to_client', 'client_to_coach', 'system')),
  subject VARCHAR(255),
  message TEXT NOT NULL,
  
  -- For tracking meal plan discussions
  related_meal_plan_id UUID REFERENCES meal_plans(id),
  related_meal_id UUID REFERENCES meal_plan_meals(id),
...
CREATE INDEX IF NOT EXISTS idx_coach_messages_client ON nutrition_coach_messages(client_id);
CREATE INDEX IF NOT EXISTS idx_coach_messages_coach ON nutrition_coach_messages(coach_id);
...
ALTER TABLE nutrition_coach_messages ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Clients can view their messages" ON nutrition_coach_messages
  FOR SELECT USING (client_id IN (SELECT id FROM clients WHERE user_id = auth.uid()));

CREATE POLICY "Clients can send messages" ON nutrition_coach_messages
...
CREATE POLICY "Staff can manage client nutrition preferences" ON nutrition_preferences
  FOR ALL USING (
    profile_id IN (
      SELECT id FROM nutrition_profiles WHERE organization_id IN (
...
CREATE POLICY "Staff can manage coach messages" ON nutrition_coach_messages
  FOR ALL USING (
    organization_id IN (
      SELECT organization_id FROM organization_staff 
      WHERE user_id = auth.uid() AND is_active = true
...
async function applyMigration() {
  console.log('🚀 Applying nutrition system migration...\n');
  
  try {
    // Execute the migration using Supabase's SQL editor API
    // Note: Direct SQL execution isn't available via JS SDK, so we'll use the admin API
    ...
    if (error) {
      ...
      console.log('Creating individual tables...\n');
      
      // Save migration file for manual execution
      fs.writeFileSync(migrationPath, migrationSQL);
      console.log(`✅ Migration SQL saved to: ${migrationPath}`);
      console.log('\nPlease execute this migration manually in Supabase SQL editor or via psql');
    ...
    console.log('Please execute this migration manually in Supabase SQL editor');
  }
}
