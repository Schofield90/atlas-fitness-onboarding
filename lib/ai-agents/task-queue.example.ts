...
async function example1_AddAdHocTask() {
  ...
  const { data: task, error } = await supabase
    ...
    .insert({
      ...
      description: 'Analyze revenue, churn, and growth metrics for the past week',
      task_type: 'adhoc',
      context: {
        report_type: 'weekly',
...
async function example3_AddDelayedTask() {
  ...
  const { data: task, error } = await supabase
    ...
    .insert({
      ...
      description: 'Send personalized retention message to member who hasn\'t visited in 14 days',
