...
export class AgentTaskQueue {
  ...
  constructor() {
    ...
    console.log(`[AgentTaskQueue] Initialized with Redis at ${process.env.REDIS_HOST || 'localhost'}:${process.env.REDIS_PORT || '6379'}`);
  }
  ...
  async addTask(
    ...
    console.log(`[AgentTaskQueue] Task ${taskId} added to queue (priority: ${priority}, delay: ${delay}ms)`);
  ...
  async addScheduledTask(
    ...
    const nextRunAt = this.calculateNextRun(cronExpression, task.schedule_timezone || 'UTC');
    ...
    const scheduledTask = cron.schedule(cronExpression, async () => {
      try {
        ...
        const nextRun = this.calculateNextRun(cronExpression, task.schedule_timezone || 'UTC');
        await this.supabase
          .from('ai_agent_tasks')
          .update({
        ...
        console.error(`[AgentTaskQueue] Error scheduling task ${taskId}:`, error);
      }
    }, {
      scheduled: true,
    ...
    console.log(`[AgentTaskQueue] Scheduled task ${taskId} with cron: ${cronExpression}`);
  }
  ...
  private async processTask(job: Job<AgentTaskJobData>): Promise<any> {
    ...
    console.log(`[AgentTaskQueue] Processing task ${taskId} (attempt ${job.attemptsMade + 1}/${job.opts.attempts})`);
    ...
    try {
      ...
      if (!result.success) {
        ...
        if (task && task.retry_count < task.max_retries) {
          ...
          console.error(`[AgentTaskQueue] Task ${taskId} failed permanently after ${task?.max_retries || 0} retries`);
          return result;
        }
      }

      console.log(`[AgentTaskQueue] Task ${taskId} completed successfully (${result.executionTimeMs}ms, ${result.tokensUsed} tokens, $${result.costUsd.toFixed(4)})`);
  ...
  private initializeScheduledTaskPolling(): void {
    ...
    setInterval(async () => {
      ...
      try {
        ...
        if (error) {
          console.error('[AgentTaskQueue] Error fetching scheduled tasks:', error);
          return;
        }

        if (tasks && tasks.length > 0) {
          console.log(`[AgentTaskQueue] Found ${tasks.length} scheduled tasks to queue`);

          for (const task of tasks) {
            try {
              await this.addTask(task.id, task.priority || 5);
            } catch (err) {
              console.error(`[AgentTaskQueue] Error queuing scheduled task ${task.id}:`, err);
            }
          }
        }
      } catch (err) {
  ...
  private setupEventListeners(): void {
    ...
    this.worker.on('failed', (job: Job | undefined, error: Error) => {
      console.error(`[AgentTaskQueue] ✗ Job ${job?.id || 'unknown'} failed:`, error.message);

      // Move to dead letter queue if max retries exceeded
      if (job && job.attemptsMade >= (job.opts.attempts || 3)) {
        console.error(`[AgentTaskQueue] ⚠ Job ${job.id} moved to dead letter queue after ${job.attemptsMade} attempts`);
      }
    });

    // Stalled jobs
    this.worker.on('stalled', (jobId: string) => {
      console.warn(`[AgentTaskQueue] ⚠ Job ${jobId} stalled - worker may have crashed`);
    });
    ...
    const shutdown = async (signal: string) => {
      ...
      try {
        ...
        if (activeJobs.length > 0) {
          console.log(`[AgentTaskQueue] Waiting for ${activeJobs.length} active jobs to complete...`);
  ...
  async cleanupJobs(): Promise<void> {
    const completedCleaned = await this.queue.clean(COMPLETED_JOB_RETENTION, 1000, 'completed');
    const failedCleaned = await this.queue.clean(FAILED_JOB_RETENTION, 1000, 'failed');

    console.log(`[AgentTaskQueue] Cleaned up ${completedCleaned.length} completed and ${failedCleaned.length} failed jobs`);
  }
