...
export class WorkflowAgentExecutor {
  ...
  async executeFromWorkflow(
    ...
    try {
      ...
      const executionResult: TaskExecutionResult = await this.orchestrator.executeTask(
        task.id
      );
 ...
 */
export function formatWorkflowContext(context: ExecutionContext): Record<string, any> {
  const { workflowId, organizationId, executionId, trigger, variables, ...nodeOutputs } =
    context;
