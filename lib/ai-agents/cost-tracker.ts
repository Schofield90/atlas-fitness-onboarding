...
export async function calculateCost(params: {
  ...
  // Apply markup
  const costBilledCents = Math.round(costBaseCents * (1 + markupPercentage / 100));
...
export async function logAIUsage(record: BillingRecord): Promise<void> {
  ...
  try {
    ...
    if (existing) {
      ...
      updatedBreakdownByAgent[record.agentId].costBilled += record.costBilledUsd;
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          total_cost_base_usd: existing.total_cost_base_usd + record.costBaseUsd,
          total_cost_billed_usd: existing.total_cost_billed_usd + record.costBilledUsd,
...
export async function getUsageHistory(
  ...
  const { data, error } = await supabase
    .from('ai_usage_billing')
    .select('billing_period_start, total_tokens_used, total_cost_base_usd, total_cost_billed_usd')
    .eq('organization_id', organizationId)
