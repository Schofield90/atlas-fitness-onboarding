...
-- Add RLS to the view
ALTER VIEW messages_with_user_info OWNER TO postgres;
CREATE POLICY "Users can view messages with user info from their organization" ON messages_with_user_info
  FOR SELECT
  USING (
    organization_id IN (
      SELECT organization_id FROM users WHERE id = auth.uid()
