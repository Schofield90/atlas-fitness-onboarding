...
export const WORKFLOW_TRIGGERS = {
  ...
  "call_booking.created": "When a call booking is made",
...
class WorkflowService {
  private executors: Map<string, (config: any, context: any) => Promise<any>> =
    new Map();
  ...
  private registerExecutors() {
    ...
    this.executors.set("send_sms", async (config, context) => {
      ...
      return await messageService.sendSMS(context.orgId, recipient, body);
    });
    ...
    this.executors.set("send_whatsapp", async (config, context) => {
      ...
      return await messageService.sendWhatsApp(context.orgId, recipient, body);
    });
  ...
  async getWorkflows(
    orgId: string,
    filter?: { active?: boolean; trigger_type?: string },
  ) {
    const supabase = await createClient();

    let query = supabase.from("workflows").select("*").eq("org_id", orgId);
  ...
  private async registerTrigger(
    ...
    if (redis) {
      await redis.setex(
        cacheKey,
        86400,
        JSON.stringify({ orgId, triggerType, config }),
      );
    }
        ...
        actions: [
          {
            ...
            config: {
              ...
              subject:
                "Reminder: Your class tomorrow at {{booking.session.start_time}}",
