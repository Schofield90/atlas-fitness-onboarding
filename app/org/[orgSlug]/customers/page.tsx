...
function CustomersContent() {
  ...
  useEffect(() => {
    ...
    router.replace(`/org/${orgSlug}/customers?${params.toString()}`, {
      scroll: false,
    });
  ...
  const fetchCustomers = async () => {
    try {
      // Use organizationId from the hook which has the bypass for sam@atlas-gyms.co.uk
      if (!organizationId) {
      ...
      const response = await fetch(
        `/api/clients-bypass?organizationId=${organizationId}&page=1&pageSize=1000`,
      );
      ...
      const clientsResult = { data: result.data, error: null };
  ...
  const exportCustomers = async () => {
    ...
    try {
      ...
      const exportData = allDetailedCustomers.filter((customer) => {
        ...
        if (statusFilter && statusFilter !== "All") {
          if (
            statusFilter === "Active Customers" &&
            customer.status !== "active"
          )
            return false;
          if (
            statusFilter === "Inactive Customers" &&
            customer.status !== "inactive"
          )
            return false;
          if (
            statusFilter === "Slipping Away" &&
      ...
      reader.onload = async (e) => {
        ...
        for (const customer of customers) {
          try {
            ...
            const isClient =
              customer.type?.toLowerCase() === "client" ||
            ...
            if (isClient) {
              ...
              await supabase.from("clients").insert({
                ...
                metadata: {
                  ...
                  date_of_birth:
                    customer["date_of_birth"] || customer.dob || null,
                  address_line_1:
                    customer.address || customer["address_line_1"] || null,
                  city: customer.city || null,
                  postal_code:
                    customer["postal_code"] || customer.postcode || null,
                },
              });
            } else {
              // Import as lead
        ...
        alert(
          `Import complete: ${successCount} customers imported successfully, ${errorCount} errors`,
        );
              ...
              >
                <div className="p-6 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    {/* Avatar */}
                    <div
                      className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl font-semibold ${getAvatarColor(customer.first_name)}`}
                    >
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="border-2 border-dashed border-gray-600 rounded-lg p-4 text-center">
                  ...
                  <p className="text-gray-500 text-xs mt-1">
                    Required columns: Name, Email. Optional: Phone, Status,
                    Type, Address, etc.
                  </p>
                  <input
                    type="file"
              ...
              <div className="bg-gray-700 rounded-lg p-4">
                ...
                <ul className="text-sm text-gray-400 space-y-1">
                  ...
                  <li>
                    • Optional columns: Phone, Status, Type, Date of Birth,
                    Address, City, Postal Code
                  </li>
                  <li>
                    • Type should be 'Client' or 'Lead' (defaults to Lead if not
                    specified)
                  </li>
                  <li>
                    • Status can be 'active', 'inactive' for clients or 'new',
                    'contacted', 'qualified' for leads
                  </li>
