    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">Choose from our available classes</p>
        </div>

        {message && (
          <div className={`mb-6 p-4 rounded-lg ${
            message.includes('confirmed') ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'
          }`}>
            {message}
          </div>
        )}
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {classes.map((classSession) => {
            const spotsLeft = classSession.max_capacity - classSession.current_bookings
