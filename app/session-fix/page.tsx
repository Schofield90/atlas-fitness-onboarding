...
export default function SessionFixPage() {
  ...
  const handleFix = async () => {
    ...
    try {
      const response = await fetch('/api/debug/session-fix', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-4">
      <div className="bg-gray-800 rounded-lg p-8 max-w-2xl w-full">
        <h1 className="text-2xl font-bold text-white mb-4">Session-Based Fix</h1>
        
        <div className="bg-blue-900/50 border border-blue-600 rounded-lg p-4 mb-6">
          <p className="text-blue-200">
            This uses your current session to create the missing user entry.
          </p>
          <p className="text-blue-300 text-sm mt-2">
            No email lookup needed - uses your active session directly.
          </p>
        </div>
        ...
        >
          {loading ? 'Running Fix...' : result?.success ? 'Fixed!' : 'Fix User Organization'}
        </button>
        ...
        {result && result.success && (
          <div className="mt-6 space-y-4">
            ...
            {result.testLead && (
              <div className="p-4 bg-gray-700 rounded-lg">
                <h3 className="text-sm font-semibold text-gray-300 mb-2">Test Lead</h3>
