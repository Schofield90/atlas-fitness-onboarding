...
export default function ClassCalendarPage() {
  ...
  const fetchClasses = async (orgId: string, startDate?: Date, endDate?: Date) => {
    try {
      console.log('ClassCalendarPage: fetchClasses called with orgId:', orgId, 'dates:', startDate, endDate);
      setLoading(true);
        ...
        <div className="flex-1 flex flex-col p-6 overflow-hidden">
          ...
          <div className="flex-1 overflow-auto mt-4">
            <PremiumCalendarGrid 
              ...
              currentDate={currentDate}
              onClassUpdate={() => {
                console.log('ClassCalendarPage: onClassUpdate callback triggered, organizationId:', organizationId);
                if (organizationId) {
                  const range = getDateRange(currentDate, calendarView);
                  fetchClasses(organizationId, range.start, range.end);
                } else {
                  console.error('ClassCalendarPage: Cannot refresh - organizationId is null');
                }
              }}
            />
          </div>
