...
export class EnhancedQueueManager {
  ...
  private schedulers: Map<QueueName, QueueScheduler>;
  ...
  private async initializeQueues(): Promise<void> {
    ...
    for (const [name, queueName] of Object.entries(QUEUE_NAMES)) {
      try {
        ...
        this.schedulers.set(queueName as QueueName, scheduler);
  ...
  async performHealthCheck(): Promise<Map<QueueName, QueueHealth>> {
    ...
    for (const [queueName, health] of healthReport) {
      if (health.status === 'critical') {
        console.error(`🚨 Critical queue health issue in ${queueName}:`, health.issues);
      } else if (health.status === 'warning') {
        console.warn(`⚠️  Queue health warning in ${queueName}:`, health.issues);
      }
    }
  ...
  private evaluateQueueHealth(stats: QueueStats): Omit<QueueHealth, 'metrics' | 'lastCheck'> {
    const issues: string[] = [];
  ...
  private async collectMetrics(): Promise<void> {
    ...
    const now = new Date();
    const hoursSinceReset = (now.getTime() - this.metrics.lastMetricsReset.getTime()) / (1000 * 60 * 60);
  ...
  getQueue(name: QueueName): Queue {
    ...
    if (!queue) {
      throw new Error(`Queue ${name} not found. Available queues: ${Array.from(this.queues.keys()).join(', ')}`);
    }
    return queue;
  }
  ...
  async addJob<T = any>(
    ...
    const jobOptions = {
      ...
      jobId: options?.jobId || (options?.delay ? undefined : `${jobType}-${Date.now()}-${Math.random()}`),
    };
  ...
  async getAllQueueStats(): Promise<Record<QueueName, QueueStats>> {
    ...
    const statPromises = Array.from(this.queues.keys()).map(async (queueName) => {
      try {
