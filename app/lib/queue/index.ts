...
export {
  ...
  startEnhancedWorkers,
  stopEnhancedWorkers,
} from "./enhanced-workers";

// Configuration exports
export * from "./enhanced-config";
// Don't export module-level connections - use getter functions instead
...
// Type exports
export type { QueueName, JobType, JobPriority } from "./enhanced-config";
...
export async function initializeQueueSystem(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `  - Queues: ${(await enhancedQueueManager.getAllQueueStats()).length || 0} active`,
    );
    console.log(
      `  - Workers: ${enhancedWorkerManager.getWorkers().size} running`,
    );
    console.log(
      `  - Connection: ${enhancedQueueManager.getConnectionHealth().redis ? "Connected" : "Disconnected"}`,
    );
  } catch (error) {
...
export function checkEnvironment(): {
  ...
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
    issues.push(
      "Supabase service role key not configured (SUPABASE_SERVICE_ROLE_KEY)",
    );
  }
  ...
  if (!process.env.SENDGRID_API_KEY && !process.env.RESEND_API_KEY) {
    issues.push(
      "Email service not configured (SENDGRID_API_KEY or RESEND_API_KEY)",
    );
  }

  if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN) {
    issues.push(
      "SMS service not configured (TWILIO_ACCOUNT_SID and TWILIO_AUTH_TOKEN)",
    );
  }
