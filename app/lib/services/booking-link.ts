...
import { format, parseISO, addDays, startOfDay, endOfDay, addMinutes, isBefore, isAfter, isSameDay } from 'date-fns'
...
export interface FormField {
  ...
  name: string
  label: string
  type: 'text' | 'email' | 'phone' | 'textarea' | 'select' | 'checkbox' | 'radio' | 'date' | 'time'
  options?: string[]
  required: boolean
  placeholder?: string
...
export class BookingLinkService {
  ...
  async listBookingLinks(organizationId: string): Promise<BookingLink[]> {
    ...
    if (error) throw new Error(`Failed to fetch booking links: ${error.message}`)
    return data || []
  }
  ...
  async deleteBookingLink(id: string): Promise<void> {
    ...
    if (error) throw new Error(`Failed to delete booking link: ${error.message}`)
  }
  ...
  async setAvailabilityRules(bookingLinkId: string, staffId: string, rules: Array<{
    day_of_week: number
    start_time: string
    end_time: string
    ...
    if (rules.length > 0) {
      ...
      if (error) throw new Error(`Failed to set availability rules: ${error.message}`)
    }
  }

  async addException(bookingLinkId: string, exception: {
    staff_id?: string
    ...
    if (error) throw new Error(`Failed to add booking exception: ${error.message}`)
  }
  ...
  async getAvailableSlots(
    ...
    try {
      ...
      if (atError) throw new Error(`Failed to fetch appointment types: ${atError.message}`)
      ...
      if (arError) throw new Error(`Failed to fetch availability rules: ${arError.message}`)
      ...
      if (exError) throw new Error(`Failed to fetch exceptions: ${exError.message}`)
      ...
      if (ebError) throw new Error(`Failed to fetch existing bookings: ${ebError.message}`)
      ...
      while (currentDate <= endDate) {
        ...
        for (const rule of dayRules) {
          ...
          const staffException = dayExceptions.find(ex => 
            ex.staff_id === rule.staff_id || ex.staff_id === null
          )
          ...
          const startTime = staffException?.custom_hours?.[0]?.start_time || rule.start_time
          const endTime = staffException?.custom_hours?.[0]?.end_time || rule.end_time
          ...
          for (const appointmentType of appointmentTypes || []) {
            ...
            while (addMinutes(currentSlot, appointmentType.duration_minutes) <= slotEndTime) {
              const slotEnd = addMinutes(currentSlot, appointmentType.duration_minutes)
              ...
              currentSlot = addMinutes(slotEnd, bookingLink.buffer_settings?.after_minutes || 15)
            }
          }
        }
  ...
  async createBooking(request: BookingRequest): Promise<{
    ...
    const title = this.generateMeetingTitle(bookingLink.meeting_title_template, {
      contact: { name: request.attendee_name },
      service: (await this.getAppointmentType(request.appointment_type_id))?.name || 'Appointment'
    })
    ...
    const { data: booking, error } = await supabase
      ...
      .insert({
        ...
        booking_status: bookingLink.confirmation_settings.auto_confirm ? 'confirmed' : 'pending',
  ...
  async cancelBooking(cancellationToken: string, reason?: string): Promise<void> {
    const supabase = this.adminSupabase
    ...
    if (bookingLink?.cancellation_policy.allowed) {
      const hoursUntil = (parseISO(booking.start_time).getTime() - new Date().getTime()) / (1000 * 60 * 60)
      if (hoursUntil < bookingLink.cancellation_policy.hours_before) {
        throw new Error(`Cancellations must be made at least ${bookingLink.cancellation_policy.hours_before} hours in advance`)
      }
    }
  ...
  async updateFormFields(bookingLinkId: string, fields: FormField[]): Promise<void> {
    const supabase = this.adminSupabase
    ...
    if (fields.length > 0) {
      ...
      if (error) throw new Error(`Failed to update form fields: ${error.message}`)
    }
  }
  ...
  async trackEvent(
    ...
    eventType: 'page_view' | 'form_started' | 'booking_completed' | 'booking_cancelled',
  ...
  async getAnalytics(bookingLinkId: string, days = 30): Promise<{
    page_views: number
    form_starts: number
    ...
    const formStarts = analytics.filter(a => a.event_type === 'form_started').length
    ...
    return {
      ...
      conversion_rate: pageViews > 0 ? (bookingsCompleted / pageViews) * 100 : 0,
  ...
  private async isSlotAvailable(staffId: string, startTime: Date, endTime: Date): Promise<boolean> {
    const { data, error } = await this.supabase
      .from('bookings')
      .select('id')
  ...
  private generateMeetingTitle(template: string, variables: Record<string, any>): string {
    let title = template
  ...
  private getNotificationBody(type: string, booking: any): string {
    const startTime = format(parseISO(booking.start_time), 'EEEE, MMMM d, yyyy at h:mm a')
  ...
  async getTrainerSpecializations(staffId: string): Promise<Array<{
    type: string
    ...
    if (error) throw new Error(`Failed to fetch trainer specializations: ${error.message}`)
  ...
  async getEquipmentRequirements(bookingLinkId: string): Promise<Array<{
    name: string
    type: string
    required: boolean
    alternatives: string[]
    ...
    if (error) throw new Error(`Failed to fetch equipment requirements: ${error.message}`)
  ...
  async checkClassCapacity(appointmentTypeId: string, startTime: string): Promise<{
    current: number
    maximum: number
    available: boolean
  }> {
  ...
  async duplicateBookingLink(originalId: string, newName: string): Promise<BookingLink> {
  ...
  async getBookingStats(bookingLinkId: string): Promise<{
    ...
    if (error) throw new Error(`Failed to fetch booking stats: ${error.message}`)
    ...
    return {
      ...
      this_month: bookings.filter(b => parseISO(b.start_time) >= thisMonthStart).length,
  ...
  async validateBookingLinkConfig(config: Partial<BookingLink>): Promise<{
    ...
    if (config.slug) {
      const isAvailable = await this.checkSlugAvailability(config.slug, config.id)
      if (!isAvailable) {
