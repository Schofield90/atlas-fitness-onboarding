...
export class SOPProcessor {
  ...
  private async extractFromPDF(buffer: Uint8Array): Promise<string> {
    try {
      ...
      throw new Error('PDF parsing not implemented. Please convert to text file or use copy-paste.')
    } catch (error) {
      throw new Error('Failed to extract PDF content. Please convert to text format.')
    }
  }
  ...
  private async extractFromDOCX(buffer: Uint8Array): Promise<string> {
    try {
      ...
      throw new Error('DOCX parsing not implemented. Please convert to text file or use copy-paste.')
    } catch (error) {
      throw new Error('Failed to extract DOCX content. Please convert to text format.')
    }
  }
...
`
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert in analyzing standard operating procedures for fitness facilities. Provide detailed, actionable insights that help staff understand and implement procedures effectively.'
          },
          {
            role: 'user',
            content: prompt
      ...
      return {
        ...
        training_recommendations: {
          ...
          difficulty: analysis.training_recommendations?.difficulty || 'intermediate',
          estimated_time_minutes: analysis.training_recommendations?.estimated_time_minutes || 30
        }
      }
    } catch (error) {
...
`
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert assistant for gym operations, helping staff understand and follow standard operating procedures. Always provide practical, actionable guidance based strictly on the provided SOP content.'
          },
          {
            role: 'user',
            content: prompt
...
`
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert in creating training materials for gym staff. Generate practical quiz questions that test understanding of standard operating procedures.'
          },
          {
            role: 'user',
            content: prompt
