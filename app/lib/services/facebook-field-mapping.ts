...
interface FieldMapping {
  ...
  facebook_field_type: 'SHORT_ANSWER' | 'PHONE_NUMBER' | 'EMAIL' | 'MULTIPLE_CHOICE' | 'DATETIME'
  crm_field: string
  ...
  // Pattern matching for text fields
    ...
    for (const [crmFieldName, patterns] of Object.entries(FIELD_PATTERNS)) {
      if (patterns.some(pattern => pattern.test(fieldLabel))) {
        ...
        crmFieldType = ['email', 'first_name', 'last_name', 'full_name', 'phone'].includes(crmFieldName) 
          ? 'standard' 
          : 'custom'
        autoDetected = true
 ...
 */
function mapFacebookFieldType(type: string): FieldMapping['facebook_field_type'] {
...
export async function applyFieldMappings(
  ...
  for (const mapping of mappings) {
    ...
    if (mapping.crm_field === 'full_name' && typeof transformedValue === 'string') {
...
async function applyTransformation(
  ...
  try {
    switch (transformation.type) {
      case 'phone_format':
        return formatPhoneNumber(value, transformation.options?.phone_region || 'GB')
      
      case 'date_format':
        return formatDate(value, transformation.options?.date_format || 'DD/MM/YYYY')
      
      case 'boolean':
        return parseBooleanValue(value, transformation.options?.boolean_mapping)
...
export function validateMappings(mappings: FieldMapping[]): {
  ...
  for (const mapping of mappings) {
    ...
    if (!mapping.crm_field) {
      errors.push(`Missing CRM field for Facebook field ${mapping.facebook_field_name}`)
    }
    
    // Check for duplicate CRM field mappings (except custom fields)
    if (mapping.crm_field_type === 'standard' && crmFields.has(mapping.crm_field)) {
  ...
  if (!hasContactInfo) {
    errors.push('Warning: No contact information fields (email, phone, or name) mapped')
  }
  ...
  switch (transformation.type) {
    case 'phone_format':
      if (transformation.options?.phone_region) {
        ...
        if (!validRegions.includes(transformation.options.phone_region)) {
          errors.push(`Invalid phone region: ${transformation.options.phone_region}`)
        }
      }
      break
    ...
    case 'date_format':
      if (transformation.options?.date_format) {
        ...
        if (!validFormats.includes(transformation.options.date_format)) {
          errors.push(`Invalid date format: ${transformation.options.date_format}`)
        }
      }
      break
    ...
    case 'boolean':
      if (transformation.options?.boolean_mapping) {
        if (!transformation.options.boolean_mapping.true || 
            !transformation.options.boolean_mapping.false) {
          errors.push('Boolean mapping must specify both true and false values')
        }
      }
      break
  }
