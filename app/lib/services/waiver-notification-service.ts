...
export class WaiverNotificationService {
   ...
   */
  async sendWaiverAssignmentEmail(data: WaiverAssignmentData): Promise<boolean> {
    try {
      ...
      const { error: notificationError } = await this.supabase
        ...
        .insert({
          ...
          notification_type: data.isReminder ? 'waiver_reminder' : 'waiver_assigned',
          channel: 'email',
      ...
      if (notificationError) {
        console.error('Error creating waiver notification record:', notificationError)
        return false
      }
   ...
   */
  async sendWaiverSignedNotifications(data: WaiverSignedData): Promise<boolean> {
    try {
      ...
      if (data.organizationEmail) {
        ...
        if (orgNotificationError) {
          console.error('Error creating organization waiver notification:', orgNotificationError)
        }
      }

      if (customerNotificationError) {
        console.error('Error creating customer waiver notification:', customerNotificationError)
        return false
      }
   ...
   */
  async sendWaiverExpiringNotifications(data: WaiverExpiringData): Promise<boolean> {
    try {
      ...
      if (notificationError) {
        console.error('Error creating waiver expiring notification:', notificationError)
        return false
      }
  ...
  async processExpiredWaivers(): Promise<number> {
    try {
      ...
      const { data: expiredCount } = await this.supabase
        .rpc('mark_expired_waivers')
  ...
  async processExpiringWaivers(daysThreshold: number = 7): Promise<number> {
    try {
      ...
      const { data: expiringWaivers } = await this.supabase
        .rpc('check_expiring_waivers')
...
${data.organizationName}
  ...
  private buildInternalSignedNotificationContent(data: WaiverSignedData): string {
    return `
