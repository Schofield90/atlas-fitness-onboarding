...
export class TriggerWorkflowTool extends BaseTool {
  ...
  description =
    "Trigger an existing automation workflow with custom context data.";
  ...
  parametersSchema = z.object({
    ...
    triggerData: z
      .record(z.any())
      .optional()
      .describe("Optional data to pass to workflow execution"),
    priority: z
...
export class ScheduleTaskTool extends BaseTool {
  ...
  name = "Schedule Task";
  description =
    "Schedule a task to be executed at a specific time or on a recurring schedule.";
  category = "automation" as const;
  ...
  parametersSchema = z.object({
    ...
    executeAt: z
      .string()
      .optional()
      .describe("ISO 8601 datetime to execute task (for one-time tasks)"),
    cronExpression: z
      .string()
      .optional()
      .describe(
        'Cron expression for recurring tasks (e.g., "0 9 * * 1" = Monday 9am)',
      ),
    taskType: z
      .enum([
        "data_export",
    ...
    taskData: z
      .record(z.any())
      .optional()
      .describe("Task execution data/parameters"),
  });
...
export class UpdateClientTagsTool extends BaseTool {
  ...
  description =
    "Add or remove tags/segments for clients. Useful for organizing and targeting specific member groups.";
  category = "automation" as const;

  parametersSchema = z.object({
    clientIds: z
      .array(z.string().uuid())
      .describe("Array of client IDs to update"),
    tagsToAdd: z
      .array(z.string())
      .optional()
      .describe("Tags to add to clients"),
    tagsToRemove: z
      .array(z.string())
      .optional()
      .describe("Tags to remove from clients"),
  });
...
export class ExportDataTool extends BaseTool {
  ...
  name = "Export Data";
  description =
    "Export data to CSV or JSON format. Useful for reporting and data analysis.";
  category = "automation" as const;

  parametersSchema = z.object({
    dataType: z
      .enum(["clients", "payments", "bookings", "subscriptions"])
    ...
    filters: z
      .record(z.any())
      .optional()
      .describe("Optional filters to apply to export"),
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          status: "queued",
          message:
            "Export job created. You will be notified when the export is ready for download.",
...
export class CreateRetentionCampaignTool extends BaseTool {
  ...
  description =
    "Create an automated retention campaign workflow to re-engage inactive members or prevent churn.";
  category = "automation" as const;

  parametersSchema = z.object({
    name: z
      .string()
      .describe('Campaign name (e.g., "Win Back Inactive Members - Q4 2025")'),
    description: z
      .string()
      .optional()
    ...
    targetAudience: z
      .object({
        status: z
          ...
          .optional()
          .default("inactive")
          .describe("Target member status"),
        inactiveDays: z
          .number()
          .optional()
          .describe("Number of days inactive (for inactive members)"),
        lastVisitBefore: z
          .string()
          .optional()
          .describe("Last visit before date (YYYY-MM-DD)"),
    ...
    actions: z
      .array(
        z.object({
          type: z
            .enum(["send_email", "send_sms", "create_task", "send_whatsapp"])
            .describe("Action type"),
          delayMinutes: z
            .number()
            .default(0)
            .describe("Delay in minutes after trigger or previous action"),
          config: z
            .record(z.any())
            .describe(
              "Action-specific configuration (template_id, message, etc.)",
            ),
        }),
      )
      ...
      const { data: campaign, error: campaignError } = await supabase
        ...
        .insert({
          ...
          description:
            ...
            `Retention campaign targeting ${validated.targetAudience.status} members`,
          platform: "automation",
          status: "active",
...
export class CreateLeadTaskTool extends BaseTool {
  ...
  description =
    "Assign a task to a staff member for follow-up with a lead or client.";
  category = "automation" as const;

  parametersSchema = z.object({
    title: z
      .string()
      .describe('Task title (e.g., "Follow up with John about trial class")'),
    description: z.string().optional().describe("Detailed task description"),
    assignedTo: z
      .string()
      .optional()
      .describe("User ID of staff member to assign to"),
    priority: z
    ...
    dueDate: z
      .string()
      .optional()
      .describe("Due date in YYYY-MM-DD HH:MM format"),
    category: z
      .enum(["follow_up", "meeting", "document", "call", "email", "other"])
      .default("follow_up")
      .describe("Task category"),
    relatedLeadId: z
      .string()
      .optional()
      .describe("Lead ID this task relates to"),
    relatedClientId: z
      .string()
      .optional()
      .describe("Client ID this task relates to"),
  });
  ...
  async execute(
    ...
    try {
      ...
      if (!validated.relatedLeadId && !validated.relatedClientId) {
        throw new Error(
          "Either relatedLeadId or relatedClientId must be provided",
        );
      }
...
export class ScheduleSocialPostTool extends BaseTool {
  ...
  description =
    "Schedule a social media post for future publishing on configured platforms.";
  category = "automation" as const;
  ...
  parametersSchema = z.object({
    ...
    platforms: z
      ...
      .describe("Platforms to post on"),
    scheduledFor: z
      .string()
      .describe("Scheduled publish time (YYYY-MM-DD HH:MM format)"),
    mediaUrls: z
      .array(z.string())
      .optional()
      .describe("URLs of images/videos to attach"),
    hashtags: z.array(z.string()).optional().describe("Hashtags to include"),
    autoPublish: z
      .boolean()
      .default(true)
      .describe("Automatically publish at scheduled time"),
  });
  ...
  async execute(
    ...
    try {
      ...
      if (validated.hashtags && validated.hashtags.length > 0) {
        const hashtagString = validated.hashtags
...
export class GenerateSocialContentTool extends BaseTool {
  ...
  description =
    "Generate engaging social media post content using AI based on topic, tone, and platform.";
  category = "automation" as const;
  ...
  parametersSchema = z.object({
    topic: z
      .string()
      .describe(
        'Topic or theme for the post (e.g., "new yoga class", "member success story")',
      ),
    platform: z
      .enum(["facebook", "instagram", "twitter", "linkedin"])
      .describe("Target platform (affects character limits and style)"),
    tone: z
      .enum([
        "professional",
        "casual",
    ...
    maxLength: z
      ...
      .optional()
      .describe(
        "Maximum character length (platform defaults apply if not specified)",
      ),
  });
  ...
  async execute(
    ...
    try {
      ...
      const maxLength =
        validated.maxLength || platformLimits[validated.platform];
      ...
      return {
        ...
        data: {
          ...
          tone: validated.tone,
          message:
            "Social media content generated. Review and edit before scheduling.",
        },
        metadata: {
          executionTimeMs: Date.now() - startTime,
        },
...
export class GenerateHashtagsTool extends BaseTool {
  ...
  description =
    "Generate relevant hashtags for social media posts based on content and industry.";
  category = "automation" as const;
  ...
  parametersSchema = z.object({
    ...
    count: z
      ...
      .max(30)
      .default(10)
      .describe("Number of hashtags to generate (3-30)"),
    industry: z
      .enum([
        "fitness",
        "yoga",
      ...
      .default("fitness")
      .describe("Industry/niche for relevant hashtags"),
    includeLocation: z
      .boolean()
      .default(false)
      .describe("Include location-based hashtags"),
    location: z
      .string()
      .optional()
      .describe('Location for geo-targeted hashtags (e.g., "London", "NYC")'),
  });
  ...
  async execute(
    ...
    try {
      ...
      let hashtags = [...industryHashtags[validated.industry]].slice(
        0,
        validated.count,
      );
...
export class ViewCalendarEventsTool extends BaseTool {
  ...
  description =
    "Get upcoming calendar events, class schedules, and appointments.";
  category = "automation" as const;
  ...
  parametersSchema = z.object({
    startDate: z
      .string()
      .optional()
      .describe("Start date (YYYY-MM-DD format, defaults to today)"),
    endDate: z
      .string()
      .optional()
      .describe("End date (YYYY-MM-DD format, defaults to 7 days from start)"),
    eventType: z
      .enum(["all", "class", "appointment", "meeting"])
      .default("all")
...
export class BookTrialClassTool extends BaseTool {
  ...
  parametersSchema = z.object({
    ...
    notes: z
      .string()
      .optional()
      .describe("Booking notes or special requirements"),
    sendConfirmation: z
      .boolean()
      .default(true)
      .describe("Send confirmation email/SMS"),
  });
...
export class ScheduleFacilityTourTool extends BaseTool {
  ...
  parametersSchema = z.object({
    ...
    tourDate: z
      .string()
      .describe("Tour date and time (YYYY-MM-DD HH:MM format)"),
    duration: z.number().default(30).describe("Tour duration in minutes"),
    assignedStaff: z
      .string()
      .optional()
      .describe("Staff member ID to conduct tour"),
    notes: z.string().optional().describe("Tour notes or special requests"),
    sendReminder: z
      .boolean()
      .default(true)
      .describe("Send reminder 24 hours before tour"),
  });
  ...
  async execute(
    ...
    try {
      ...
      const tourEndTime = new Date(
        new Date(validated.tourDate).getTime() + validated.duration * 60 * 1000,
      );
      ...
      const { data: appointment, error: appointmentError } = await supabase
        ...
        .insert({
          ...
          description:
            validated.notes || "Facility tour for prospective member",
      ...
      await supabase.from("staff_tasks").insert({
        ...
        priority: "high",
        due_date: new Date(
          new Date(validated.tourDate).getTime() - 60 * 60 * 1000,
        ).toISOString(),
        related_lead_id: validated.leadId,
        ai_generated: true,
        ai_context: {
      ...
      await supabase.from("staff_tasks").insert({
        ...
        priority: "high",
        due_date: new Date(
          tourEndTime.getTime() + 2 * 60 * 60 * 1000,
        ).toISOString(),
        related_lead_id: validated.leadId,
        ai_generated: true,
        ai_context: {
