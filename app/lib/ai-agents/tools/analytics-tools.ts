...
export class GenerateRevenueReportTool extends BaseTool {
  ...
  description =
    "Generate comprehensive revenue report with breakdown by month/year, including trends, comparisons, and payment method analysis";
  ...
  parametersSchema = z.object({
    ...
    groupBy: z
      ...
      .default("month")
      .describe("Group data by period"),
    includeBreakdown: z
      .boolean()
      .default(true)
      .describe("Include category and payment method breakdown"),
  });
  ...
  async execute(
    ...
    try {
      ...
      payments?.forEach((payment) => {
        ...
        group.paymentMethods.set(
          method,
          (group.paymentMethods.get(method) || 0) + amount,
        );

        // Track categories
        const category =
          (payment.customer_memberships as any)?.membership_plans?.category ||
          "Uncategorized";
        group.categories.set(
          category,
          (group.categories.get(category) || 0) + amount,
        );
      });
      ...
      const periods = Array.from(grouped.values()).map((g) => ({
        ...
        paymentMethods: includeBreakdown
          ? Object.fromEntries(g.paymentMethods)
          : undefined,
        categories: includeBreakdown
          ? Object.fromEntries(g.categories)
          : undefined,
      }));
      ...
      const avgPeriodRevenue =
        periods.length > 0 ? totalRevenue / periods.length : 0;
      const growth =
        periods.length >= 2
          ? ((periods[periods.length - 1].revenue - periods[0].revenue) /
              periods[0].revenue) *
            100
          : 0;
...
export class GenerateChurnReportTool extends BaseTool {
  ...
  description =
    "Analyze customer churn rate, identify churned customers, and calculate retention metrics";
  category: "analytics" = "analytics";
  ...
  parametersSchema = z.object({
    ...
    includeReasons: z
      .boolean()
      .default(true)
      .describe("Include churn reasons analysis"),
  });
  ...
  async execute(
    ...
    try {
      ...
      const churnRate =
        activeAtStart > 0 ? (churned.length / activeAtStart) * 100 : 0;
      ...
      // Monthly breakdown
      const monthlyChurn = new Map<
        string,
        { churned: number; active: number }
      >();
      churned.forEach((m) => {
        if (m.cancelled_at) {
          const month = new Date(m.cancelled_at).toISOString().substring(0, 7);
          const existing = monthlyChurn.get(month) || {
            churned: 0,
      ...
      return {
        ...
        data: {
          ...
          monthlyBreakdown: Array.from(monthlyChurn.entries()).map(
            ([month, data]) => ({
              month,
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to generate churn report",
      };
    }
  }
}
...
export class GenerateLTVReportTool extends BaseTool {
  ...
  description =
    "Calculate customer lifetime value (LTV) with breakdown by cohort, plan, and acquisition source";
  category: "analytics" = "analytics";
  ...
  parametersSchema = z.object({
    cohortBy: z
      ...
      .default("month")
      .describe("Group customers into cohorts"),
    includeProjection: z
      .boolean()
      .default(true)
      .describe("Include LTV projection"),
  });
  ...
  async execute(
    ...
    try {
      ...
      const customerLTVs =
        customers?.map((customer) => {
          ...
          return {
            ...
            lifespanDays,
            avgMonthlyValue:
              lifespanDays > 0 ? (totalSpent / lifespanDays) * 30 : 0,
          };
        }) || [];
      ...
      cohorts.forEach((cohort) => {
        cohort.avgLTV =
          cohort.customers > 0 ? cohort.totalLTV / cohort.customers : 0;
        cohort.avgLifespan =
          cohort.customers > 0 ? cohort.avgLifespan / cohort.customers : 0;
      });
      ...
      customerLTVs.forEach((c) => {
        ...
        sourceData.avgLTV =
          (sourceData.avgLTV * sourceData.customers + c.totalSpent) /
          (sourceData.customers + 1);
        sourceData.customers++;
      });

      const totalLTV = customerLTVs.reduce((sum, c) => sum + c.totalSpent, 0);
      const avgLTV =
        customerLTVs.length > 0 ? totalLTV / customerLTVs.length : 0;
      const medianLTV =
        customerLTVs.length > 0
          ? customerLTVs.sort((a, b) => a.totalSpent - b.totalSpent)[
              Math.floor(customerLTVs.length / 2)
            ].totalSpent
          : 0;
...
export class GenerateMonthlyTurnoverReportTool extends BaseTool {
  ...
  description =
    "Generate detailed monthly turnover report with category breakdown, trends, and year-over-year comparisons";
  category: "analytics" = "analytics";
  ...
  parametersSchema = z.object({
    ...
    includeComparison: z
      .boolean()
      .default(true)
      .describe("Include year-over-year comparison"),
  });
    ...
    try {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to generate monthly turnover report",
      };
    }
  }
}
...
export class CalculateMRRTool extends BaseTool {
  ...
  name = "Calculate MRR";
  description =
    "Calculate Monthly Recurring Revenue (MRR) with breakdown by plan type and growth metrics";
  category: "analytics" = "analytics";
  ...
  parametersSchema = z.object({
    asOfDate: z
      ...
      .optional()
      .describe(
        "Calculate MRR as of this date (YYYY-MM-DD), defaults to today",
      ),
  });
    ...
    try {
      ...
      memberships?.forEach((membership) => {
        const monthlyPrice = parseFloat(
          (membership.monthly_price as any) || "0",
        );
        ...
        const category =
          (membership.membership_plans as any)?.category || "Uncategorized";
        categoryBreakdown.set(
          category,
          (categoryBreakdown.get(category) || 0) + monthlyPrice,
        );
      });
      ...
      return {
        ...
        data: {
          ...
          avgRevenuePerMember: memberships?.length
            ? totalMRR / memberships.length
            : 0,
          asOfDate,
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to calculate MRR",
      };
    }
  }
}
...
export class CalculateARRTool extends BaseTool {
  ...
  name = "Calculate ARR";
  description =
    "Calculate Annual Recurring Revenue (ARR) with growth trends and projections";
  category: "analytics" = "analytics";
  ...
  parametersSchema = z.object({
    asOfDate: z
      ...
      .optional()
      .describe(
        "Calculate ARR as of this date (YYYY-MM-DD), defaults to today",
      ),
  });
  ...
  async execute(
    ...
    try {
      ...
      const mrr =
        memberships?.reduce(
          (sum, m) => sum + parseFloat((m.monthly_price as any) || "0"),
          0,
        ) || 0;
      const arr = mrr * 12;
      ...
      const lastYearMRR =
        lastYearMemberships?.reduce(
          (sum, m) => sum + parseFloat((m.monthly_price as any) || "0"),
          0,
        ) || 0;
      const lastYearARR = lastYearMRR * 12;
      const growth =
        lastYearARR > 0 ? ((arr - lastYearARR) / lastYearARR) * 100 : 0;
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to calculate ARR",
      };
    }
  }
}
...
export class AnalyzePaymentTrendsTool extends BaseTool {
  ...
  description =
    "Analyze payment patterns, methods, timing, and identify anomalies or trends";
  category: "analytics" = "analytics";
  ...
  async execute(
    ...
    try {
      ...
      const { data: payments, error } = await supabase
        .from("payments")
        .select(
          "payment_date, amount, payment_method, payment_status, created_at",
        )
        .eq("organization_id", context.organizationId)
      ...
      return {
        ...
        data: {
          ...
          timing: {
            ...
            dayOfWeekDistribution: dayNames.map((day, i) => ({
              day,
              count: dayOfWeek[i],
            })),
            hourlyDistribution: hourOfDay.map((count, hour) => ({
              hour,
              count,
            })),
          },
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to analyze payment trends",
      };
    }
  }
}
...
export class AnalyzeClassAttendanceTool extends BaseTool {
  ...
  description =
    "Analyze class attendance patterns, capacity utilization, and popular class times";
  category: "analytics" = "analytics";
  ...
  parametersSchema = z.object({
    ...
    programId: z
      .string()
      .optional()
      .describe("Filter by specific program/class type"),
  });
  ...
  async execute(
    ...
    try {
      ...
      sessions?.forEach((session) => {
        ...
        const attended =
          (session.bookings as any[])?.filter((b) => b.attended_at).length || 0;
      ...
      return {
        ...
        data: {
          ...
          byProgram: Array.from(programStats.entries()).map(
            ([program, stats]) => ({
              program,
              sessions: stats.sessions,
              avgCapacity:
                stats.sessions > 0 ? stats.capacity / stats.sessions : 0,
              avgBookings:
                stats.sessions > 0 ? stats.bookings / stats.sessions : 0,
              utilizationRate:
                stats.capacity > 0
                  ? (stats.bookings / stats.capacity) * 100
                  : 0,
              attendanceRate:
                stats.bookings > 0
                  ? (stats.attended / stats.bookings) * 100
                  : 0,
            }),
          ),
        },
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to analyze class attendance",
      };
    }
  }
}
...
export class AnalyzeMemberEngagementTool extends BaseTool {
  ...
  description =
    "Analyze member engagement metrics including activity levels, visit frequency, and engagement trends";
  category: "analytics" = "analytics";
    ...
    try {
          ...
          summary: {
            ...
            engagementDistribution: {
              high: {
                ...
                percentage:
                  totalClients > 0
                    ? (engagementLevels.high / totalClients) * 100
                    : 0,
              },
              medium: {
                ...
                percentage:
                  totalClients > 0
                    ? (engagementLevels.medium / totalClients) * 100
                    : 0,
              },
              low: {
                ...
                percentage:
                  totalClients > 0
                    ? (engagementLevels.low / totalClients) * 100
                    : 0,
              },
              inactive: {
                ...
                percentage:
                  totalClients > 0
                    ? (engagementLevels.inactive / totalClients) * 100
                    : 0,
              },
            },
          },
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to analyze member engagement",
      };
    }
  }
}
...
export class AnalyzeNoShowRatesTool extends BaseTool {
  ...
  description =
    "Analyze booking no-show rates with breakdown by class, time, and customer patterns";
  category: "analytics" = "analytics";
  ...
  async execute(
    ...
    try {
      ...
      // No-shows by program
      const programNoShows = new Map<
        string,
        { total: number; noShows: number }
      >();
      pastBookings.forEach((booking) => {
        const program =
          (booking.class_sessions as any)?.programs?.name || "Unknown";
      ...
      return {
        ...
        data: {
          ...
          byProgram: Array.from(programNoShows.entries()).map(
            ([program, stats]) => ({
              program,
              totalBookings: stats.total,
              noShows: stats.noShows,
              noShowRate:
                stats.total > 0 ? (stats.noShows / stats.total) * 100 : 0,
            }),
          ),
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to analyze no-show rates",
      };
    }
  }
}
...
export class IdentifyAtRiskMembersTool extends BaseTool {
  ...
  description =
    "Identify members at risk of churning based on engagement patterns, payment history, and activity";
  category: "analytics" = "analytics";
  ...
  parametersSchema = z.object({
    daysInactive: z
      .number()
      .default(30)
      .describe("Number of days without activity to consider at-risk"),
    includeRecommendations: z
      .boolean()
      .default(true)
      .describe("Include retention recommendations"),
  });
    ...
    try {
        ...
        clients
          ?.map((client) => {
            ...
            const recentBookings = bookings.filter(
              (b) => new Date(b.booking_time) >= cutoffDate,
            );

            const recentAttendance = bookings.filter(
              (b) => b.attended_at && new Date(b.attended_at) >= cutoffDate,
            );

            const recentPayments = payments.filter(
              (p) =>
                ["paid_out", "succeeded", "confirmed"].includes(
                  p.payment_status,
      ...
      return {
        ...
        data: {
          summary: {
            ...
            highRisk: atRiskMembers.filter((m) => m.riskLevel === "high")
              .length,
            mediumRisk: atRiskMembers.filter((m) => m.riskLevel === "medium")
              .length,
          ...
          atRiskMembers: atRiskMembers
            ...
            .map((m) => ({
              ...
              lastActivity:
                m.lastActivity > 0
                  ? new Date(m.lastActivity).toISOString()
                  : null,
            })),
        },
        metadata: {
      ...
      return {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Failed to identify at-risk members",
      };
    }
  }
}
...
export class GenerateOperationsReportTool extends BaseTool {
  ...
  description =
    "Generate comprehensive operations dashboard with key metrics, capacity, staff performance, and operational insights";
  category: "analytics" = "analytics";
    ...
    try {
      ...
      const [
        ...
        supabase
          .from("class_sessions")
          .select(
            "id, start_time, max_capacity, current_bookings, session_status",
          )
          .eq("organization_id", context.organizationId)
          .gte("start_time", params.startDate)
          .lte("start_time", params.endDate),
      ...
      // Calculate metrics
      const totalCapacity =
        sessions?.reduce((sum, s) => sum + (s.max_capacity || 0), 0) || 0;
      const totalBookings =
        sessions?.reduce((sum, s) => sum + (s.current_bookings || 0), 0) || 0;
      const utilizationRate =
        totalCapacity > 0 ? (totalBookings / totalCapacity) * 100 : 0;
      ...
      const attendanceRate =
        confirmedBookings > 0 ? (attended / confirmedBookings) * 100 : 0;
      ...
      return {
        ...
        data: {
          ...
          revenue: {
            ...
            avgTransactionSize: payments?.length
