...
export function exampleBrowseTools() {
  ...
  console.log(
    'Tools related to "client":',
    clientTools.map((t) => t.id),
  );
...
export function exampleFilterToolsByPermission() {
  ...
  console.log(
    "Public tools:",
    publicTools.map((t) => t.id),
  );

  // Tools that require specific permissions
  const clientReadTools = allTools.filter(
    (t) => t.requiresPermission === "clients:read",
  );
  console.log(
    "Client read tools:",
    clientReadTools.map((t) => t.id),
  );

  const clientWriteTools = allTools.filter(
    (t) => t.requiresPermission === "clients:write",
  );
  console.log(
    "Client write tools:",
    clientWriteTools.map((t) => t.id),
  );
