...
export class AgentScheduler {
  ...
  async start(): Promise<void> {
    ...
    console.log(
      `[AgentScheduler] Scheduler started (polling every ${POLL_INTERVAL_MS / 1000}s)`,
    );
  }
  ...
  async checkScheduledTasks(): Promise<void> {
    ...
    try {
      ...
      if (error) {
        console.error(
          "[AgentScheduler] Error querying scheduled tasks:",
          error,
        );
        return;
      }
        ...
        try {
          ...
          if (queued) {
            ...
            if (task.schedule_cron) {
              ...
              console.log(
                `[AgentScheduler] Task ${task.id} queued (one-time execution)`,
              );
            }
          } else {
            tasksFailedThisCheck++;
          ...
          console.error(
            `[AgentScheduler] Error processing task ${task.id}:`,
            taskError,
          );
          tasksFailedThisCheck++;
          this.metrics.tasksFailed++;
  ...
  async queueTask(taskId: string): Promise<boolean> {
    ...
    while (attempts < RETRY_QUEUE_ATTEMPTS) {
      try {
        ...
        if (fetchError || !task) {
          console.error(
            `[AgentScheduler] Task not found: ${taskId}`,
            fetchError,
          );
          return false;
        }
        ...
        if (task.schedule_cron) {
          ...
          if (updateError) {
            console.error(
              `[AgentScheduler] Error updating task ${taskId} status:`,
              updateError,
            );
          }
        }
