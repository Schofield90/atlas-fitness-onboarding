 ...
 */
export async function createUserOrganization(userId: string, organizationId: string, role: string = 'member') {
  ...
  if (error) {
    throw new Error(`Failed to create organization association: ${error.message}`)
  }
