...
function getRedisConnection() {
  if (!redisConnection && !isBuildTime && Redis) {
    try {
...
let _workflowQueue: any = null;
export function getWorkflowQueue() {
  if (!_workflowQueue && !isBuildTime) {
    const conn = getRedisConnection();
    if (conn && Queue) {
      _workflowQueue = new Queue("workflow-executions", {
        connection: conn,
        defaultJobOptions: {
          removeOnComplete: {
            count: 100,
...
export const workflowQueue = null; // For backward compatibility, will be null during build

let _priorityQueue: any = null;
export function getPriorityQueue() {
  if (!_priorityQueue && !isBuildTime) {
    const conn = getRedisConnection();
    if (conn && Queue) {
      _priorityQueue = new Queue("priority-executions", {
        connection: conn,
        defaultJobOptions: {
          removeOnComplete: {
            count: 50,
...
export const priorityQueue = null; // For backward compatibility

// Queue for delayed actions (wait nodes)
let _delayedQueue: any = null;
export function getDelayedQueue() {
  if (!_delayedQueue && !isBuildTime) {
    const conn = getRedisConnection();
    if (conn && Queue) {
      _delayedQueue = new Queue("delayed-actions", {
        connection: conn,
      });
    }
  }
...
export const delayedQueue = null; // For backward compatibility

// Queue events for monitoring
let _queueEvents: any = null;
function getQueueEvents() {
  if (!_queueEvents && !isBuildTime) {
    const conn = getRedisConnection();
    if (conn && QueueEvents) {
      _queueEvents = new QueueEvents("workflow-executions", {
        connection: conn,
      });
    }
  }
...
export async function enqueueWorkflowExecution(
  ...
  const queue =
    options?.priority && options.priority > 5 ? priorityQueue : workflowQueue;
...
async function emitWorkflowEvent(event: WorkflowEvent): Promise<void> {
  ...
  await supabase.channel(`workflow-events-${event.workflowId}`).send({
    type: "broadcast",
...
export async function getQueueStats() {
  const [workflowStats, priorityStats, delayedStats] = await Promise.all([
    getQueueMetrics(getWorkflowQueue()),
    getQueueMetrics(getPriorityQueue()),
    getQueueMetrics(getDelayedQueue()),
  ]);
...
export async function pauseQueue(
  queueName: "workflow" | "priority" | "delayed",
) {
  const queue =
    queueName === "workflow"
      ? workflowQueue
