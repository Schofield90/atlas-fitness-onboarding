...
// Create Redis connection for DLQ - only if not during build
...
if (!isBuildTime && Redis) {
  try {
    ...
    console.warn("Failed to create Redis connection for DLQ:", error);
  }
}
...
export class ErrorClassifier {
  ...
  static classifyError(
    error: any,
  ): "transient" | "persistent" | "configuration" | "data" | "unknown" {
    ...
    if (
      transientPatterns.some(
        (pattern) =>
          pattern.test(errorMessage) || (errorCode && pattern.test(errorCode)),
      )
    ) {
      return "transient";
    }
...
export class DeadLetterQueueManager {
  ...
  async addToDeadLetterQueue(
    ...
    try {
      ...
      const isRecoverable = ErrorClassifier.isRecoverable(
        classification,
        originalJob.attemptsMade,
      );
      const recoveryStrategy =
        ErrorClassifier.getRecoveryStrategy(classification);
      ...
      const job = await this.dlQueue.add("process-dead-letter", dlJobData, {
  ...
  private async processDeadLetterJob(job: any): Promise<any> {
    ...
    try {
      ...
      console.error(
        `💀 Failed to process dead letter job ${job.id}:`,
        processingError,
      );
  ...
  private async analyzeFailure(dlJobData: DeadLetterJobData): Promise<any> {
    ...
    return {
      failurePattern: {
        ...
        affectedWorkflows: new Set(recentFailures.map((f) => f.workflow_id))
          .size,
  ...
  private async determineAction(
    ...
    if (
      !analysis.systemHealth.redisHealthy ||
      !analysis.systemHealth.databaseHealthy
    ) {
      return {
        type: "delay_retry",
  ...
  private async checkSystemHealth(): Promise<any> {
    try {
      // Check Redis health
      const redisHealthy = redisConnection
        ? (await redisConnection.ping()) === "PONG"
        : false;
...
Please investigate and resolve this issue. Update the task status when complete.
  ...
  private async updateDeadLetterJobRecord(
    dlJobData: DeadLetterJobData,
    updates: any,
  ): Promise<void> {
    try {
  ...
  private async sendEscalationNotifications(
    ...
    console.log(
      `🚨 Escalation notifications sent for job ${dlJobData.originalJob.id}`,
    );
  }
  ...
  private async notifyTaskAssignment(
    ...
    console.log(
      `📋 Task assignment notifications sent for job ${dlJobData.originalJob.id}`,
    );
  }
