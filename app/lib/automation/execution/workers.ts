...
export class WorkerManager {
  ...
  async startAll(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `✅ All workers started successfully (${this.workers.size} workers)`,
      );
  ...
  private async startWorkflowWorker(): Promise<void> {
    ...
    console.log(
      `✅ Workflow execution worker started (concurrency: ${config.concurrency})`,
    );
  }
  ...
  private async startPriorityWorker(): Promise<void> {
    ...
    console.log(
      `✅ Priority execution worker started (concurrency: ${config.concurrency})`,
    );
  }
  ...
  private async startDelayedActionWorker(): Promise<void> {
    ...
    console.log(
      `✅ Delayed action worker started (concurrency: ${config.concurrency})`,
    );
  }
  ...
  private async startDeadLetterWorker(): Promise<void> {
    ...
    console.log(
      `✅ Dead letter queue worker started (concurrency: ${config.concurrency})`,
    );
  }
  ...
  private createJobProcessor(queueType: string) {
    return async (job: any) => {
      ...
      try {
        ...
        const result = await JobProcessorFactory.processJob(
          job.data as JobData,
        );
  ...
  private createDelayedActionProcessor() {
    return async (job: any) => {
      ...
      try {
        const { executionId, nodeId, resumeData, workflowId, organizationId } =
          job.data;
        ...
        // Resume execution
        const executor = await WorkflowExecutor.resume(
          executionId,
          nodeId,
          resumeData,
        );
  ...
  private async logJobError(
    ...
    try {
      ...
      await supabase.from("worker_error_logs").insert({
  ...
  private async notifyAdministrators(
    ...
    try {
      console.warn(
        "🚨 Critical job failure - Administrator notification triggered:",
        {
  ...
  private async canRecoverJob(originalJob: any, error: any): Promise<boolean> {
    ...
    const isRecoverableError = recoverableErrors.some((pattern) =>
  ...
  private async performHealthCheck(): Promise<void> {
    try {
      ...
      // Log health status
      const unhealthyWorkers = healthStatuses.filter(
        (h) => h.status !== "healthy",
      );
      if (unhealthyWorkers.length > 0) {
        console.warn(
          "⚠️  Unhealthy workers detected:",
          unhealthyWorkers.map((w) => w.name),
        );
      }
  ...
  private async collectMetrics(): Promise<void> {
    try {
      const metrics = {
        timestamp: new Date().toISOString(),
        workers: Array.from(this.workerStats.entries()).map(
          ([name, stats]) => ({
            name,
            ...stats,
          }),
    ...
    const gracefulShutdown = async (signal: string) => {
      ...
      try {
        ...
        while (Date.now() - startTime < shutdownTimeout) {
          const activeJobs = Array.from(this.workerStats.values()).reduce(
            (total, stats) => total + (stats.jobsActive || 0),
            0,
          );
          ...
          console.log(
            `⏳ Waiting for ${activeJobs} active jobs to complete...`,
          );
...
// Export singleton worker manager - lazy initialization
...
export const workerManager = null; // For backward compatibility - use getWorkerManager() instead
