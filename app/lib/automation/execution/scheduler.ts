...
// Scheduler queue - lazy initialization
let _schedulerQueue: any = null;
export function getSchedulerQueue() {
  if (!_schedulerQueue && redisConnection && Queue) {
    _schedulerQueue = new Queue("workflow-scheduler", {
      connection: redisConnection,
    });
  }
  return _schedulerQueue;
}
export const schedulerQueue = null; // For backward compatibility
...
export async function initializeScheduler() {
  ...
  for (const schedule of schedules || []) {
    try {
      ...
      console.log(
        `Scheduled workflow ${schedule.workflow_id} with trigger ${schedule.id}`,
      );
    } catch (error) {
      console.error(
        `Failed to schedule workflow ${schedule.workflow_id}:`,
        error,
      );
    }
  }
...
export function getNextRunTimes(config: any, count: number = 5): Date[] {
  const times: Date[] = [];
