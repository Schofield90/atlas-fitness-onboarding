...
export class QueueManagementService {
  ...
  async performHealthCheck(): Promise<QueueHealthStatus> {
    ...
    try {
      ...
      console.log(
        `🏥 Health check completed in ${checkTime}ms - Status: ${overall}`,
      );
  ...
  // Build queue status
  private async buildQueueStatus(
    queueName: string,
    stats: any,
  ): Promise<QueueStatus> {
    try {
      ...
      const successRate =
        totalJobs > 0 ? (stats.completed / totalJobs) * 100 : 100;
      ...
      const completedPerMinute = this.calculateThroughput(
        queueName,
        "completed",
      );
      ...
      if (stats.waiting > this.config.alertThresholds.queueDepth) {
        ...
        recommendations.push(
          "Consider scaling up workers or investigating processing bottlenecks",
        );
      }
      ...
      if (errorRate > this.config.alertThresholds.errorRate) {
        ...
        recommendations.push(
          "Investigate job failures and improve error handling",
        );
      }
      ...
      if (stats.stalled > 10) {
        ...
        recommendations.push(
          "Check worker health and consider restarting stalled jobs",
        );
      }
  ...
  private generateRecommendations(
    ...
    for (const [queueName, queue] of Object.entries(queues)) {
      ...
      if (q.metrics.waiting > 500) {
        recommendations.push(
          `Consider increasing concurrency for ${queueName} queue`,
        );
      }

      if (q.performance.errorRate > 0.05) {
        recommendations.push(
          `Investigate and fix high error rate in ${queueName} queue`,
        );
      }
    ...
    if (unhealthyWorkers.length > 0) {
      recommendations.push(
        "Review worker health and consider restarting unhealthy workers",
      );
    }
    ...
    if (redis.connected && redis.memory.fragmentation > 1.5) {
      recommendations.push(
        "Redis memory fragmentation is high - consider restarting Redis during maintenance window",
      );
    }
    ...
    if (memoryUsage > 0.8) {
      recommendations.push(
        "Memory usage is high - consider increasing heap size or optimizing memory usage",
      );
    }

    if (system.loadAverage[0] > 5) {
      recommendations.push(
        "CPU load is high - consider scaling up infrastructure",
      );
    }
  ...
  private calculateQueueHealthScore(stats: any, performance: any): number {
    ...
    if (performance.avgProcessingTime > 60000) {
      score -= Math.min(
        20,
        ((performance.avgProcessingTime - 60000) / 60000) * 10,
      );
    }
  ...
  private determineWorkerStatus(
    ...
    if (
      stats.lastError &&
      Date.now() - new Date(stats.lastError).getTime() < 300000
    )
      return "warning";
      ...
      const metrics: PerformanceMetrics = {
        ...
        queues: {
          workflow: {
            ...
            errorRate:
              queueStats.workflow.failed /
                (queueStats.workflow.completed + queueStats.workflow.failed) ||
              0,
          ...
          priority: {
            ...
            errorRate:
              queueStats.priority.failed /
                (queueStats.priority.completed + queueStats.priority.failed) ||
              0,
          ...
          delayed: {
            ...
            errorRate:
              queueStats.delayed.failed /
                (queueStats.delayed.completed + queueStats.delayed.failed) || 0,
            avgProcessingTime: await this.getAverageProcessingTime("delayed"),
            queueDepth: queueStats.delayed.waiting,
          },
        },
        workers: Object.fromEntries(
          Object.entries(workerStatus.stats).map(
        ...
        system: {
          ...
          memoryUsage:
            systemStatus.memoryUsage.heapUsed /
            systemStatus.memoryUsage.heapTotal,
        ...
        redis: {
          ...
          memoryUsage:
            parseInt(redisStatus.memory.used.replace(/[^\d]/g, "")) || 0,
  ...
  private async performMaintenance(): Promise<void> {
    try {
      ...
      const isOffPeak =
        this.config.maintenance.offPeakHours.includes(currentHour);
  ...
  private async storeHealthStatus(
    ...
    try {
      ...
      await supabase.from("queue_health_status").insert({
  ...
  private async storeMetrics(metrics: PerformanceMetrics): Promise<void> {
    try {
      ...
      await supabase.from("queue_performance_metrics").insert({
  ...
  private async storeAlert(alert: Alert): Promise<void> {
    try {
      ...
      await supabase.from("queue_alerts").insert({
        alert_id: alert.id,
  ...
  private async cleanupOldMetrics(): Promise<void> {
    try {
      ...
      const cutoffDate = new Date(
        Date.now() - this.config.maintenance.retentionPeriod,
      ).toISOString();
  ...
  async getAlerts(active?: boolean): Promise<Alert[]> {
    if (active !== undefined) {
      return this.alerts.filter((a) =>
        active ? !a.resolvedAt : !!a.resolvedAt,
      );
    }
    return this.alerts;
  }
...
// Export singleton queue management service - lazy initialization
let _queueManagementService: QueueManagementService | null = null;
export function getQueueManagementService() {
  if (!_queueManagementService) {
    _queueManagementService = new QueueManagementService();
  }
  return _queueManagementService;
}
export const queueManagementService = null; // For backward compatibility - use getQueueManagementService() instead
