...
class FacebookAdsErrorHandler {
    ...
    for (let attempt = 0; attempt <= retryConfig.maxRetries; attempt++) {
      try {
        ...
        if (attempt < retryConfig.maxRetries && this.shouldRetry(fbError)) {
          ...
          console.warn(`Facebook API error (attempt ${attempt + 1}/${retryConfig.maxRetries + 1}):`, {
            error: fbError,
            retryingIn: `${delay}ms`
          });
        ...
        if (attempt < retryConfig.maxRetries) {
          ...
          console.warn(`Network error (attempt ${attempt + 1}/${retryConfig.maxRetries + 1}):`, {
            error: lastError,
            retryingIn: `${delay}ms`
          });
   ...
   */
  private static getErrorSeverity(error: FacebookError): 'info' | 'warning' | 'critical' {
    if (this.OAUTH_ERROR_CODES.has(error.code)) {
...
export class FacebookAPIClient {
   ...
   */
  public async get<T>(endpoint: string, params: Record<string, any> = {}): Promise<FacebookAPIResponse<T>> {
   ...
   */
  public async post<T>(endpoint: string, body: Record<string, any> = {}): Promise<FacebookAPIResponse<T>> {
   ...
   */
  public async patch<T>(endpoint: string, body: Record<string, any> = {}): Promise<FacebookAPIResponse<T>> {
    const url = `https://graph.facebook.com/v18.0/${endpoint}`;
