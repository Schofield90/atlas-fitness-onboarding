...
export class EnhancedWorkflowExecutor extends EventEmitter {
  ...
  async execute(triggerData: any): Promise<void> {
    try {
      ...
      const triggerValid = await this.evaluateTriggerConditions(
        triggerNode,
        triggerData,
      );
      if (!triggerValid) {
  ...
  private async executeAction(
    ...
    const job = await getQueueManager().addJob(
    ...
    if (!node.data?.async) {
      const result = await job.waitUntilFinished(getQueueManager().events);
      return result;
    }
  ...
  private async executeLoop(
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (const bodyNode of bodyNodes) {
        const result = await this.executeNodeWithContext(
          bodyNode,
          workflow,
          loopContext,
        );
  ...
  private async executeWebhook(
    ...
    const headers = this.interpolateObject(
      webhookConfig.headers || {},
      this.context,
    );
    const payload = this.interpolateObject(
  ...
  private async getNextNodes(
    ...
    if (currentNode.type === "condition" && result.output?.branch) {
      targetEdges = targetEdges.filter(
        (e) => e.data?.branch === result.output.branch || !e.data?.branch,
      );
    }
  ...
  private async calculateDynamicDelay(
    config: any,
    context: any,
  ): Promise<number> {
