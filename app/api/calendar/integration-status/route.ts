...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: googleToken, error: tokenError } = await supabase
      ...
      .select('sync_enabled, calendar_id, expiry_date, auto_create_events, created_at, updated_at')
      .eq('user_id', user.id)
      .single()

    // Check if token exists and is valid
    const tokenExists = !!googleToken && !tokenError
    const isExpired = googleToken?.expiry_date ? new Date(googleToken.expiry_date) < new Date() : false
