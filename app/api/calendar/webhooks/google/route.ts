  ...
  try {
    ...
    for (const booking of bookings) {
      ...
      if (!googleEvent) {
        ...
        console.log(
          `Google event ${booking.google_event_id} not found - cancelling booking ${booking.id}`,
        );
        ...
        // Create audit record
        await adminSupabase.from("booking_audit").insert({
        ...
        const timeChanged =
          googleStart.getTime() !== bookingStart.getTime() ||
          googleEnd.getTime() !== bookingEnd.getTime();

        if (timeChanged) {
          console.log(
            `Google event ${booking.google_event_id} time changed - updating booking ${booking.id}`,
          );
...
function generateAutoRescheduleEmail(
  booking: any,
  newStart: Date,
  newEnd: Date,
): string {
