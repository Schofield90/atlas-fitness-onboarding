...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: localEvents } = await adminSupabase
      ...
      .eq('organization_id', user.user_metadata?.organization_id || '63589490-8f55-4157-bd3a-e141594b748e')
    
    console.log(`Fetched ${localEvents?.length || 0} events from local database`)
    ...
    for (const [googleId, localEvent] of localEventsByGoogleId) {
      if (!googleEventIds.has(googleId)) {
        ...
        console.log(`Deleting local event ${localEvent.id} (Google event ${googleId} was deleted)`)
    ...
    for (const googleEvent of googleEvents) {
      ...
      if (localEvent) {
        ...
        await adminSupabase
          ...
          .insert({
            ...eventData,
            organization_id: user.user_metadata?.organization_id || '63589490-8f55-4157-bd3a-e141594b748e',
            created_by: user.id
          })
    ...
    const localEventsWithoutGoogle = (localEvents || []).filter(e => !e.google_event_id)
...
async function fetchAllGoogleEvents(calendar: any, timeMin: string, timeMax: string) {
  const events: any[] = []
...
function hasEventChanged(localEvent: any, googleEventData: any): boolean {
  return (
    ...
    JSON.stringify(localEvent.attendees) !== JSON.stringify(googleEventData.attendees)
  )
}