...
interface ActivityItem {
  id: string;
  type: 'booking' | 'payment' | 'note' | 'communication' | 'membership' | 'login';
  title: string;
  description: string;
  date: string;
  metadata?: any;
  ...
  try {
    ...
    if (bookings) {
      bookings.forEach((booking) => {
        const className = booking.class_session?.name || booking.class_session?.programs?.name || 'Unknown Class';
    ...
    if (messages) {
      messages.forEach((message) => {
        activities.push({
          ...
          type: 'communication',
          title: message.direction === 'outbound' ? 'Message Sent' : 'Message Received',
          description: message.body ? (message.body.length > 100 ? message.body.substring(0, 100) + '...' : message.body) : 'Media message',
    ...
    if (notes) {
      notes.forEach((note) => {
        activities.push({
          ...
          type: 'note',
          title: 'Note Added',
          description: `${note.users?.name || 'Staff'} added a note: ${note.content.length > 100 ? note.content.substring(0, 100) + '...' : note.content}`,
