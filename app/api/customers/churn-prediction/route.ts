...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const customer of customers || []) {
      try {
        const churnData = await analyzeCustomerChurnRisk(supabase, customer, organization.id);
        ...
        console.error(`Error analyzing churn for customer ${customer.id}:`, error);
      }
    }
    ...
    return NextResponse.json({
      ...
      high_risk_customers: predictions.filter(p => p.risk_level === 'high').length,
      medium_risk_customers: predictions.filter(p => p.risk_level === 'medium').length,
      low_risk_customers: predictions.filter(p => p.risk_level === 'low').length,
      predictions
    });
...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedCustomers = (customers || []).map(customer => ({
      ...customer,
      risk_level: customer.churn_risk_score >= 0.7 ? 'high' : 
...
async function analyzeCustomerChurnRisk(
  ...
  const lastVisitDaysAgo = customer.last_visit_date 
    ? Math.floor((now.getTime() - new Date(customer.last_visit_date).getTime()) / (24 * 60 * 60 * 1000))
    : 999;
  ...
  const cancellationRate = bookings?.length ? cancelledBookings.length / bookings.length : 0;
  ...
  const totalPayments = payments?.reduce((sum, p) => 
    p.status === 'succeeded' ? sum + p.amount_pennies : sum, 0
  ) || 0;
...
function calculateChurnPrediction(data: CustomerChurnData): ChurnPrediction {
  ...
  if (data.membership_status === 'cancelled' || data.membership_status === 'expired') {
    churnScore += 0.15;
  ...
  // Payment issues
  const recentFailures = data.payment_history.filter(p => 
    p.status === 'failed' && 
