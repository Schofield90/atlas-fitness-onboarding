    ...
    const usageSummary =
      usageMetrics?.reduce((acc, metric) => {
        return {
          ...
          bookings_created:
            (acc.bookings_created || 0) + metric.bookings_created,
          active_customers: Math.max(
            acc.active_customers || 0,
...
export async function POST(request: NextRequest) {
  try {
    ...
    let stripeCustomerId =
      organization.saas_subscriptions?.[0]?.stripe_customer_id;
    ...
    const existingSubscriptionId =
      organization.saas_subscriptions?.[0]?.stripe_subscription_id;
    ...
    if (existingSubscriptionId) {
      ...
      stripeSubscription = await stripe.subscriptions.update(
        existingSubscriptionId,
        {
          items: [
            {
              id: (await stripe.subscriptions.retrieve(existingSubscriptionId))
                .items.data[0].id,
    ...
    await supabase.from("saas_subscriptions").upsert(subscriptionData, {
