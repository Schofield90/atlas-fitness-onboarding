...
export async function POST(request: NextRequest) {
  try {
    ...
    if (insertError) {
      ...
      if (insertError.message.includes('column') || insertError.message.includes('max_days_in_advance')) {
        return NextResponse.json({ 
          error: 'Database schema needs updating',
          message: 'The booking_links table is missing required columns. Please contact support to apply the migration.',
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      requiredColumnsPresent: columns.includes('max_days_in_advance') && 
