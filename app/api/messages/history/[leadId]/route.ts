...
export async function GET(
  ...
  try {
    ...
    const phoneConditions = uniquePhones
    ...
    const { data: whatsappMessages = [], error: whatsappError } =
      await adminSupabase
        .from("whatsapp_logs")
        .select("*")
        .or(phoneConditions)
    ...
    console.log(
      "Total messages to return:",
      smsMessages.length + whatsappMessages.length + emailMessages.length,
    );
    ...
    const allMessages = [
      ...smsMessages.map((msg) => ({
        ...
        type: "sms" as const,
        direction:
          msg.from_number === normalizedPhone
            ? ("inbound" as const)
            : ("outbound" as const),
        status: msg.status,
        body: msg.message,
    ...
    smsMessages.forEach((msg) => {
      ...
      if (!allMessagesMap.has(key)) {
        const direction =
          msg.from_number === normalizedPhone || msg.from_number === lead.phone
            ? "inbound"
            : "outbound";
    ...
    return NextResponse.json({
      messages: {
        ...
        calls: mergedMessages.filter(
          (m) => m.type === "call" || (m.body && m.body.includes("Phone call")),
        ),
      },
      leadId,
    });
