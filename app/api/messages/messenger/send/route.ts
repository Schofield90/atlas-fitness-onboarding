  ...
  try {
    ...
    if (hoursSinceLastInbound > 24) {
      return NextResponse.json({ 
        ...
        message: 'Cannot send message outside 24-hour window. Last customer message was ' + 
                 (lastInbound ? `${Math.floor(hoursSinceLastInbound)} hours ago` : 'never received')
      }, { status: 400 })
    }
    ...
    if (!psid) {
      return NextResponse.json({ error: 'Invalid conversation thread ID' }, { status: 400 })
    }
    ...
    try {
      if (attachmentUrl && attachmentType) {
        messageId = await client.sendAttachment(psid, attachmentType, attachmentUrl)
      } else {
