...
export async function PUT(
  ...
  try {
    ...
    if (body.break_start !== undefined) updateData.break_start = body.break_start
    if (body.break_end !== undefined) updateData.break_end = body.break_end
    if (body.break_duration !== undefined) updateData.break_duration = body.break_duration
    ...
    if (body.break_duration !== undefined || body.hourly_rate !== undefined) {
      ...
      const clockOut = existingTimesheet.clock_out ? new Date(existingTimesheet.clock_out) : null
      
      if (clockOut) {
        const totalMinutes = Math.floor((clockOut.getTime() - clockIn.getTime()) / (1000 * 60))
        const breakDuration = body.break_duration ?? existingTimesheet.break_duration
        ...
        if (hourlyRate) {
          updateData.total_pay = Math.round((totalHours * hourlyRate) * 100) / 100
        }
      }
    }
  ...
  try {
    ...
    if (existingTimesheet.status === 'active' && !existingTimesheet.clock_out) {
      return NextResponse.json({
        ...
        message: 'Please clock out the staff member first, or use the clock-out endpoint'
      }, { status: 409 })
    }
    ...
    if (payrollEntry && payrollEntry.length > 0) {
      return NextResponse.json({
        ...
        message: 'This timesheet entry is associated with processed payroll and cannot be deleted'
      }, { status: 409 })
    }
