...
export async function PUT(
  ...
  try {
    ...
    const supabase = await createClient()
    const body: UpdateTimeOffRequest & { approved_by?: string } = await request.json()
    ...
    const isBeingApproved = body.status === 'approved' || existingRequest.status === 'approved'
    const datesChanging = body.start_date || body.end_date
    
    if ((isBeingApproved && datesChanging) || (body.status === 'approved' && existingRequest.status !== 'approved')) {
  ...
  try {
    ...
    if (startDate <= today && existingRequest.status === 'approved') {
      return NextResponse.json({
        ...
        message: 'Time off request has already started or passed and cannot be deleted'
      }, { status: 409 })
    }
