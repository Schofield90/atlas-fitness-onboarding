...
export async function POST(request: NextRequest) {
  try {
    ...
    const supabase = await createClient()
    const body: CreateTimeOffRequest & { staff_id: string } = await request.json()
    ...
    if (conflictingRequests && conflictingRequests.length > 0) {
      return NextResponse.json({
        ...
        message: 'There is already an approved time off request that overlaps with these dates',
