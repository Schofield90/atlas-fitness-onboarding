...
export async function POST(request: NextRequest) {
  try {
    ...
    const { leadId, previousScore, newScore, changeReason, triggerAutomations = true } = body
    ...
    if (triggerAutomations) {
      ...
      const automations = await checkScoringAutomations(supabase, userWithOrg.organizationId, lead, previousScore, newScore)
      
      for (const automation of automations) {
        try {
          const result = await executeAutomation(supabase, automation, lead, userWithOrg.organizationId)
...
async function checkScoringAutomations(supabase: any, organizationId: string, lead: any, previousScore: number, newScore: number) {
  try {
...
function shouldTriggerWorkflow(triggerConfig: any, lead: any, previousScore: number, newScore: number): boolean {
...
async function executeAutomation(supabase: any, workflow: any, lead: any, organizationId: string) {
  ...
  for (const action of actions) {
    try {
      ...
      switch (action.type) {
        case 'send_email':
          result = await sendAutomationEmail(supabase, action, lead, organizationId)
          break
        ...
        case 'create_task':
          result = await createAutomationTask(supabase, action, lead, organizationId)
          break
          
        case 'assign_to_user':
          result = await assignLeadToUser(supabase, action, lead.id, organizationId)
          break
          
        case 'add_tag':
          result = await addTagToLead(supabase, action, lead.id, organizationId)
          break
          
        case 'update_status':
          result = await updateLeadStatus(supabase, action, lead.id, organizationId)
          break
...
async function sendAutomationEmail(supabase: any, action: any, lead: any, organizationId: string) {
...
async function createAutomationTask(supabase: any, action: any, lead: any, organizationId: string) {
  const { data, error } = await supabase
    .from('tasks')
    .insert({
      organization_id: organizationId,
      title: action.title || `Follow up with ${lead.name}`,
      description: action.description || `Lead score changed - follow up required`,
      priority: action.priority || 'medium',
      assigned_to: action.assigned_to,
      lead_id: lead.id,
      due_date: action.due_date || new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString()
    })
    .select()
    .single()
...
async function assignLeadToUser(supabase: any, action: any, leadId: string, organizationId: string) {
...
async function addTagToLead(supabase: any, action: any, leadId: string, organizationId: string) {
  const { error } = await supabase
    .from('lead_tags')
    .insert({
      lead_id: leadId,
...
async function updateLeadStatus(supabase: any, action: any, leadId: string, organizationId: string) {
  const { error } = await supabase
    .from('leads')
    .update({
...
async function sendScoringWebhooks(supabase: any, organizationId: string, data: any) {
  try {
