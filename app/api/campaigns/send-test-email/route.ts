  ...
  try {
    ...
    if (!transporter) {
      return NextResponse.json(
        {
          error:
            "No email service configured. Please set up SMTP, Gmail, SendGrid, Mailgun, or Resend in environment variables.",
        },
        { status: 500 },
      );
    }
    ...
    const info = await transporter.sendMail({
      from:
        ...
        "GymLeadHub <sam@gymleadhub.co.uk>",
      to: to,
      subject: subject,
      html: htmlContent,
    });
    ...
    if (error.message?.includes("EAUTH")) {
      return NextResponse.json(
        {
          error:
            "Email authentication failed. Please check your email credentials in environment variables.",
        },
        { status: 500 },
      );
    }
    ...
    if (error.message?.includes("ENOTFOUND")) {
      return NextResponse.json(
        {
          error:
            "Email server not found. Please check your SMTP host settings.",
        },
        { status: 500 },
      );
    }
