  ...
  try {
    ...
    const insightsByType = (insights || []).reduce((acc, insight) => {
      ...
      acc[insight.insight_type].push({
        ...insight,
        age_minutes: Math.round((Date.now() - new Date(insight.created_at).getTime()) / 60000),
        is_expired: insight.expires_at && new Date(insight.expires_at) < new Date()
      })
      return acc
    }, {})

    // Calculate freshness metrics
    const freshInsights = (insights || []).filter(i => 
      !i.expires_at || new Date(i.expires_at) > new Date()
    )
    
    const avgInsightAge = insights?.length > 0 
      ? insights.reduce((sum, i) => sum + (Date.now() - new Date(i.created_at).getTime()), 0) / insights.length / 60000 
      : 0
    ...
    return NextResponse.json({
      ...
      insights: {
        ...
        all: insights || [],
        fresh: freshInsights,
        expired: (insights || []).filter(i => i.expires_at && new Date(i.expires_at) < new Date())
      },
      scoring: {
