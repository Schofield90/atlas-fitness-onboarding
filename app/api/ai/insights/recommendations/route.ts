...
export async function POST(request: NextRequest) {
  try {
    ...
    const { leadId, insightTypes = ['action_recommendations', 'next_steps', 'optimization_tips'] } = body
    ...
    const leadData = await getLeadDataForRecommendations(supabase, leadId, userWithOrg.organizationId)
    ...
    const recommendations = await generateRecommendations(leadData, insightTypes)
  ...
  try {
    ...
    if (leadId) {
      ...
      let query = supabase
        ...
        .in('insight_type', ['action_recommendations', 'next_steps', 'optimization_tips'])
      ...
      const dashboardData = await getRecommendationsDashboard(supabase, userWithOrg.organizationId)
...
async function getLeadDataForRecommendations(supabase: any, leadId: string, organizationId: string) {
  try {
...
async function generateRecommendations(leadData: any, insightTypes: string[]) {
  ...
  const { lead, interactions, activities, existingInsights, scoreHistory } = leadData
...
${interactions.slice(0, 5).map(int => `- ${int.direction}: ${int.type} - ${int.content?.substring(0, 100)}...`).join('\n')}

Recent Activities (${activities.length}):
${activities.slice(0, 10).map(act => `- ${act.activity_type} (value: ${act.activity_value})`).join('\n')}

Score History:
${scoreHistory.slice(0, 5).map(hist => `- ${new Date(hist.created_at).toLocaleDateString()}: ${hist.previous_score} → ${hist.new_score} (${hist.change_reason})`).join('\n')}
        ...
        follow_up_sequence: [
          {
            ...
            channel: 'email',
            content_suggestion: 'Thank you for your interest in our fitness programs'
          }
        ],
        milestone_goals: {
        ...
        score_improvement_tips: [
          {
            ...
            improvement_actions: ['Increase communication frequency', 'Provide valuable content'],
...
async function getRecommendationsDashboard(supabase: any, organizationId: string) {
  try {
