...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: aiStats, error: statsError } = await supabase.rpc(
    ...
    const [
      ...
      supabase.rpc("get_leads_needing_ai_analysis", {
        org_id: userWithOrg.organizationId,
        max_age_hours: 24,
        limit_count: 10,
      }),
    ...
    const backgroundStats = await backgroundProcessor.getJobStatistics(
    ...
    const response = {
      ...
      status: {
        processingJobs: {
          ...
          pending:
            processingJobs.data?.filter((j) => j.status === "pending").length ||
            0,
          processing:
            processingJobs.data?.filter((j) => j.status === "processing")
              .length || 0,
          retrying:
            processingJobs.data?.filter((j) => j.status === "retrying")
              .length || 0,
        ...
        notifications: {
          ...
          urgent:
            urgentNotifications.data?.filter((n) => n.priority === "urgent")
              .length || 0,
          high:
            urgentNotifications.data?.filter((n) => n.priority === "high")
              .length || 0,
          unreadNotifications: includeDetails
            ? urgentNotifications.data
            : undefined,
        },
      ...
      performance: {
        ...
        processingTimes: recentProcessingTimes
          ? {
              ...
              avgMs:
                recentProcessingTimes.reduce(
                  (sum, r) => sum + (r.processing_time_ms || 0),
                  0,
                ) / Math.max(recentProcessingTimes.length, 1),
              minMs: Math.min(
                ...recentProcessingTimes.map((r) => r.processing_time_ms || 0),
              ),
              maxMs: Math.max(
                ...recentProcessingTimes.map((r) => r.processing_time_ms || 0),
              ),
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("AI monitoring action requested:", {
      action,
      organizationId: userWithOrg.organizationId,
    });
    ...
    switch (action) {
      case "cleanup_old_data":
        ...
        const { data: cleanupResult } = await supabase.rpc(
          "cleanup_old_ai_processing_data",
      ...
      case "queue_bulk_analysis":
        ...
        if (leadIds.length === 0) {
          ...
          const { data: leadsToAnalyze } = await supabase.rpc(
            "get_leads_needing_ai_analysis",
            {
              org_id: userWithOrg.organizationId,
              max_age_hours: 24,
          ...
          const leadsForProcessing = (leadsToAnalyze || []).map(
            (lead) => lead.lead_id,
          );
          ...
          if (leadsForProcessing.length > 0) {
            ...
            result = {
              action: "bulk_analysis_queued",
              jobId,
              leadCount: leadsForProcessing.length,
            };
          } else {
      ...
      case "mark_notifications_read":
        ...
        if (notificationError) {
          throw new Error(
            `Failed to mark notifications as read: ${notificationError.message}`,
          );
        }
      ...
      case "schedule_refresh":
        const refreshJobId = await backgroundProcessor.queueScheduledRefresh(
          userWithOrg.organizationId,
        );
...
function calculateHealthMetrics(
  ...
  const errorRate =
    recentErrors.length /
    Math.max(aiStats?.real_time_processing?.total_messages || 1, 1);
  const backgroundSuccessRate = backgroundStats.successRate || 0;

  return {
    overall:
      errorRate < 0.05 && backgroundSuccessRate > 90
        ? "healthy"
        : errorRate < 0.15 && backgroundSuccessRate > 70
          ? "warning"
          : "critical",
    errorRate: Math.round(errorRate * 100 * 100) / 100, // Round to 2 decimal places
    backgroundSuccessRate,
    avgProcessingTime:
      aiStats?.real_time_processing?.avg_processing_time_ms || 0,
    lastUpdated: now.toISOString(),
    indicators: {
      realTimeProcessing: errorRate < 0.1 ? "healthy" : "warning",
      backgroundProcessing:
        backgroundSuccessRate > 80
          ? "healthy"
          : backgroundSuccessRate > 60
            ? "warning"
            : "critical",
      responseTime:
        (aiStats?.real_time_processing?.avg_processing_time_ms || 0) < 2000
          ? "healthy"
          : "warning",
    },
  };
...
function generateRecommendations(
  ...
  aiStats: any,
  processingJobs: any[],
  leadsNeedingAnalysis: any[],
): string[] {
  const recommendations = [];

  if (healthMetrics.overall === "critical") {
    recommendations.push(
      "🚨 System health is critical - investigate processing errors immediately",
    );
  }

  if (healthMetrics.errorRate > 10) {
    recommendations.push(
      "High error rate detected - check AI service configurations and API keys",
    );
  }

  if (processingJobs?.length > 20) {
    recommendations.push(
      "Large processing queue detected - consider increasing background processing capacity",
    );
  }

  if (leadsNeedingAnalysis?.length > 50) {
    recommendations.push(
      "Many leads need AI analysis - schedule bulk processing job",
    );
  }

  if (healthMetrics.avgProcessingTime > 5000) {
    recommendations.push(
      "Processing times are high - optimize AI queries or increase timeout limits",
    );
  }

  if (aiStats?.processing_jobs?.failed > aiStats?.processing_jobs?.completed) {
    recommendations.push(
      "More jobs failing than succeeding - check AI service health and credentials",
    );
  }
