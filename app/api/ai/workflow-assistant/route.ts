...
function parseWorkflowDescription(description: string) {
  ...
  sentences.forEach((sentence, index) => {
    ...
    if (
      !hasTrigger &&
      (index === 0 ||
        triggerKeywords.some((keyword) => sentence.includes(keyword)))
    ) {
      nodeType = "trigger";
      hasTrigger = true;

      if (
        sentence.includes("lead comes in") ||
        sentence.includes("new lead") ||
        ...
        sentence.includes("appointment") ||
        sentence.includes("booking")
      ) {
    ...
    // Detect condition
      ...
      sentence.includes("don't respond") ||
      sentence.includes("no response")
    ) {
      nodeType = "condition";
      label = "If/Else";
...
function generateSuggestions(prompt: string): string[] {
  ...
  if (
    ...
    prompt.toLowerCase().includes("nurtur")
  ) {
    suggestions.push(
      "Consider adding a lead scoring action to prioritize hot leads",
    );
    suggestions.push(
      "Add a condition to check if the lead has opened previous emails",
    );
    suggestions.push(
      "Include a task assignment for sales team follow-up after 3 touchpoints",
    );
  }
  ...
  if (
    ...
    suggestions.push(
      "Add a welcome email with gym introduction and special offer",
    );
    suggestions.push(
      "Schedule a personal training consultation within first week",
    );
    suggestions.push(
      "Send class schedule and beginner-friendly class recommendations",
    );
  }
  ...
  if (
    ...
    prompt.toLowerCase().includes("reminder")
  ) {
  ...
  if (
    prompt.toLowerCase().includes("inactive") ||
    prompt.toLowerCase().includes("re-engage")
  ) {
    suggestions.push(
      "Segment by last activity date for personalized messaging",
    );
    suggestions.push("Offer a special comeback discount or free session");
    suggestions.push(
      "Use multiple channels (email, SMS, WhatsApp) for better reach",
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "AI Assistant - Generated workflow:",
    ...
    return NextResponse.json({
      ...
      suggestions,
      message:
        "Workflow generated successfully. You can now customize each node by clicking on it.",
    });
  } catch (error: any) {
