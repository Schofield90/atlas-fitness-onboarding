...
export async function POST(request: NextRequest) {
  try {
    ...
    if (testLeadId) {
      testResults.push(await testRealTimeProcessing(userWithOrg.organizationId, testLeadId))
    }

    // Test 6: Background Job Processing
    if (testLeadId) {
      testResults.push(await testBackgroundProcessing(userWithOrg.organizationId, testLeadId))
    }
  ...
  try {
    ...
    return {
      ...
      details: {
        ...
        errors: tableChecks.filter(c => c.error).map(c => ({ table: c.table, error: c.error }))
      }
    }
  } catch (error) {
    return {
    ...
    const testInteractions = [
      {
        ...
        direction: 'inbound',
        content: "Hi! I'm interested in joining your gym. What membership options do you have?",
      ...
      },
      {
        ...
        direction: 'outbound',
        content: "Thanks for your interest! We have several great membership options. When would be a good time to discuss them?",
      ...
      },
      {
        ...
        type: 'inbound_message',
        direction: 'inbound',
        content: "That sounds great! I'm really motivated to get in shape for summer. How much would a premium membership cost?",
...
async function testRealTimeProcessing(organizationId: string, leadId: string): Promise<TestResult> {
  const startTime = Date.now()
  ...
  try {
    const result = await realTimeProcessor.processMessage({
      ...
      messageContent: "I need to start working out ASAP! My wedding is in 2 months and I really need to get fit fast. What's the quickest way to join?",
...
async function testBackgroundProcessing(organizationId: string, leadId: string): Promise<TestResult> {
  const startTime = Date.now()
...
async function testPerformanceBenchmarks(leadId: string): Promise<TestResult> {
  ...
  try {
    ...
    benchmarks.push({ test: 'single_lead_processing', timeMs: singleProcessTime })
    ...
    benchmarks.push({ test: 'real_time_processing', timeMs: realTimeProcessTime })
...
async function testErrorHandling(): Promise<TestResult> {
  ...
  try {
    ...
    try {
      ...
      errorTests.push({ test: 'invalid_lead_id', handled: true, error: error.message })
    }
    ...
    try {
      ...
      errorTests.push({ test: 'missing_message_data', handled: true, error: error.message })
    }
...
function generateTestRecommendations(testResults: TestResult[]): string[] {
  ...
  if (failedTests.length === 0) {
    recommendations.push('✅ All tests passed! Your AI system is functioning correctly.')
  ...
  failedTests.forEach(test => {
    switch (test.testName) {
      case 'AI Service Connectivity':
        recommendations.push('🔧 Check AI service configurations (API keys for Claude/OpenAI)')
        break
      ...
      case 'Enhanced Lead Processing':
        recommendations.push('🤖 Debug enhanced lead processing - check AI service integration')
        break
      case 'Real-time Processing':
        recommendations.push('⚡ Fix real-time processing pipeline - check webhook integration')
        break
      case 'Performance Benchmarks':
        recommendations.push('🚀 Optimize AI processing performance - consider caching improvements')
        break
      default:
