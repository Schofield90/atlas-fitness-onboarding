...
export async function POST(request: NextRequest) {
  try {
    ...
    if (processHistorical) {
      ...
      universalProcessor
        .processHistoricalData(organizationId)
        .then(() => {
          console.log(
            `Historical data processing completed for org: ${organizationId}`,
          );
        })
        .catch((error) => {
          console.error(
            `Error processing historical data for org ${organizationId}:`,
            error,
          );
        });
    }
