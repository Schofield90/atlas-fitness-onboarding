...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let i = 0; i < leadsToProcess.length; i += batchSize) {
      ...
      console.log(
        `Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(leadsToProcess.length / batchSize)} (${batch.length} leads)`,
      );

      // Process current batch
      const batchProcessingResults =
        await enhancedLeadProcessor.processBulkLeads(
  ...
  try {
    ...
    const totalProcessed =
      processingStats?.reduce(
        (sum, log) => sum + (log.leads_processed || 0),
        0,
      ) || 0;
    const totalFailed =
      processingStats?.reduce((sum, log) => sum + (log.leads_failed || 0), 0) ||
      0;
    ...
    return NextResponse.json({
      ...
      recommendations: {
        suggestedBatchSize:
          totalProcessed > 0 && avgProcessingTime > 0
            ? Math.min(
                Math.max(Math.floor(60000 / (avgProcessingTime / 10)), 5),
                20,
              ) // Aim for ~1min per batch
            : 10,
        nextProcessingWindow: new Date(
          Date.now() + 60 * 60 * 1000,
        ).toISOString(), // Suggest 1 hour from now
      },
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
