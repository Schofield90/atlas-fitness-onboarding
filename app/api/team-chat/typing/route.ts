...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!channel_id || typeof is_typing !== 'boolean') {
      return NextResponse.json({ error: 'Channel ID and typing status are required' }, { status: 400 });
    }
    ...
    if (is_typing) {
      ...
      if (deleteError) {
        ...
        return NextResponse.json({ error: 'Failed to remove typing indicator' }, { status: 500 });
      }
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ 
      typing_users: typingIndicators?.map(indicator => ({
        user_id: indicator.user_id,
        full_name: (indicator as any).user?.full_name || (indicator as any).user?.email || 'Someone',
...
export async function DELETE() {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to cleanup typing indicators' }, { status: 500 });
    }

    return NextResponse.json({ success: true, message: 'Expired typing indicators cleaned up' });
