...
const createRecurringBookingSchema = z.object({
  ...
  payment_method: z.enum(['per_class', 'monthly', 'package']).default('per_class'),
...
const doesDateMatchPattern = (date: Date, pattern: any, recurrenceType: string, startDate: Date) => {
...
const findMatchingSchedules = async (supabase: any, pattern: any, organizationId: string, classTypeId?: string, instructorId?: string) => {
  ...
  return schedules.filter(schedule => {
    ...
    const scheduleMinutes = scheduleTime.getHours() * 60 + scheduleTime.getMinutes()
  ...
  try {
    ...
    let result = data?.map(rb => ({
      ...
      instructor: rb.instructor ? {
        id: rb.instructor.id,
        name: rb.instructor.raw_user_meta_data?.full_name || rb.instructor.email,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (validated.auto_book) {
      ...
      for (const schedule of matchingSchedules.slice(0, 5)) { // Limit initial bookings
        const scheduleDate = new Date(schedule.start_time)
        
        if (doesDateMatchPattern(scheduleDate, validated.recurrence_pattern, validated.recurrence_type, startDate)) {
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        instructor: recurringBooking.instructor ? {
          ...
          name: recurringBooking.instructor.raw_user_meta_data?.full_name || recurringBooking.instructor.email,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const updateData = Object.fromEntries(
      Object.entries(validated).filter(([key, value]) => key !== 'recurring_booking_id' && value !== undefined)
    )
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        instructor: updatedBooking.instructor ? {
          ...
          name: updatedBooking.instructor.raw_user_meta_data?.full_name || updatedBooking.instructor.email,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const cancelFutureBookings = searchParams.get('cancel_future_bookings') === 'true'
