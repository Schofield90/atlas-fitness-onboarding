...
import { generateSlots, distributeSlots } from '@/packages/core/availability/generateSlots';
...
export async function GET(
  ...
  try {
    ...
    const staffMembers = calendar.calendar_staff?.map((cs: any) => ({
      id: cs.staff.id,
      name: cs.staff.name,
      weight: cs.weight || 1
    })) || [];
    ...
    const response = {
      ...
      availability: finalSlots.map(day => ({
        date: day.date,
        slots: day.slots.filter(s => s.available).map(s => ({
