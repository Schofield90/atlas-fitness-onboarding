...
export async function POST() {
  try {
    ...
    if (intError) {
      ...
      if (
        intError.message.includes("relation") &&
        intError.message.includes("does not exist")
      ) {
        return NextResponse.json(
          {
            error: "Database tables not found",
            solution:
              "Please run the Facebook integration migration in Supabase",
            migration:
              "/supabase/migrations/20250806_facebook_integration_comprehensive.sql",
