  ...
  try {
    ...
    if (meData.error) {
      return NextResponse.json({
        ...
        suggestion: meData.error.code === 190 ? 'Token expired. Please reconnect Facebook.' : 'Token error. Please check permissions.'
      })
    }
    ...
    return NextResponse.json({
      ...
      pages: {
        ...
        list: pagesData.data?.map((p: any) => ({
          id: p.id,
          name: p.name,
          category: p.category
        })) || []
      },
      suggestion: pagesData.data?.length === 0 
