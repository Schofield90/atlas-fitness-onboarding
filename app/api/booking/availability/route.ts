...
export async function GET(request: NextRequest) {
  try {
    ...
    if (bookingLinkConfig?.type === 'team' || bookingLinkConfig?.type === 'round_robin') {
    ...
    const availability = await availabilityEngine.getAvailability(organizationId, options)

    // Get staff information for slots
    const staffIds = [...new Set(availability.map(slot => slot.staff_id).filter(Boolean))]
