  ...
  try {
    ...
    return NextResponse.json({
      ...
      booking: {
        ...
        can_reschedule: booking.booking_status === 'confirmed' && booking.reschedule_count < 3,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (booking.reschedule_count >= 3) {
      return NextResponse.json({
        error: 'Maximum reschedule limit reached. Please contact us directly.'
      }, { status: 400 })
    }
    ...
    try {
      await adminSupabase
        ...
        .insert({
          ...
          body: generateRescheduleConfirmationEmail(updatedBooking, booking.appointment_type),
    ...
    try {
      await adminSupabase
        ...
        .insert({
          ...
          body: generateCancellationConfirmationEmail(cancelledBooking, booking.appointment_type),
          send_at: new Date().toISOString()
        })
    } catch (notificationError) {
...
function generateRescheduleConfirmationEmail(booking: any, appointmentType: any): string {
  return `
...
function generateCancellationConfirmationEmail(booking: any, appointmentType: any): string {
  return `
