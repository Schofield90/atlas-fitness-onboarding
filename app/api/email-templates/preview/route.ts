  ...
  try {
    ...
    if (validated.sampleData) {
      variables = {
        ...
        name: validated.sampleData.name || 
    ...
    const htmlPreview = generateHTMLPreview(processedContent, processedSubject, variables.organizationName);
...
function generateHTMLPreview(content: string, subject: string, organizationName: string): string {
