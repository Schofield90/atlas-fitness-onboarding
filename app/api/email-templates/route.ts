...
export async function POST(request: NextRequest) {
  try {
    ...
    const allVariables = Array.from(new Set([...contentVariables, ...(validated.variables || [])]));
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (validated.content) {
      const contentVariables = extractVariables(validated.content);
      validated.variables = Array.from(new Set([...contentVariables, ...(validated.variables || [])]));
    }
...
function extractVariables(content: string): string[] {
  ...
  const commonVariables = ['name', 'firstName', 'lastName', 'email', 'phone', 'organizationName'];
