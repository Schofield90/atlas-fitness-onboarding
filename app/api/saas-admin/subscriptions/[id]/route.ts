...
export async function GET(
  ...
  try {
    ...
    console.error('Unexpected error in GET /api/saas-admin/subscriptions/[id]:', error)
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    if (body.plan_id && body.plan_id !== existingSubscription.plan_id) {
      ...
      const billingCycle = body.billing_cycle || existingSubscription.billing_cycle
      updateData.amount = billingCycle === 'yearly' ? newPlan.price_yearly : newPlan.price_monthly
    }

    // Handle billing cycle change
    if (body.billing_cycle && body.billing_cycle !== existingSubscription.billing_cycle) {
      ...
      updateData.amount = body.billing_cycle === 'yearly' ? plan.price_yearly : plan.price_monthly
    ...
    console.error('Unexpected error in PATCH /api/saas-admin/subscriptions/[id]:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Unexpected error in DELETE /api/saas-admin/subscriptions/[id]:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
