...
function validateSubscriptionData(data: Partial<CreateSubscriptionRequest>): ValidationError[] {
  const errors: ValidationError[] = []

  if (!data.organization_id) {
    errors.push({ field: 'organization_id', message: 'Organization ID is required', code: 'REQUIRED' })
  }

  if (!data.plan_id) {
    errors.push({ field: 'plan_id', message: 'Plan ID is required', code: 'REQUIRED' })
  }

  if (!data.billing_cycle || !['monthly', 'yearly'].includes(data.billing_cycle)) {
    errors.push({ field: 'billing_cycle', message: 'Billing cycle must be monthly or yearly', code: 'INVALID_VALUE' })
  }

  if (data.trial_days !== undefined && (data.trial_days < 0 || data.trial_days > 365)) {
    errors.push({ field: 'trial_days', message: 'Trial days must be between 0 and 365', code: 'INVALID_VALUE' })
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Unexpected error in GET /api/saas-admin/subscriptions:', error)
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const amount = body.billing_cycle === 'yearly' ? plan.price_yearly : plan.price_monthly
    ...
    console.error('Unexpected error in POST /api/saas-admin/subscriptions:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!action || !Array.isArray(subscription_ids) || subscription_ids.length === 0) {
      return NextResponse.json(
    ...
    console.error('Unexpected error in PATCH /api/saas-admin/subscriptions:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
