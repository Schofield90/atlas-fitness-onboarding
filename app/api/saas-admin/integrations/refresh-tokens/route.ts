...
export async function POST(request: NextRequest) {
  try {
    ...
    if (integrationId) {
      ...
      const result = await refreshIntegrationTokens(integrationId, forceRefresh)
      results.push(result)
    } else {
      // Refresh all OAuth integrations
      const oauthIntegrations = ['google-calendar', 'facebook']
      results = await Promise.all(
        oauthIntegrations.map(id => refreshIntegrationTokens(id, forceRefresh))
      )
    }
...
async function refreshIntegrationTokens(integrationId: string, forceRefresh: boolean): Promise<TokenRefreshResult> {
  const supabase = createClient()
...
async function refreshGoogleTokens(supabase: any, forceRefresh: boolean): Promise<TokenRefreshResult> {
  const errors: string[] = []
  ...
  try {
    ...
    for (const integration of integrations || []) {
      try {
        ...
        const shouldRefresh = forceRefresh || expiresAt <= new Date(now.getTime() + 300000) // 5 minutes buffer
...
async function refreshFacebookTokens(supabase: any, forceRefresh: boolean): Promise<TokenRefreshResult> {
  const errors: string[] = []
  let tokensRefreshed = 0
  let tenantsAffected = 0
  ...
  try {
    ...
    for (const integration of integrations || []) {
      try {
        const now = new Date()
        const expiresAt = new Date(integration.expires_at)
        const shouldRefresh = forceRefresh || expiresAt <= new Date(now.getTime() + 86400000) // 24 hours buffer
        ...
        if (!tokenResponse.ok) {
          throw new Error(`Facebook token refresh failed: ${tokenData.error?.message || 'Unknown error'}`)
        }
