...
async function gatherIntegrationStats(supabase: any): Promise<IntegrationStats[]> {
  const now = new Date()
...
async function getGoogleCalendarStats(
  ...
  try {
    ...
    const expiredTokens = integrations?.filter(i => new Date(i.expires_at) <= now).length || 0
    const expiringSoon = integrations?.filter(i => {
      const expiresAt = new Date(i.expires_at)
      return expiresAt > now && expiresAt <= new Date(now.getTime() + 24 * 60 * 60 * 1000)
    }).length || 0
    ...
    const successRate = Math.max(0, 100 - (errorCount24h / totalRequests) * 100)
...
async function getWhatsAppStats(
  ...
  try {
    ...
    const successRate = Math.max(0, 100 - (errorCount24h / totalMessages) * 100)
...
async function getFacebookStats(
  ...
  try {
    ...
    const tenantCount = integrations?.length || 0
    const expiredTokens = integrations?.filter(i => 
...
async function getWebhookStats(
  ...
  try {
    ...
    const delivered = deliveryLogs?.filter(d => d.status === 'success').length || 0
    const failed = failures?.length || 0
    const pending = failures?.filter(f => !f.resolved && f.retry_count < 3).length || 0
