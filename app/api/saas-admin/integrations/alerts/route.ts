...
interface AlertRule {
  ...
  integration: string
  condition: {
    type: 'error_rate' | 'success_rate' | 'token_expiry' | 'quota_usage' | 'webhook_failures'
    threshold: number
...
async function sendEmailAlert(rule: AlertRule, data: any, isTest: boolean = false): Promise<void> {
  // In production, this would use a proper email service
  console.log(`Sending ${isTest ? 'test ' : ''}email alert for rule: ${rule.name}`)
...
async function sendWebhookAlert(rule: AlertRule, data: any, isTest: boolean = false): Promise<void> {
  // In production, this would call configured webhook endpoints
  console.log(`Sending ${isTest ? 'test ' : ''}webhook alert for rule: ${rule.name}`)
