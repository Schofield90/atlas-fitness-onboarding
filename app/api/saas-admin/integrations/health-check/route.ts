...
async function checkWhatsApp(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const response = await fetch(
      `https://api.twilio.com/2010-04-01/Accounts/${accountSid}/Messages.json?Limit=1`,
      {
        headers: {
...
async function checkWebhooks(): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const successRate =
      (checks.filter((check) => check).length / checks.length) * 100;
