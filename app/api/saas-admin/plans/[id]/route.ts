...
function validatePlanUpdateData(data: Partial<UpdatePlanRequest>): ValidationError[] {
  const errors: ValidationError[] = []

  if (data.name !== undefined && (!data.name || data.name.trim().length === 0)) {
    errors.push({ field: 'name', message: 'Plan name cannot be empty', code: 'REQUIRED' })
  }

  if (data.slug !== undefined) {
    if (!data.slug || data.slug.trim().length === 0) {
      errors.push({ field: 'slug', message: 'Plan slug cannot be empty', code: 'REQUIRED' })
    } else if (!/^[a-z0-9-]+$/.test(data.slug)) {
      errors.push({ field: 'slug', message: 'Slug can only contain lowercase letters, numbers, and hyphens', code: 'INVALID_FORMAT' })
    }
  }

  if (data.price_monthly !== undefined && data.price_monthly < 0) {
    errors.push({ field: 'price_monthly', message: 'Monthly price must be a positive number', code: 'INVALID_VALUE' })
  }

  if (data.price_yearly !== undefined && data.price_yearly < 0) {
    errors.push({ field: 'price_yearly', message: 'Yearly price must be a positive number', code: 'INVALID_VALUE' })
  }
...
export async function PATCH(
  ...
  try {
    ...
    console.error('Unexpected error in PATCH /api/saas-admin/plans/[id]:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
...
export async function DELETE(
  ...
  try {
    ...
    if (subscriptions && subscriptions.length > 0) {
      return NextResponse.json(
        { error: 'Cannot delete plan with existing subscriptions. Deactivate it instead.' },
        { status: 400 }
      )
    }
    ...
    console.error('Unexpected error in DELETE /api/saas-admin/plans/[id]:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
