...
function validatePlanData(data: Partial<CreatePlanRequest>): ValidationError[] {
  ...
  if (!data.name || data.name.trim().length === 0) {
    errors.push({ field: 'name', message: 'Plan name is required', code: 'REQUIRED' })
  }

  if (!data.slug || data.slug.trim().length === 0) {
    errors.push({ field: 'slug', message: 'Plan slug is required', code: 'REQUIRED' })
  ...
  if (!data.tier) {
    errors.push({ field: 'tier', message: 'Plan tier is required', code: 'REQUIRED' })
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const planData = {
      ...
      limits: body.limits || {},
      config: body.config || { trial_days: 14, requires_setup_call: false, priority_support: false, dedicated_success_manager: false },
      is_active: true,
