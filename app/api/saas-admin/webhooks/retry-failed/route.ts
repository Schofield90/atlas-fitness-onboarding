...
async function retryFailedWebhooks(
  ...
  try {
    ...
    const { data: failedWebhooks, error } = await query.order('created_at', { ascending: true })
...
async function attemptWebhookDelivery(webhook: FailedWebhook): Promise<boolean> {
  try {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [failuresResult, deliveriesResult, retryLogsResult] = await Promise.all([
    ...
    const successRate = totalAttempts > 0 ? (deliveries.length / totalAttempts) * 100 : 100

    // Group failures by type
    const failuresByType = failures.reduce((acc: Record<string, number>, failure) => {
      acc[failure.webhook_type] = (acc[failure.webhook_type] || 0) + 1
      return acc
    }, {})

    // Group failures by tenant
    const failuresByTenant = failures.reduce((acc: Record<string, any>, failure) => {
    ...
    return NextResponse.json({
      statistics: {
        ...
        pendingRetries: failures.filter(f => !f.resolved && f.retry_count < 3).length
      },
      failuresByType,
