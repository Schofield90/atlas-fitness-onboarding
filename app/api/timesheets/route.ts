  ...
  try {
    ...
    if (action === 'clock_in') {
      ...
      if (findError || !activeTimesheet) {
        return NextResponse.json({ 
          error: 'No active timesheet found for today. Please clock in first.' 
        }, { status: 400 });
      }
      ...
      const breakMinutes = body.break_duration_minutes || activeTimesheet.break_duration_minutes || 0;
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateData.clock_in_time || updateData.clock_out_time || updateData.break_duration_minutes !== undefined) {
      const clockInTime = new Date(updateData.clock_in_time || existingTimesheet.clock_in_time);
      const clockOutTime = new Date(updateData.clock_out_time || existingTimesheet.clock_out_time);
      
      if (clockOutTime && clockInTime) {
        const totalMinutes = Math.floor((clockOutTime.getTime() - clockInTime.getTime()) / (1000 * 60));
        const breakMinutes = updateData.break_duration_minutes !== undefined 
