...
export async function POST(request: NextRequest) {
  try {
    ...
    if (sanitizedReport.severity === 'critical' || shouldAlertOnError(sanitizedReport)) {
...
function validateErrorReport(report: any): void {
  ...
  if (report.severity && !['low', 'medium', 'high', 'critical'].includes(report.severity)) {
    throw ValidationError.invalid('severity', report.severity, 'one of: low, medium, high, critical')
  }

  if (report.category && !['javascript', 'network', 'ui', 'performance', 'security'].includes(report.category)) {
    throw ValidationError.invalid('category', report.category, 'one of: javascript, network, ui, performance, security')
  }
  ...
  if (isNaN(timestamp.getTime())) {
    throw ValidationError.invalid('timestamp', report.timestamp, 'valid ISO date string')
  }
  ...
  if (now.getTime() - timestamp.getTime() > maxAge) {
    throw ValidationError.invalid('timestamp', report.timestamp, 'timestamp within last 24 hours')
  }
}
...
function sanitizeErrorReport(report: ClientErrorReport): ClientErrorReport {
  const sanitized: ClientErrorReport = {
    ...
    category: report.category || 'javascript'
  }
...
function shouldAlertOnError(report: ClientErrorReport): boolean {
  ...
  if (report.category === 'javascript' && report.severity === 'critical') return true
  
  // Alert on network errors that might indicate service issues
  if (report.category === 'network' && report.message.includes('500')) return true
