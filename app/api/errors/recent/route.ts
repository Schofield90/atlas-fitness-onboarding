...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!validTimeRanges.includes(timeRange)) {
      throw ValidationError.invalid('time_range', timeRange, 'one of: 1h, 24h, 7d, 30d')
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        filters: {
          timeRange,
          level,
          organizationId: userWithOrg.role !== 'owner' ? userWithOrg.organizationId : organizationId,
          errorCode
        },
        metadata: {
          timeRange: {
