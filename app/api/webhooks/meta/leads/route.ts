  ...
  for (const change of entry.changes) {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Failed to process lead immediately, will retry later:",
          processError,
        );
      ...
      console.error(
        `Error processing leadgen webhook for lead ${leadgen_id}:`,
        error,
      );
    }
  }
...
async function processLeadCapture({
  ...
  if (leadData.field_data) {
    for (const field of leadData.field_data) {
      const fieldName =
        field.name?.toLowerCase().replace(/\s+/g, "_") || "unknown";
  ...
  if (fbLeadError && fbLeadError.code !== "23505") {
    // Ignore duplicates
    throw fbLeadError;
  }
  ...
  if (fbLead) {
    try {
      ...
      const firstName =
        processedLeadData.standard_fields?.first_name ??
        processedLeadData.first_name ??
        "";
      const lastName =
        processedLeadData.standard_fields?.last_name ??
        processedLeadData.last_name ??
        "";
      const email =
        processedLeadData.standard_fields?.email ??
        processedLeadData.email ??
        "";
      const phone =
        processedLeadData.standard_fields?.phone ??
        processedLeadData.phone ??
        "";
  ...
  console.log(
    `Successfully processed webhook lead ${leadId} for organization ${organizationId}`,
  );
}
