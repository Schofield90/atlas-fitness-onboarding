...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const webhookSecret = request.headers.get('stripe-connect') === 'true'
...
async function handleSubscriptionCreated(event: Stripe.Event) {
  ...
  await updateOrganizationFeatures(customer.organization_id, plan?.plan_key || 'starter')
...
async function handleSubscriptionUpdated(event: Stripe.Event) {
  ...
  if (sub) {
    await updateOrganizationFeatures(sub.organization_id, plan?.plan_key || 'starter')
  }
}
...
async function updateOrganizationFeatures(organizationId: string, planKey: string) {
  const supabase = await createClient()
  
  const features = {
    free: { max_clients: 10, max_staff: 1, features: ['basic_crm'] },
    starter: { max_clients: 100, max_staff: 3, features: ['crm', 'booking', 'basic_automation'] },
    starter_monthly: { max_clients: 100, max_staff: 3, features: ['crm', 'booking', 'basic_automation'] },
    starter_annual: { max_clients: 100, max_staff: 3, features: ['crm', 'booking', 'basic_automation'] },
    pro: { max_clients: 500, max_staff: 10, features: ['crm', 'booking', 'automation', 'ai_features'] },
    pro_monthly: { max_clients: 500, max_staff: 10, features: ['crm', 'booking', 'automation', 'ai_features'] },
    pro_annual: { max_clients: 500, max_staff: 10, features: ['crm', 'booking', 'automation', 'ai_features'] },
    enterprise: { max_clients: 999999, max_staff: 999999, features: ['crm', 'booking', 'automation', 'ai_features', 'white_label', 'priority_support'] },
    enterprise_monthly: { max_clients: 999999, max_staff: 999999, features: ['crm', 'booking', 'automation', 'ai_features', 'white_label', 'priority_support'] },
    enterprise_annual: { max_clients: 999999, max_staff: 999999, features: ['crm', 'booking', 'automation', 'ai_features', 'white_label', 'priority_support'] }
  }
