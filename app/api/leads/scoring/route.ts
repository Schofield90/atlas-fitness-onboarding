...
export async function GET(request: NextRequest) {
  try {
    ...
    if (leadId) {
      ...
      if (error) {
        return NextResponse.json({ error: 'Failed to fetch scoring dashboard' }, { status: 500 })
      }
      
      // Get organization scoring statistics
      const stats = await getOrganizationScoringStats(supabase, userWithOrg.organizationId)
    ...
    switch (action) {
      case 'recalculate_single':
        ...
        const { data: newScore, error } = await supabase
          .rpc('update_lead_score_with_history', {
            lead_id: leadId,
            triggered_by: 'manual',
      ...
      case 'recalculate_organization':
        ...
        const { data: updatedCount } = await supabase
          .rpc('refresh_organization_lead_scores', {
...
async function getOrganizationScoringStats(supabase: any, organizationId: string) {
  try {
    ...
    if (temperatureStats) {
      ...
      stats.scoringCoverage = Math.round((scoredLeads / stats.totalLeads) * 100)
    }
