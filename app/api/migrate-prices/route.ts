...
export async function POST(request: Request) {
  try {
    ...
    if (checkError) {
      ...
      return NextResponse.json({ error: 'Failed to check plans' }, { status: 500 })
    }
    ...
    for (const plan of plansNeedingMigration) {
      ...
      if (updateError) {
        ...
        migrationCount++
        console.log(`Migrated ${plan.name}: £${plan.price} -> ${pricePennies} pennies`)
      }
    }
    ...
    const verificationResults = verifyData?.map(plan => ({
      id: plan.id,
      name: plan.name,
      price: plan.price,
      price_pennies: plan.price_pennies,
      status: plan.price && plan.price_pennies === (plan.price * 100) 
