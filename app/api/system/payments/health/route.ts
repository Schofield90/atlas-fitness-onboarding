  ...
  try {
    ...
    if (process.env.STRIPE_SECRET_KEY) {
      ...
      try {
        ...
        health.checks.stripe.error = error instanceof Error ? error.message : 'Unknown error'
    ...
    if (process.env.GOCARDLESS_CLIENT_ID && process.env.GOCARDLESS_CLIENT_SECRET) {
      ...
      if (process.env.GOCARDLESS_ENVIRONMENT) {
        ...
        health.checks.gocardless.error = 'GOCARDLESS_ENVIRONMENT not configured'
        health.status = 'degraded'
      }
    } else {
    ...
    try {
      ...
      for (const table of tables) {
        try {
          ...
          if (!error) {
            health.checks.database.tables[table as keyof typeof health.checks.database.tables] = true
          }
        } catch {
    ...
    const criticalChecks = [
      ...
      health.checks.environment.stripe_keys || health.checks.environment.gocardless_keys
    ]
