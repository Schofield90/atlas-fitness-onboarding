...
export async function POST(request: Request) {
  try {
    ...
    if (checkError) {
      ...
      return NextResponse.json({ error: 'Failed to check plans', details: checkError.message }, { status: 500 })
    }

    // Find plans that need migration
    const plansNeedingMigration = checkData?.filter(plan => 
      plan.price !== null && 
    ...
    console.log(`Found ${plansNeedingMigration.length} plans needing migration out of ${checkData?.length || 0} total`)
    ...
    for (const plan of plansNeedingMigration) {
      ...
      console.log(`Migrating ${plan.name} (${plan.organization_id}): £${plan.price} -> ${pricePennies} pennies`)
    ...
    const finalStatus = verifyData?.map(plan => ({
      id: plan.id,
      name: plan.name,
      price: plan.price,
      price_pennies: plan.price_pennies,
      status: plan.price_pennies && plan.price_pennies > 0 ? 'OK' : 'NEEDS_ATTENTION'
    })) || []
...
export async function GET(request: Request) {
  try {
    ...
    const status = plans?.map(plan => ({
      ...
      price: plan.price,
      price_pennies: plan.price_pennies,
      needs_migration: plan.price !== null && plan.price > 0 && 
                       (plan.price_pennies === null || plan.price_pennies === 0)
    })) || []

    const summary = {
      total_plans: status.length,
      plans_with_price: status.filter(p => p.price !== null && p.price > 0).length,
      plans_with_price_pennies: status.filter(p => p.price_pennies !== null && p.price_pennies > 0).length,
