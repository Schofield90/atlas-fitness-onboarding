...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, description, type, subject, content, variables, is_active, category } = body;
    ...
    // Remove duplicates
    const allVariables = Array.from(new Set([...extractedVariables, ...(variables || [])]));
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updateData.content) {
      const extractedVariables = extractVariablesFromContent(updateData.content);
      if (updateData.subject) {
        extractedVariables.push(...extractVariablesFromContent(updateData.subject));
      }
      updateData.variables = Array.from(new Set([...extractedVariables, ...(updateData.variables || [])]));
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const previewSubject = template.subject ? renderTemplate(template.subject, mergedData) : undefined;
