  ...
  try {
    ...
    if (isComplete) {
      ...
      const extractionMessages: OpenAI.Chat.ChatCompletionMessageParam[] = [
        ...
        {
          role: 'user',
          content: `Extract profile data from this conversation: ${JSON.stringify(conversationHistory.concat([
            { role: 'user', content: message },
      ...
      try {
        ...
        extractedData = {
          ...
          current_weight: extractedJson.current_weight || extractedJson.currentWeight,
          goal_weight: extractedJson.goal_weight || extractedJson.goalWeight,
          activity_level: extractedJson.activity_level || extractedJson.activityLevel,
          training_frequency: extractedJson.training_frequency || extractedJson.trainingFrequency,
          training_types: extractedJson.training_types || extractedJson.trainingTypes || [],
          dietary_preferences: extractedJson.dietary_preferences || extractedJson.dietaryPreferences || [],
          allergies: extractedJson.allergies || [],
          food_likes: extractedJson.food_likes || extractedJson.foodLikes || [],
          food_dislikes: extractedJson.food_dislikes || extractedJson.foodDislikes || [],
          cooking_time: extractedJson.cooking_time || extractedJson.cookingTime || 'MODERATE',
          budget_constraint: extractedJson.budget_constraint || extractedJson.budgetConstraint || 'MODERATE'
        }
      } catch (parseError) {
    ...
    const cleanResponse = assistantResponse.replace('PROFILE_COMPLETE', '').trim()
