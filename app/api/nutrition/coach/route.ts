...
export async function POST(request: Request) {
  try {
    ...
    const { message, profile, memberData, todaysMacros, conversationHistory } =
      await request.json();
...
`;
    ...
    const isAskingForMealPlan = /meal plan|what.*eat|menu|recipes/i.test(
      message,
    );
    const isAskingForMacros = /macros|protein|carbs|fats|calories/i.test(
      message,
    );
    ...
    if (isAskingForMealPlan) {
      ...
      aiResponse =
        ...
        "I can help you create a meal plan. Could you provide more details about your preferences?";
      ...
      try {
        ...
        aiResponse =
          completion.choices[0].message.content ||
          "I understand. Let me help you with your nutrition goals.";
      }
    }
...
function extractMealsFromResponse(response: string): any {
  ...
  for (const line of lines) {
    if (/breakfast/i.test(line)) currentMeal = "breakfast";
      ...
      meals[currentMeal as keyof typeof meals].push(
        line.replace(/[•\-]/g, "").trim(),
      );
    }
  }
      ...
      messages: [
        {
          role: "system",
          content:
            "Extract food items and estimate their macros from the user message. Return JSON format: { foods: [{ name, calories, protein, carbs, fats }] }",
        },
        {
          role: "user",
          content: message,
...
function generateSmartSuggestions(profile: any, todaysMacros: any): string[] {
  ...
  if (!profile) {
    suggestions.push(
      "Set up your nutrition profile for personalized recommendations",
    );
    return suggestions;
  }

  const remainingCalories =
    (profile.targetCalories || 2000) - (todaysMacros?.calories || 0);
  const remainingProtein =
    (profile.targetMacros?.protein || 150) - (todaysMacros?.protein || 0);
  ...
  if (remainingProtein > 30) {
    suggestions.push(
      `Consider a high-protein snack (${remainingProtein}g protein remaining)`,
    );
  }
  ...
  if (hour < 10 && !todaysMacros?.calories) {
    ...
    suggestions.push(
      "Consider a light evening snack to meet your calorie goals",
    );
  }
