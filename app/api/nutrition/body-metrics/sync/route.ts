  ...
  try {
    ...
    if (latestMetric) {
      ...
      if (profile) {
        ...
        const actualTrainingFrequency = trainingSessions ? Math.round(trainingSessions.length / 4) : profile.training_frequency

        // Calculate TDEE with actual data
        let activityMultiplier = 1.2 // Sedentary base
        
        if (actualTrainingFrequency >= 6) activityMultiplier = 1.725 // Very Active
        else if (actualTrainingFrequency >= 4) activityMultiplier = 1.55 // Moderately Active
        ...
        const leanMass = latestMetric.lean_body_mass || 
          (latestMetric.weight * (1 - (latestMetric.body_fat_percentage || 20) / 100))
...
export async function GET(request: NextRequest) {
  try {
    ...
    const nextScanDate = latestMetric 
      ? new Date(new Date(latestMetric.measurement_date).getTime() + 30 * 24 * 60 * 60 * 1000) // 30 days later
      : null
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        daysSinceLastScan: latestMetric 
          ? Math.floor((today.getTime() - new Date(latestMetric.measurement_date).getTime()) / (1000 * 60 * 60 * 24))
          : null
      }
    })
