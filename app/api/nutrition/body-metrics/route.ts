...
export async function GET(request: NextRequest) {
  try {
    ...
    if (metrics && metrics.length > 1 && !latest) {
      ...
      progress = {
        weight_change: latestMetric.weight && oldestMetric.weight 
          ? (latestMetric.weight - oldestMetric.weight).toFixed(1)
          : null,
        body_fat_change: latestMetric.body_fat_percentage && oldestMetric.body_fat_percentage
          ? (latestMetric.body_fat_percentage - oldestMetric.body_fat_percentage).toFixed(1)
          : null,
        muscle_mass_change: latestMetric.muscle_mass && oldestMetric.muscle_mass
          ? (latestMetric.muscle_mass - oldestMetric.muscle_mass).toFixed(1)
          : null,
        days_tracked: Math.ceil(
          (new Date(latestMetric.date).getTime() - new Date(oldestMetric.date).getTime()) 
          / (1000 * 60 * 60 * 24)
        )
      }
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: existingMetric ? 'Body metrics updated successfully' : 'Body metrics recorded successfully',
      data: result
