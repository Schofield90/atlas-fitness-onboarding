...
export async function POST(
  ...
  try {
    ...
    const newMealData = await regenerateMeal(existingMeal, profile, constraints)
...
async function regenerateMeal(existingMeal: any, profile: any, additionalConstraints?: string) {
  ...
  try {
    const completion = await getOpenAI().chat.completions.create({
      ...
      messages: [
        ...
        { role: 'user', content: 'Generate the new meal now. Make sure it\'s different from the current meal.' }
      ],
      temperature: 0.8,
      max_tokens: 1000,
    ...
    const newMealData = JSON.parse(completion.choices[0]?.message?.content || '{}')
    return newMealData
  } catch (error) {
