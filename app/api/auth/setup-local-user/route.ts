...
export async function GET() {
  try {
    ...
    const url = new URL('/dashboard', process.env.NEXT_PUBLIC_URL || 'http://localhost:3000')
    url.searchParams.set('setup', 'complete')
    url.searchParams.set('data', Buffer.from(JSON.stringify(trialData)).toString('base64'))
