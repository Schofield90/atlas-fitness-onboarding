...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!adAccountRecord) {
      return NextResponse.json({ error: 'Ad account not found or inactive' }, { status: 404 })
    }
    ...
    for (const campaign of campaigns) {
      try {
        ...
        const leadActions = actions.find((action: any) => action.action_type === 'lead')
        const linkClickActions = actions.find((action: any) => action.action_type === 'link_click')
        ...
        campaignInsights.push({
          ...
          insights: {
            ...
            cost_per_lead: leadActions ? 
              (parseFloat(insightData.spend || '0') / parseInt(leadActions.value || '1')).toFixed(2) : 
              '0.00',
            video_plays: insightData.video_play_actions ? 
              insightData.video_play_actions.reduce((sum: number, action: any) => sum + parseInt(action.value || '0'), 0) : 0
          }
        })
    ...
    const summary = campaignInsights.reduce((acc, campaign) => {
    ...
    summary.averageCPM = summary.totalImpressions > 0 ? 
      ((summary.totalSpend / summary.totalImpressions) * 1000).toFixed(2) : '0.00'
    summary.averageCPC = summary.totalClicks > 0 ? 
      (summary.totalSpend / summary.totalClicks).toFixed(2) : '0.00'
    summary.averageCTR = summary.totalImpressions > 0 ? 
      ((summary.totalClicks / summary.totalImpressions) * 100).toFixed(2) : '0.00'
    summary.costPerLead = summary.totalLeads > 0 ? 
      (summary.totalSpend / summary.totalLeads).toFixed(2) : '0.00'
