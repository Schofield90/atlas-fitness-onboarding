...
export async function POST(request: NextRequest) {
  try {
    ...
    if (intError || !integration) {
      ...
      return NextResponse.json({ 
        error: 'No Facebook integration found. Please reconnect your Facebook account.' 
      }, { status: 400 })
    }

    if (!integration.access_token) {
      return NextResponse.json({ 
        error: 'No access token found. Please reconnect your Facebook account.' 
      }, { status: 400 })
    }
    ...
    if (pages.length === 0) {
      return NextResponse.json({ 
        message: 'No pages found. Make sure you have admin access to at least one Facebook page.',
        pages: []
      })
    }
