...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!forceRefresh) {
      ...
      if (lastSync?.last_sync_at) {
        ...
        if (diffMinutes < 5) {
          ...
          return NextResponse.json({
            message:
              "Forms were recently synced. Use forceRefresh=true to override.",
    ...
    const leadForms = await metaClient.getPageLeadForms(
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: forms, error } = await query.order("created_at", {
      ascending: false,
    });
