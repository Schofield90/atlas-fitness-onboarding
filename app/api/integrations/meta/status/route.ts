...
export async function GET(request: NextRequest) {
  try {
    ...
    const isTokenExpired = integration.token_expires_at ? 
      new Date(integration.token_expires_at) < new Date() : false
    ...
    const pagesCount = pagesResult.status === 'fulfilled' ? (pagesResult.value.count || 0) : 0
    const formsCount = formsResult.status === 'fulfilled' ? (formsResult.value.count || 0) : 0
    const accountsCount = accountsResult.status === 'fulfilled' ? (accountsResult.value.count || 0) : 0
