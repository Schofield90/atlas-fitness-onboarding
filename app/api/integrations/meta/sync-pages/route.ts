...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          ...
          details:
            "Your user account is not associated with an organization. Please visit /fix-organization to resolve this.",
          user_id: user.id,
        },
        { status: 400 },
      );
    ...
    if (!pages || pages.length === 0) {
      console.log(
        "⚠️ No pages found - user may not have page admin permissions",
      );
      return NextResponse.json({
