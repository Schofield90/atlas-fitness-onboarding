  ...
  try {
    ...
    if (config.selectedForms && config.selectedForms.length > 0) {
      ...
      const formsToSave = config.selectedForms.map((formId: string) => {
        ...
        const formDetail = config.selectedFormDetails?.find(
          (f: any) => f.id === formId,
        );
        return {
      ...
      if (formsToSave.length > 0) {
        console.log(
          "Attempting to save forms:",
          JSON.stringify(formsToSave, null, 2),
        );
        ...
        if (upsertError) {
          console.error(
            "Error upserting forms - Full error:",
          ...
          for (const form of formsToSave) {
            ...
            if (insertError && insertError.code === "23505") {
              ...
              if (updateError) {
                console.error(
                  `Failed to update form ${form.facebook_form_id}:`,
                  updateError,
                );
              } else {
              ...
              console.error(
                `Failed to insert form ${form.facebook_form_id}:`,
                insertError,
              );
            } else {
          ...
          console.log(
            `Successfully saved ${successCount} out of ${formsToSave.length} forms`,
          );
        } else {
          console.log(
            "Successfully upserted forms:",
            upsertedForms?.length || 0,
          );
        }
      }
    }
    ...
    if (config.selectedForms.length > 0) {
      ...
      console.log(
        `📋 Queuing sync for ${config.selectedForms.length} lead forms`,
      );
    }
