  ...
  try {
    ...
    if (integration?.token_expires_at) {
      ...
      diagnostics.checks.token = {
        ...
        expiresIn: Math.floor((expiryDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)) + ' days'
      }
    }
    ...
    diagnostics.overallStatus = {
      isConnected: integration?.is_active && !diagnostics.checks.token?.isExpired,
      canConnect: diagnostics.checks.auth.success && diagnostics.checks.organization.success,
