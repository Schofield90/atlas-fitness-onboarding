  ...
  try {
    ...
    if (intError || !integration) {
      return NextResponse.json({ 
        error: 'No active Facebook connection. Please connect Facebook first.' 
      }, { status: 404 })
    }
    ...
    if (!pagesData.data || pagesData.data.length === 0) {
      return NextResponse.json({ 
        error: 'No Facebook pages found. Please ensure you have admin access to at least one page.' 
      }, { status: 400 })
    }
    ...
    const pageAccessToken = primaryPage.page_access_token || primaryPage.access_token
    if (!pageAccessToken) {
      return NextResponse.json({ 
    ...
    const formsUrl = `https://graph.facebook.com/v18.0/${primaryPage.facebook_page_id}/leadgen_forms?` +
    ...
    for (const form of forms) {
      ...
      const formDetailsUrl = `https://graph.facebook.com/v18.0/${form.id}?` +
      ...
      if (!saveError && savedForm) {
        ...
        if (!savedForm.field_mappings && formDetails.questions) {
          ...
          if (mappings.length > 0) {
            ...
            console.log(`  ✅ Auto-configured ${mappings.length} field mappings`)
          }
        }
      }
    }
