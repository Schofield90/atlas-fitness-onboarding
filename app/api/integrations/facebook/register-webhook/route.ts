...
export async function POST(request: NextRequest) {
  try {
    ...
    if (integrationError || !integration) {
      ...
      return NextResponse.json(
        { error: "Facebook not connected. Please reconnect your account." },
        { status: 401 },
      );
    }
    ...
    console.log(
      `🔗 Registering webhook for page: ${pageId} (${page.page_name})`,
    );
    ...
    if (!fbAppId || !appUrl) {
      return NextResponse.json(
        { error: "Facebook app ID or site URL not configured" },
        { status: 500 },
      );
    }
    const finalWebhookUrl = webhookUrl || `${appUrl}/api/webhooks/meta/leads`;
    const verifyToken =
    ...
    try {
      ...
      if (!subscribeResponse.ok && subscribeData.error) {
        throw new Error(
          subscribeData.error.message || "Failed to subscribe page",
        );
      }
      ...
      if (updateError) {
        console.error(
          "Failed to update webhook status in database:",
          updateError,
        );
      }
      ...
      return NextResponse.json({
        ...
        message:
          "Real-time sync enabled successfully! New leads will now appear instantly.",
        subscribed: true,
      });
    } catch (fbError) {
      ...
      return NextResponse.json(
        {
          error:
            "Failed to enable real-time sync. Please check your Facebook permissions.",
