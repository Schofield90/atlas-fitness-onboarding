...
export async function POST(request: NextRequest) {
  try {
    ...
    const detectedMappings = await mappingService.autoDetectFieldMappings({
      questions: questions
    })
    
    // Get any existing saved mappings
    const savedMappings = await mappingService.getFieldMappings(formId, organizationId)
    ...
    if (savedMappings) {
      ...
      finalMappings = mappingService.mergeMappings(detectedMappings, savedMappings.mappings)
    }
    ...
    const suggestedConfig = {
      ...
      default_lead_source: savedMappings?.default_lead_source || 'Facebook Lead Form'
    }
    
    // Validate the suggested configuration
    const validation = mappingService.validateMappings(suggestedConfig)
    
    console.log(`🔍 Auto-detected ${finalMappings.length} field mappings for form ${formId}`)
