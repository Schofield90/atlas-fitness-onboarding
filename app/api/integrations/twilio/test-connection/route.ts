...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!accountSid.startsWith('AC') || accountSid.length !== 34) {
      return NextResponse.json({
        success: false,
        error: 'Invalid Account SID format. Should start with "AC" and be 34 characters long.'
      }, { status: 400 })
    }
    ...
    try {
      ...
      return NextResponse.json({
        success: false,
        error: 'Failed to create Twilio client. Please check your credentials.'
      }, { status: 400 })
    }
      ...
      try {
        ...
        availableNumbers = numbers
          ...
          .map(num => ({
            ...
            capabilities: Object.keys(num.capabilities).filter(cap => num.capabilities[cap]),
    ...
    const isTrialAccount = accountInfo.status === 'active' && accountInfo.type === 'Trial'
    ...
    if (isTrialAccount) {
      responseData.warnings.push('Your account is in trial mode. Some features may be limited.')
      responseData.recommendations.push('Consider upgrading your account to remove trial limitations.')
    }

    if (balance && parseFloat(balance.balance) < 5) {
      responseData.warnings.push('Your account balance is low. Add credits to ensure uninterrupted service.')
      responseData.recommendations.push('Add at least $10 in credits for reliable SMS service.')
    }

    if (!phoneNumber && availableNumbers.length === 0) {
      responseData.recommendations.push('Purchase a phone number with SMS capabilities to start sending messages.')
    }
    ...
    return NextResponse.json({
      success: false,
      error: error.message || 'An unexpected error occurred while testing the connection.'
    }, { status: 500 })
  }
}
