...
export async function POST(request: NextRequest) {
  try {
    ...
    await supabase.from("integration_validations").insert({
...
async function validateFacebookIntegration(
  config: any,
  testAction?: string,
): Promise<ValidationResult> {
  try {
...
async function validateGoogleIntegration(
  config: any,
  testAction?: string,
): Promise<ValidationResult> {
  try {
...
async function validateTwilioIntegration(
  config: any,
  testAction?: string,
): Promise<ValidationResult> {
  try {
...
async function validateWhatsAppIntegration(
  config: any,
  testAction?: string,
): Promise<ValidationResult> {
  try {
...
async function validateEmailIntegration(
  ...
  try {
    const { provider, smtp_host, smtp_port, username, password, from_email } =
      config;
...
async function validateWebhookIntegration(
  ...
  try {
    ...
    if (!secret_key) {
      warnings.push(
        "No secret key configured - webhooks will not be authenticated",
      );
    }
...
async function testFacebookLeadFetch(
  accessToken: string,
  pageId?: string,
): Promise<any> {
