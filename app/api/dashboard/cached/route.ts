  ...
  try {
    ...
    switch (type) {
      case 'metrics':
        ...
        responseData = await cachedAnalyticsService.getDashboardMetrics(userWithOrg.organizationId)
        break
        
      case 'realtime':
        // Get real-time metrics with shorter cache
        responseData = await cachedAnalyticsService.getRealTimeDashboardMetrics(userWithOrg.organizationId)
        break
        
      case 'full':
      default:
        // Get full dashboard data
        responseData = await cachedAnalyticsService.getFullDashboard(userWithOrg.organizationId)
        break
    }
    ...
    logger.info(`Dashboard API (cached) - Org: ${userWithOrg.organizationId}, Type: ${type}, Response time: ${responseTime}ms`, {
  ...
  try {
    ...
    switch (action) {
      case 'refresh-metrics':
        ...
        await cachedAnalyticsService.refreshDailyMetrics(userWithOrg.organizationId, targetDate)
      ...
      case 'warm-cache':
        ...
        await cachedAnalyticsService.warmAnalyticsCaches(userWithOrg.organizationId)
      ...
      case 'schedule-refresh':
        ...
        await cachedAnalyticsService.scheduleAnalyticsRefresh(userWithOrg.organizationId, intervalMinutes)
      ...
      default:
        return NextResponse.json({
          ...
          availableActions: ['refresh-metrics', 'clear-cache', 'warm-cache', 'schedule-refresh']
        }, { status: 400 })
    }
    
    const responseTime = Date.now() - startTime
    
    logger.info(`Dashboard action completed - Org: ${userWithOrg.organizationId}, Action: ${action}, Response time: ${responseTime}ms`)
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    logger.info(`Dashboard settings updated - Org: ${userWithOrg.organizationId}, Response time: ${responseTime}ms`)
