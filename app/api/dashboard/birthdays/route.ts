  ...
  try {
    ...
    const upcomingAnniversaries = (memberships || [])
      .map(membership => {
        ...
        const yearsAsMember = today.getFullYear() - startDate.getFullYear() + (anniversaryDate.getFullYear() - today.getFullYear());
      ...
      .filter(a => a && a.daysUntil >= 0 && a.daysUntil <= 30 && a.yearsAsMember > 0)
      .sort((a, b) => a.daysUntil - b.daysUntil)
      .slice(0, 5);
