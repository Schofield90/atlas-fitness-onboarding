...
export async function GET(request: NextRequest) {
  ...
  return handleApiRoute(request, async (req) => {
    ...
    const startDate = include_past
      ? new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000)
      : today; // 7 days ago if including past
    ...
    const contactsWithUpcomingBirthdays = birthdayReminders
      ?.map((reminder) => {
        const contact = Array.isArray(reminder.contacts)
          ? reminder.contacts[0]
          : reminder.contacts;
      ...
      .filter((contact): contact is NonNullable<typeof contact> => {
        ...
        if (include_past) {
          return (
            contact.days_until_birthday >= -7 &&
            contact.days_until_birthday <= days_ahead
          );
        } else {
          return (
