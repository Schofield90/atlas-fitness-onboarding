...
async function fetchAndParseWebpage(url: string) {
  try {
    ...
    // Extract headers
    $('header, nav, .header, .navbar, [class*="header"], [class*="nav"]').each((i, el) => {
      const $el = $(el)
      pageStructure.headers.push({
        html: $el.html()?.substring(0, 500),
        text: $el.text().trim().substring(0, 200),
        links: $el.find('a').map((i, a) => $(a).text().trim()).get()
      })
    })
    
    // Extract hero sections
    $('[class*="hero"], [class*="banner"], .jumbotron, section:first-child').each((i, el) => {
      const $el = $(el)
      pageStructure.sections.push({
        type: 'hero',
        heading: $el.find('h1, h2').first().text().trim(),
        subheading: $el.find('p').first().text().trim(),
        buttons: $el.find('a.btn, button, [class*="button"]').map((i, btn) => $(btn).text().trim()).get(),
        backgroundImage: $el.css('background-image') || $el.find('img').first().attr('src')
      })
    })
    
    // Extract features sections
    $('[class*="features"], [class*="services"], .features, .services').each((i, el) => {
      const $el = $(el)
      const features = $el.find('[class*="feature"], [class*="service"], .col, .card').map((i, feat) => {
        const $feat = $(feat)
        return {
    ...
    $('[class*="cta"], [class*="call-to-action"], .cta').each((i, el) => {
      ...
      pageStructure.sections.push({
        ...
        buttons: $el.find('a, button').map((i, btn) => $(btn).text().trim()).get()
      })
    })
    ...
    $('form').each((i, el) => {
      const $el = $(el)
      const fields = $el.find('input, textarea, select').map((i, field) => {
        const $field = $(field)
        return {
          type: $field.prop('type') || $field.prop('tagName').toLowerCase(),
          name: $field.attr('name'),
          placeholder: $field.attr('placeholder'),
          label: $(`label[for="${$field.attr('id')}"]`).text().trim()
        }
      }).get()
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an expert at analyzing web pages and creating landing page templates. Always return valid JSON.'
        },
        {
          role: 'user',
          content: prompt
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const generatedTemplate = await analyzeAndGenerateComponents(pageStructure, url)
    ...
    const { data: landingPage, error: pageError } = await supabase
      ...
      .insert({
        organization_id: organizationId,
        name: generatedTemplate.name || `Generated from ${new URL(url).hostname}`,
        slug: `generated-${Date.now()}`,
        title: generatedTemplate.meta?.title || generatedTemplate.name,
        description: generatedTemplate.meta?.description || generatedTemplate.description,
