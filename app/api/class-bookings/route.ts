...
const createBookingSchema = z.object({
  ...
  booking_type: z.enum(['single', 'recurring', 'package', 'drop_in']).default('single'),
...
// Helper functions
const canBookClass = async (supabase: any, scheduleId: string, clientId: string) => {
  ...
  if (hoursUntilClass < cutoffHours) {
    return { canBook: false, error: `Booking closes ${cutoffHours} hours before class` }
  }

  return { canBook: true, schedule }
}

const processPackagePayment = async (supabase: any, packageId: string, organizationId: string, clientId: string) => {
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (type === 'upcoming') {
      ...
      query = query
        .or(`status.neq.confirmed,class_schedule.start_time.lt.${new Date().toISOString()}`)
    }
    ...
    if (body.bookings && Array.isArray(body.bookings)) {
      ...
      for (const bookingData of validated.bookings) {
        try {
          ...
          const { canBook, error: canBookError, schedule } = await canBookClass(
            supabase,
          ...
          if (bookingData.payment_method_id.startsWith('package_')) {
            const packageId = bookingData.payment_method_id.replace('package_', '')
            await processPackagePayment(supabase, packageId, schedule.organization_id, validated.client_id)
            bookingType = 'package'
            metadata = { package_id: packageId }
          } else if (bookingData.payment_method_id === 'card' && schedule.price_pennies > 0) {
      ...
      if (validated.use_package_id) {
        await processPackagePayment(supabase, validated.use_package_id, schedule.organization_id, validated.client_id)
        bookingType = 'package'
        metadata = { ...metadata, package_id: validated.use_package_id }
      } else if (validated.payment_method_id === 'card' && schedule.price_pennies > 0) {
        paymentStatus = 'pending'
        paymentAmount = schedule.price_pennies
      }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (hoursUntilClass < cutoffHours) {
      return NextResponse.json(
        { success: false, error: `Cannot cancel less than ${cutoffHours} hours before class` },
        { status: 400 }
      )
    }
