...
const processWaitlistForSchedule = async (
  supabase: any,
  scheduleId: string,
) => {
  ...
  try {
    ...
    if (existingWaitlist) {
      return NextResponse.json(
        {
          success: false,
          error: "Client is already on waitlist for this class",
        },
        { status: 400 },
      );
    }
  ...
  try {
    ...
    const updateData = Object.fromEntries(
      Object.entries(validated).filter(
        ([key, value]) => key !== "waitlist_id" && value !== undefined,
      ),
    );
    ...
    if (
      validated.priority_score !== undefined &&
      validated.priority_score !== currentEntry.priority_score
    ) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const processed = await processWaitlistForSchedule(
      supabase,
      entry.schedule_id,
    );
