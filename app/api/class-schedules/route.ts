...
const getSchedulesSchema = z.object({
  ...
  available_only: z.string().optional().transform(val => val === 'true'),
...
const createScheduleSchema = z.object({
  ...
  class_level: z.enum(['beginner', 'intermediate', 'advanced', 'all']).default('all'),
...
const updateScheduleSchema = z.object({
  ...
  room_location: z.string().optional(),
  equipment_needed: z.array(z.string()).optional(),
  class_level: z.enum(['beginner', 'intermediate', 'advanced', 'all']).optional(),
...
const checkInstructorConflicts = async (supabase: any, instructorId: string, startTime: string, endTime: string, excludeScheduleId?: string) => {
  let query = supabase
...
export async function GET(request: NextRequest) {
  try {
    ...
    const params = getSchedulesSchema.parse(Object.fromEntries(searchParams.entries()))
    ...
    const formattedData = data?.map(schedule => ({
      ...
      instructor: schedule.instructor ? {
        ...
        name: schedule.instructor.raw_user_meta_data?.full_name || schedule.instructor.email,
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (validated.start_time && new Date(validated.start_time).getTime() !== new Date(currentSchedule.start_time).getTime()) {
      ...
      if (bookings && bookings.length > 0) {
        ...
        const timeDiffMinutes = Math.abs(newTime.getTime() - currentTime.getTime()) / (1000 * 60)
        
        if (timeDiffMinutes > 30) {
          return NextResponse.json(
            { success: false, error: 'Cannot change class time by more than 30 minutes when bookings exist' },
            { status: 400 }
          )
        }
      }
    ...
    if (validated.instructor_id || validated.start_time || validated.end_time) {
      const instructorId = validated.instructor_id || currentSchedule.instructor_id
    ...
    const updateData = Object.fromEntries(
      Object.entries(validated).filter(([key, value]) => key !== 'schedule_id' && value !== undefined)
    )
