...
export async function POST(request: Request) {
  try {
    ...
    const messages = await fetchConversationMessages(conversation_id, time_period)
...
async function fetchConversationMessages(conversationId: string, timePeriod?: string) {
...
async function generateConversationSummary(
  ...
  for (let i = 1; i < messages.length; i++) {
    ...
    if (prevMsg.direction === 'inbound' && currentMsg.direction === 'outbound') {
      const responseTime = (new Date(currentMsg.timestamp).getTime() - new Date(prevMsg.timestamp).getTime()) / (1000 * 60)
  ...
  const positiveWords = ['great', 'thanks', 'good', 'awesome', 'perfect', 'yes', 'interested', 'love', 'amazing']
  const negativeWords = ['no', 'cancel', 'problem', 'issue', 'complaint', 'disappointed', 'bad', 'terrible']
  ...
  if (allContent.includes('member') || allContent.includes('plan')) keyTopics.push('Membership')
  if (allContent.includes('class') || allContent.includes('session') || allContent.includes('book')) keyTopics.push('Class Booking')
  if (allContent.includes('trial') || allContent.includes('tour')) keyTopics.push('Trial/Tour')
  if (allContent.includes('price') || allContent.includes('cost') || allContent.includes('fee')) keyTopics.push('Pricing')
  if (allContent.includes('personal') || allContent.includes('trainer')) keyTopics.push('Personal Training')
  if (allContent.includes('hour') || allContent.includes('time') || allContent.includes('schedule')) keyTopics.push('Schedule/Hours')

  // Determine outcomes based on conversation flow
  const outcomes: string[] = []
  const nextSteps: string[] = []
  
  if (allContent.includes('trial') && messages.some(m => m.direction === 'outbound' && m.content.includes('book'))) {
  ...
  for (const [date, dayMessages] of Object.entries(messagesByDate)) {
    ...
    const daySentiment = analyzeSentiment(dayMessages.map(m => m.content).join(' '))
  ...
  const customerSatisfaction = overallSentiment === 'positive' ? 85 : 
                              overallSentiment === 'negative' ? 35 : 60

  // Calculate resolution time (time from first message to last)
  const resolutionTime = messages.length > 1 
    ? (new Date(messages[messages.length - 1].timestamp).getTime() - new Date(messages[0].timestamp).getTime()) / (1000 * 60)
    : 0
...
function extractTopicsFromMessages(messages: any[]): string[] {
  ...
  if (content.includes('book') || content.includes('class')) topics.push('Booking')
  if (content.includes('member') || content.includes('plan')) topics.push('Membership')
...
function analyzeSentiment(content: string): 'positive' | 'neutral' | 'negative' {
  ...
  const positiveWords = ['great', 'thanks', 'good', 'awesome', 'perfect', 'yes']
...
function determineResolutionStatus(messages: any[]): 'resolved' | 'pending' | 'escalated' {
