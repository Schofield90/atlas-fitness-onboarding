...
async function generateAISuggestions(
  ...
  const positiveWords = ['great', 'thanks', 'good', 'awesome', 'perfect', 'yes', 'interested']
  const negativeWords = ['no', 'cancel', 'problem', 'issue', 'complaint', 'disappointed']
  ...
  const urgentWords = ['urgent', 'asap', 'immediately', 'emergency', 'now', 'cancel', 'refund']
  ...
  if (lastContent.includes('book') || lastContent.includes('class') || lastContent.includes('session')) {
    suggestions.push(getPersonalizedMessage("I'd be happy to help you book a class! What type of workout are you interested in?", personality))
    suggestions.push(getPersonalizedMessage("Our most popular classes are yoga, HIIT, and strength training. Which would you prefer?", personality))
    nextActions.push('Show available class times')
    nextActions.push('Book preferred class')
  }

  // Membership inquiries
  if (lastContent.includes('member') || lastContent.includes('plan') || lastContent.includes('price')) {
    suggestions.push(getPersonalizedMessage("We have several membership options to suit different needs. Would you like me to send you our pricing?", personality))
    suggestions.push(getPersonalizedMessage("I can offer you a complimentary trial session to try our facilities first. Interested?", personality))
    nextActions.push('Send membership pricing')
    nextActions.push('Schedule trial session')
  }

  // Information requests
  if (lastContent.includes('hour') || lastContent.includes('open') || lastContent.includes('time')) {
    const hours = businessInfo.start && businessInfo.end 
  ...
  if (suggestions.length === 0) {
    suggestions.push(getPersonalizedMessage("Thanks for your message! How can I help you today?", personality))
    suggestions.push(getPersonalizedMessage("Is there anything specific you'd like to know about our gym?", personality))
    suggestions.push(getPersonalizedMessage("Would you like to book a class or learn about our membership options?", personality))
  }

  // Generate summary
  const messageCount = messages.length
  const customerName = customerInfo?.name || 'Customer'
  const conversationTone = sentiment === 'positive' ? 'positive and engaged' : 
                          sentiment === 'negative' ? 'concerned or frustrated' : 
  ...
  const summary = `Conversation with ${customerName} (${messageCount} messages). Customer appears ${conversationTone}. ${
    lastMessage.direction === 'inbound' ? 'Waiting for response' : 'Last message sent'
  }.`
...
function getPersonalizedMessage(baseMessage: string, personality: any): string {
  ...
  if (personality.emoji_usage) {
    ...
    if (personalizedMessage.includes('book') || personalizedMessage.includes('class')) {
      personalizedMessage += ' 📅'
    } else if (personalizedMessage.includes('member') || personalizedMessage.includes('trial')) {
      personalizedMessage += ' 💪'
    } else if (personalizedMessage.includes('help') || personalizedMessage.includes('support')) {
      personalizedMessage += ' 😊'
    }
  }
  ...
  if (personality.formality === 'casual') {
    personalizedMessage = personalizedMessage.replace(/Would you like/, "Would you like")
