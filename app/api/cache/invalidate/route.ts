...
import { cacheService, invalidateOrgCache, invalidateCache } from '@/app/lib/cache/cache-utils';
  ...
  try {
    ...
    switch (type) {
      case 'organization':
        if (!orgId) {
          return NextResponse.json({
            success: false,
            error: 'Missing orgId',
            message: 'Organization ID is required for organization cache invalidation'
          }, { status: 400 });
        }
      ...
      case 'pattern':
        if (!pattern) {
          return NextResponse.json({
            ...
            message: 'Pattern is required for pattern-based cache invalidation'
          }, { status: 400 });
        }
      ...
      case 'all':
        ...
        logger.warn('ALL CACHE FLUSHED - This should only be done in emergency situations');
        break;
      ...
      data: {
        availableTypes: [
          ...
          },
          {
            ...
            warning: 'This will remove ALL cached data across all organizations'
          }
        ],
        commonPatterns: [
          'org:*:dashboard:*',
