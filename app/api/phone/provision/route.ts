...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      await adminSupabase.from("phone_configurations").insert({
      ...
      await adminSupabase.from("phone_settings").insert({
        organization_id: organizationId,
        primary_number: purchasedNumber.phoneNumber,
        display_name: "Your Gym",
        voicemail_enabled: true,
        voicemail_greeting:
          "Thanks for calling! We're currently unavailable. Please leave a message and we'll get back to you soon.",
        text_enabled: true,
      ...
      if (twilioError.code === 21422) {
        return NextResponse.json(
          {
            error:
              "This phone number is no longer available. Please search for another.",
          },
          { status: 400 },
        );
      }
