...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const phoneNumbers = await twilioClient.incomingPhoneNumbers.list({ limit: 20 })
      ...
      if (formattedNumbers.length === 0) {
        return NextResponse.json({
          success: true,
          numbers: [],
          message: 'No phone numbers found. Please purchase a number in your Twilio Console first.'
        })
      }
