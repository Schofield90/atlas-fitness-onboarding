...
export async function GET(request: NextRequest) {
  try {
    ...
    const campaignsWithMetrics = campaigns?.map(campaign => ({
      ...
      cost_per_lead: campaign.leads_count > 0 ? campaign.spend / campaign.leads_count : 0,
      conversion_rate: campaign.clicks > 0 ? (campaign.leads_count / campaign.clicks) * 100 : 0
    })) || [];
