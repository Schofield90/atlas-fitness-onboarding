...
export async function PATCH(
  ...
  try {
    ...
    const integration = (campaign as any).facebook_ad_accounts.facebook_integrations;
    const accessToken = integration.access_token;
    const fbCampaignId = campaign.facebook_campaign_id;

    // Update status on Facebook
    const fbResponse = await fetch(`https://graph.facebook.com/v18.0/${fbCampaignId}`, {
      method: 'POST',
      headers: {
    ...
    if (updateError) {
      ...
      return NextResponse.json({ error: 'Failed to update campaign in database' }, { status: 500 });
    }
    ...
    if (!adsetError) {
      ...
      if (adsets && adsets.length > 0) {
        await supabase
          .from('facebook_ads')
          .update({ status: status })
          .in('adset_id', adsets.map(as => as.id))
          .eq('organization_id', user.organization_id);
      }
    }
