...
export async function POST(
  ...
  try {
    ...
    const integration = (originalCampaign as any).facebook_ad_accounts.facebook_integrations;
    ...
    const newCampaignResponse = await fetch(`https://graph.facebook.com/v18.0/${adAccountId}/campaigns`, {
      method: 'POST',
      headers: {
    ...
    if (insertError) {
      ...
      return NextResponse.json({ error: 'Failed to save duplicate campaign' }, { status: 500 });
    }
      ...
      for (const originalAdSet of originalAdSets) {
        try {
          ...
          if (fbAdSetResponse.ok) {
            ...
            const newAdSetResponse = await fetch(`https://graph.facebook.com/v18.0/${adAccountId}/adsets`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
              },
              ...
              if (!adSetInsertError) {
                ...
                if (!adsError && originalAds && originalAds.length > 0) {
                  ...
                  for (const originalAd of originalAds) {
                    try {
                      const creative = (originalAd as any).facebook_ad_creatives;
                      
                      if (creative) {
                        // Create duplicate creative on Facebook
                        const newCreativeResponse = await fetch(`https://graph.facebook.com/v18.0/${adAccountId}/adcreatives`, {
                          method: 'POST',
                          headers: {
                            'Content-Type': 'application/json',
                          },
                        ...
                        if (newCreativeResponse.ok) {
                          const newFbCreative = await newCreativeResponse.json();
                          ...
                          if (newCreative) {
                            ...
                            const newAdResponse = await fetch(`https://graph.facebook.com/v18.0/${adAccountId}/ads`, {
                              method: 'POST',
                              headers: {
                                'Content-Type': 'application/json',
                              },
