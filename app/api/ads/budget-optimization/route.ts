...
export async function POST(request: NextRequest) {
  try {
    ...
    const { account_id, campaigns }: { account_id: string; campaigns: CampaignData[] } = body;
    ...
    const { data: campaignDetails, error } = await supabase
      ...
      .in('facebook_campaign_id', campaigns.map(c => c.facebook_campaign_id))
    ...
    const campaignNameMap = new Map(campaignDetails?.map(c => [c.facebook_campaign_id, c.campaign_name]) || []);
    ...
    campaigns.forEach(campaign => {
      const campaignName = campaignNameMap.get(campaign.facebook_campaign_id) || `Campaign ${campaign.id}`;
      
      // Calculate key metrics
      const costPerLead = campaign.leads > 0 ? campaign.spend / campaign.leads : Infinity;
      ...
      const budgetUtilizationRate = campaign.current_budget > 0 ? (campaign.spend / campaign.current_budget) * 100 : 0;
      
      // Calculate average metrics for comparison
      const avgCostPerLead = totalLeads > 0 ? totalSpend / totalLeads : 0;
      const avgCTR = campaigns.reduce((sum, c) => sum + (c.impressions > 0 ? (c.clicks / c.impressions) * 100 : 0), 0) / campaigns.length;
      const avgConversionRate = campaigns.reduce((sum, c) => sum + (c.clicks > 0 ? (c.leads / c.clicks) * 100 : 0), 0) / campaigns.length;
      ...
      if (
        ...
        (conversionRate > avgConversionRate * 1.2 && clickThroughRate > avgCTR * 1.1)
      ) {
        const increasePercentage = costPerLead < avgCostPerLead * 0.6 ? 0.5 : 0.3; // 50% or 30% increase
        ...
        potentialLeadsChange = Math.floor((recommendedBudget - campaign.current_budget) / costPerLead);
        confidence = 85;
      }
      ...
      // Poor performing campaign (decrease budget or pause)
        ...
        if (campaign.leads === 0 && campaign.spend > campaign.current_budget * 0.5) {
          recommendedBudget = campaign.current_budget * 0.3; // Significant decrease
          reason = 'No leads generated despite significant spend. Consider pausing or reducing budget dramatically.';
          priority = 'high';
        } else {
        ...
        impact = 'decrease';
        potentialLeadsChange = -Math.floor((campaign.current_budget - recommendedBudget) / Math.max(costPerLead, avgCostPerLead));
        confidence = 80;
      }
      ...
      else if (budgetUtilizationRate > 95 && costPerLead <= avgCostPerLead * 1.1 && campaign.leads >= 3) {
      ...
      if (Math.abs(recommendedBudget - campaign.current_budget) > campaign.current_budget * 0.1) {
    ...
    recommendations.sort((a, b) => {
      ...
      return Math.abs(b.potential_leads_change) - Math.abs(a.potential_leads_change);
    });
    ...
    const sortedByLeads = campaigns
      .sort((a, b) => b.leads - a.leads);

    const bestPerformingCampaigns = sortedByCostPerLead
      .slice(0, 3)
      .map(c => campaignNameMap.get(c.facebook_campaign_id) || `Campaign ${c.id}`);

    const underperformingCampaigns = campaigns
      .filter(c => c.leads === 0 || (c.leads > 0 && c.spend / c.leads > (totalSpend / totalLeads) * 1.5))
      .slice(0, 3)
      .map(c => campaignNameMap.get(c.facebook_campaign_id) || `Campaign ${c.id}`);
