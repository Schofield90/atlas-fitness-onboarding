...
export async function GET(request: NextRequest) {
  try {
    ...
    const averageCPC = totals.totalClicks > 0 ? 
      totals.totalSpend / totals.totalClicks : 0;
    
    const averageCPL = totals.totalLeads > 0 ? 
      totals.totalSpend / totals.totalLeads : 0;
    
    // Calculate ROAS (placeholder - would need conversion value data)
    const roas = totals.totalSpend > 0 ? 
      (totals.totalLeads * 50) / totals.totalSpend : 0; // Assuming $50 per lead value
