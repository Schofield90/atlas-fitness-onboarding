...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!integration.is_active) {
      return NextResponse.json({ error: 'Facebook integration is not active' }, { status: 400 });
    }
        ...
        for (const fbCampaign of campaignData.data || []) {
          ...
          if (insightsResponse.ok) {
            ...
            if (insightsData.data && insightsData.data.length > 0) {
              ...
              if (insight.actions) {
                const leadActions = insight.actions.find((action: any) => 
                  action.action_type === 'lead' || action.action_type === 'offsite_conversion.fb_pixel_lead'
                );
                leads_count = leadActions ? parseInt(leadActions.value || 0) : 0;
              }
            }
          }
          ...
          if (adSet) {
              ...
              if (creative) {
                ...
                const { data: newCreative } = await supabase
                  ...
                  .insert({
                    ...
                    creative_name: fbAd.creative.name || `Creative ${fbAd.creative.id}`,
                    title: fbAd.creative.title || '',
                    body: fbAd.creative.body || '',
                    call_to_action_type: fbAd.creative.call_to_action_type || 'LEARN_MORE',
                    image_url: fbAd.creative.image_url,
                    video_id: fbAd.creative.video_id,
                    creative_type: fbAd.creative.video_id ? 'video' : 'single_image'
                  })
                  .select('id')
                  .single();
            ...
            const { error: adError } = await supabase
              ...
              .upsert({
                ...
                conversion_rate: clicks > 0 ? (leads_count / clicks) * 100 : 0,
                insights_data: insights,
                last_metrics_sync_at: new Date().toISOString(),
