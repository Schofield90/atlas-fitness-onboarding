...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingWaitlist) {
      return NextResponse.json({ error: 'Customer already on waitlist' }, { status: 400 });
    }
    ...
    if (existingBooking) {
      return NextResponse.json({ error: 'Customer already booked for this class' }, { status: 400 });
    }
  ...
  try {
    ...
    if (action === 'promote') {
      ...
      if (classSession.current_bookings >= classSession.max_capacity) {
        return NextResponse.json({ error: 'Class is still full' }, { status: 400 });
      }
