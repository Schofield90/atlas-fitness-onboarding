...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalGrossThisMonth = (monthlyBatches || []).reduce(
      (sum, batch) => sum + (batch.total_gross_pay || 0),
      0,
    );
    const totalNetThisMonth = (monthlyBatches || []).reduce(
      (sum, batch) => sum + (batch.total_net_pay || 0),
      0,
    );
    ...
    const totalHoursThisMonth = (timesheets || []).reduce(
    ...
    const averageHourlyRate =
      timesheets && timesheets.length > 0
        ? timesheets.reduce((sum, ts) => sum + (ts.hourly_rate || 0), 0) /
          timesheets.length
        : 0;
