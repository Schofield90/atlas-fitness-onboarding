    ...
    if (queueName) {
      ...
      stats = {
        ...
        summary: {
          ...
          totalWaiting: Object.values(allStats).reduce((sum: number, s: any) => sum + s.waiting, 0),
          totalActive: Object.values(allStats).reduce((sum: number, s: any) => sum + s.active, 0),
          totalCompleted: Object.values(allStats).reduce((sum: number, s: any) => sum + s.completed, 0),
          totalFailed: Object.values(allStats).reduce((sum: number, s: any) => sum + s.failed, 0),
          totalDelayed: Object.values(allStats).reduce((sum: number, s: any) => sum + s.delayed, 0),
        },
        timestamp: new Date().toISOString(),
      };
    }
...
function convertStatsToCSV(stats: any): string {
  if (stats.queue) {
    ...
    // All queues CSV
    const header = 'Queue,Waiting,Active,Completed,Failed,Delayed,Stalled,Paused';
    const rows = Object.entries(stats.queues).map(([queueName, queueStats]: [string, any]) => {
