...
export async function GET(
  ...
  try {
    ...
    const trainingStats = {
      ...
      completed:
        sop.training_records?.filter((r) => r.status === "completed").length ||
        0,
      in_progress:
        sop.training_records?.filter((r) => r.status === "in_progress")
          .length || 0,
      overdue:
        sop.training_records?.filter((r) => r.status === "overdue").length || 0,
    };
...
export async function PUT(
  ...
  try {
    ...
    const contentChanged =
      updates.content && updates.content !== currentSop.content;
    ...
    if (!canDelete) {
      ...
      if (!userData || userData.role !== "admin") {
        return NextResponse.json(
          {
            error:
              "Only creators can delete draft SOPs or admins can delete any SOP",
          },
          { status: 403 },
        );
      }
