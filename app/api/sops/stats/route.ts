...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sopsError) {
      ...
      return NextResponse.json(
        { error: "Failed to fetch SOP statistics" },
        { status: 500 },
      );
    }
    ...
    const sopStats = {
      total: sops?.length || 0,
      approved:
        sops?.filter((sop) => sop.status === SOP_STATUSES.APPROVED).length || 0,
      review:
        sops?.filter((sop) => sop.status === SOP_STATUSES.REVIEW).length || 0,
      draft:
        sops?.filter((sop) => sop.status === SOP_STATUSES.DRAFT).length || 0,
      archived:
        sops?.filter((sop) => sop.status === SOP_STATUSES.ARCHIVED).length || 0,
      training_required:
        sops?.filter((sop) => sop.training_required).length || 0,
    };
    ...
    if (trainingError) {
      console.error(
        "Error fetching training records for stats:",
        trainingError,
      );
    }
    ...
    const trainingStats = {
      ...
      completed:
        trainingRecords?.filter(
          (record) => record.status === TRAINING_STATUSES.COMPLETED,
        ).length || 0,
      in_progress:
        trainingRecords?.filter(
          (record) => record.status === TRAINING_STATUSES.IN_PROGRESS,
        ).length || 0,
      assigned:
        trainingRecords?.filter(
          (record) => record.status === TRAINING_STATUSES.ASSIGNED,
        ).length || 0,
      overdue:
        trainingRecords?.filter((record) => {
          if (
            record.status !== TRAINING_STATUSES.ASSIGNED &&
            record.status !== TRAINING_STATUSES.IN_PROGRESS
          ) {
            return false;
          }
    ...
    const { data: recentSOPs, error: recentSOPsError } = await supabase
      ...
      .eq("organization_id", organization.id)
      .or(
        `created_at.gte.${sevenDaysAgo.toISOString()},updated_at.gte.${sevenDaysAgo.toISOString()}`,
      )
    ...
    if (recentTraining) {
      recentTraining.forEach((record) => {
        if (
          record.status === TRAINING_STATUSES.COMPLETED &&
          record.completed_at
        ) {
          recentActivity.push({
