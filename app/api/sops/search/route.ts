...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to search SOPs' }, { status: 500 })
    }
    ...
    for (const textResult of textSearchResults || []) {
      if (!existingIds.has(textResult.id)) {
        combinedResults.push({
          ...
          matching_sections: await findMatchingSections(query, textResult.content, textResult.title)
        })
      }
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (sourceError || !sourceSop || !sourceSop.embedding) {
      return NextResponse.json({ error: 'SOP not found or no embedding available' }, { status: 404 })
    }
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to find similar SOPs' }, { status: 500 })
    }
