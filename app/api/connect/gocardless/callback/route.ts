...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return redirect(`/billing?error=${encodeURIComponent(errorDescription || error)}`)
    }
    ...
    return redirect(`/billing?error=${encodeURIComponent('Failed to connect GoCardless account')}`)
  }
}