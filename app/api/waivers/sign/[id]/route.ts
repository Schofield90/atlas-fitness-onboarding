...
const signWaiverSchema = z.object({
  ...
  signature_method: z.enum(['digital', 'wet_signature', 'uploaded']).default('digital'),
...
export async function GET(
  ...
  try {
    ...
    if (customerWaiver.expires_at && new Date(customerWaiver.expires_at) < new Date()) {
      return NextResponse.json({ 
...
export async function POST(
  ...
  try {
    ...
    try {
      const { WaiverNotificationService } = await import('@/app/lib/services/waiver-notification-service')
      ...
      console.error('Error sending waiver signed notifications:', notificationError)
