...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to fetch customer waivers' }, { status: 500 })
    }

    // Also get customer details for each waiver
    const customerIds = [...new Set(customerWaivers?.map(cw => cw.customer_id) || [])]
    ...
    leads?.forEach(l => customerMap.set(l.id, {
      ...l,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      if (error.code === '23505') {
        return NextResponse.json({ error: 'Customer already has this waiver assigned' }, { status: 400 })
      }
      console.error('Error assigning waiver:', error)
      return NextResponse.json({ error: 'Failed to assign waiver' }, { status: 500 })
    }
    ...
    if (validatedData.send_email) {
      ...
      console.log(`Should send waiver email for customer ${validatedData.customer_id}`)
    }

    return NextResponse.json({ success: true, data: customerWaiver }, { status: 201 })
  } catch (error) {
