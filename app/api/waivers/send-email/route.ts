...
export async function POST(request: NextRequest) {
  try {
    ...
    if (fetchError || !customerWaiver) {
      return NextResponse.json({ error: 'Customer waiver not found' }, { status: 404 })
    }
    ...
    const notificationData = {
      ...
      isReminder,
      reminderCount: isReminder ? (customerWaiver.reminder_count || 0) + 1 : undefined,
    }

    const emailSent = await waiverService.sendWaiverAssignmentEmail(notificationData)
