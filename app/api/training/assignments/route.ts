...
export async function GET(request: NextRequest) {
  try {
    ...
    if (includeStats) {
      ...
      const statusCounts = statsData?.reduce((acc: any, record) => {
      ...
      stats = {
        ...
        completion_rate: totalAssignments > 0 ? ((statusCounts.completed || 0) / totalAssignments * 100).toFixed(1) : '0'
      };
    }
  ...
  try {
    ...
    switch (action) {
      case 'bulk_extend_due_date':
        if (!data?.due_date) {
          return NextResponse.json({ error: 'Due date is required for extend action' }, { status: 400 });
        }
