...
export async function GET() {
  try {
    ...
    const stripeStats = {
      ...
      volume30d:
        stripeCharges?.reduce(
          (sum, c) =>
            c.status === "succeeded" ? sum + c.amount_cents / 100 : sum,
          0,
        ) || 0,
      fees30d:
        stripeCharges?.reduce(
          (sum, c) =>
            c.status === "succeeded" ? sum + c.platform_fee_cents / 100 : sum,
          0,
        ) || 0,
      successRate: stripeCharges?.length
        ? Math.round(
            (stripeCharges.filter((c) => c.status === "succeeded").length /
    ...
    const gocardlessStats = {
      ...
      successRate: gcCharges?.length
        ? Math.round(
            (gcCharges.filter((c) => c.status === "succeeded").length /
              gcCharges.length) *
              100,
          )
        : 0,
