...
export async function GET() {
  try {
    ...
    const organizationId = userOrg?.organization_id || orgMember?.organization_id
  ...
  try {
    ...
    for (const query of fixQueries) {
      try {
        ...
        results.push({ query: query.substring(0, 50) + '...', error: error?.message || 'Success' })
      } catch (e: any) {
        results.push({ query: query.substring(0, 50) + '...', error: e?.message || 'Failed' })
      }
    }
