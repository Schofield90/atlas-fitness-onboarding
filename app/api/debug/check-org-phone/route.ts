...
export async function GET(request: NextRequest) {
  try {
    ...
    const hasOrgIdColumn =
      !testError ||
      !testError.message.includes('column "organization_id" does not exist');
    ...
    return NextResponse.json({
      organization: {
        ...
        readyForCalls:
          ...
          userWithOrg.organizationId === "63589490-8f55-4157-bd3a-e141594b740e",
      },
      twilioConfig,
      databaseStatus: {
...
function getRecommendations(org: any, hasOrgIdColumn: boolean): string[] {
  ...
  if (
    ...
    org.id === "63589490-8f55-4157-bd3a-e141594b740e"
  ) {
    recommendations.push(
      "Run the multi-tenant migration to set your phone number: +447450308627",
    );
  }

  if (!hasOrgIdColumn) {
    recommendations.push(
      "The migration has not been applied. Run the SQL migration first.",
    );
  }
  ...
  if (
    ...
    recommendations.push(
      "This organization needs a phone number. Contact support for provisioning.",
    );
  }
