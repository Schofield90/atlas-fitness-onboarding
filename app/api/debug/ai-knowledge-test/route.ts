...
import { fetchRelevantKnowledge, fetchCoreKnowledge } from '@/app/lib/knowledge'
import { generateAIResponse, formatKnowledgeContext } from '@/app/lib/ai/anthropic'

export async function GET(request: NextRequest) {
  try {
    const searchParams = request.nextUrl.searchParams
    const testMessage = searchParams.get('message') || 'where is the gym located'
    ...
    let aiError = null
    try {
      aiResponse = await generateAIResponse(testMessage, '+447123456789', knowledgeContext)
    } catch (error) {
    ...
    const hasRealData = {
      harrogate: allKnowledge?.some(k => k.content.toLowerCase().includes('harrogate')) || false,
      york: allKnowledge?.some(k => k.content.toLowerCase().includes('york')) || false,
      claroCount: allKnowledge?.some(k => k.content.toLowerCase().includes('claro court')) || false,
      austerRoad: allKnowledge?.some(k => k.content.toLowerCase().includes('auster road')) || false,
      hasAddresses: allKnowledge?.some(k => 
    ...
    const locationKnowledge = allKnowledge?.filter(k => 
    ...
    return NextResponse.json({
      ...
      knowledgeTypes: allKnowledge?.reduce((acc, k) => {
      ...
      sampleKnowledge: allKnowledge?.slice(0, 5).map(k => ({
        type: k.type,
      ...
      aiTestResult: aiError ? {
        ...
        usesRealData: aiResponse?.message.includes('Harrogate') || 
                      aiResponse?.message.includes('York') || 
                      aiResponse?.message.includes('Claro Court') ||
                      aiResponse?.message.includes('Auster Road')
      },
      recommendations: generateRecommendations(hasRealData, locationKnowledge.length, aiResponse?.message || '')
    })
  } catch (error) {
...
function generateRecommendations(hasRealData: any, locationCount: number, aiResponse: string): string[] {
  const recommendations = []
  
  if (!hasRealData.harrogate && !hasRealData.york) {
    recommendations.push('No gym location data found. Add location information to knowledge base.')
  }
  
  if (!hasRealData.hasPrices) {
    recommendations.push('No pricing data found. Add membership prices to knowledge base.')
  }
  
  if (locationCount === 0) {
    recommendations.push('No location-specific knowledge entries. Create FAQ entries with gym addresses.')
  }
  
  if (aiResponse && !aiResponse.includes('Harrogate') && !aiResponse.includes('York')) {
    recommendations.push('AI is not using real location data. Check knowledge formatting and AI prompt.')
  }
  
  if (recommendations.length === 0) {
    recommendations.push('Knowledge base appears properly configured with real gym data.')
  }
