...
export async function POST() {
  try {
    ...
    try {
      ...
      results.push({ table: 'booking_links', status: 'failed', error: e.message })
    }
    ...
    try {
      ...
      results.push({ table: 'appointment_types', status: 'failed', error: e.message })
    }
    ...
    if (insertError) {
      results.push({ action: 'insert_appointment_types', status: 'failed', error: insertError.message })
    } else {
    ...
    const { data: testAppointmentTypes, error: testAppointmentError } = await supabase
      .from('appointment_types')
      .select('*')
      .limit(3)
