...
export async function GET() {
  try {
    ...
    try {
      const syncRes = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/integrations/meta/sync-pages`, {
        method: 'POST',
        headers: {
          'Cookie': `${Object.entries(await supabase.auth.getSession()).map(([k,v]) => `${k}=${v}`).join('; ')}`
        }
      })
    ...
    try {
      const oldRes = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/integrations/facebook/pages`, {
        headers: {
          'Cookie': `${Object.entries(await supabase.auth.getSession()).map(([k,v]) => `${k}=${v}`).join('; ')}`
        }
      })
    ...
    return NextResponse.json({
      ...
      integration: {
        ...
        token_length: integration.access_token ? integration.access_token.length : 0
      },
      database: {
      ...
      recommendations: [
        (!dbPages || dbPages.length === 0) && directPages && directPages.length > 0 && 
        ...
        oldEndpointError && 'Old endpoint failing - likely due to column name issue (fixed now)'
      ].filter(Boolean)
    })
