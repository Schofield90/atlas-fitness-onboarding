  ...
  try {
    ...
    try {
      const accountsResponse = await fetch(
        "https://graph.facebook.com/v18.0/me/accounts?fields=id,name,category,access_token",
        {
          headers: {
            Authorization: `Bearer ${integration.access_token}`,
          },
    ...
    try {
      const debugResponse = await fetch(
        `https://graph.facebook.com/v18.0/debug_token?input_token=${integration.access_token}&access_token=${integration.access_token}`,
      );
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        has_pages_show_list: results.permissions?.data?.some(
          (p: any) =>
