...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      expectedWebhooks,
      recentCalls,
      recommendations: generateRecommendations(envCheck, phoneNumberInfo, twilioError)
    })
...
function generateRecommendations(envCheck: any, phoneNumberInfo: any, twilioError: any): string[] {
  const recommendations = []
  
  if (!envCheck.TWILIO_ACCOUNT_SID || !envCheck.TWILIO_AUTH_TOKEN) {
    recommendations.push('Twilio credentials not configured in environment variables')
  }
  
  if (envCheck.TWILIO_SMS_FROM === 'NOT SET') {
    recommendations.push('TWILIO_SMS_FROM not configured - this is required for making calls')
  }
  
  if (envCheck.USER_PHONE_NUMBER === 'NOT SET') {
    recommendations.push('USER_PHONE_NUMBER not configured - calls will not connect to any destination')
  }
  ...
  if (phoneNumberInfo) {
    if (!phoneNumberInfo.capabilities.voice) {
      recommendations.push('Your Twilio phone number does not have voice capabilities enabled')
    }
    
    if (phoneNumberInfo.voiceUrl && !phoneNumberInfo.voiceUrl.includes('atlas-fitness-onboarding')) {
      recommendations.push(`Voice webhook is pointing to: ${phoneNumberInfo.voiceUrl} - should point to your app`)
    }
  }
