...
export async function GET(request: NextRequest) {
  try {
    ...
    whatsappPhones?.forEach(log => {
      ...
      if (log.from_number) uniquePhones.add(`WhatsApp from: ${log.from_number}`)
    })
    ...
    return NextResponse.json({
      ...
      recommendations: {
        ...
        explanation: 'The lead phone number format might not match how Twilio saves them',
        commonFormats: [
        ...
        solution: 'Check the phone formats above and ensure lead phone numbers match'
      }
    })
  } catch (error: any) {
    return NextResponse.json({
