...
export async function GET() {
  try {
    ...
    const { data: leads } = await supabase
      .from('leads')
      .select('*')
      .or(`name.ilike.%Sam Schofield%,first_name.ilike.%Sam%,email.eq.samschofield90@hotmail.co.uk`);
    ...
    results.foreignKeyInfo = constraintCheck || 'Could not fetch foreign key info';
