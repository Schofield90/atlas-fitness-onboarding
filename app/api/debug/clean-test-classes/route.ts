...
export async function DELETE() {
  try {
    ...
    if (fetchError) {
      return NextResponse.json({ error: 'Failed to fetch programs', details: fetchError }, { status: 500 })
    }

    if (!programs || programs.length === 0) {
      return NextResponse.json({ message: 'No "Group Pt" program found to clean up' })
    }
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      totalSessions: programStats?.reduce((sum, p) => sum + p.sessionCount, 0) || 0
    })
