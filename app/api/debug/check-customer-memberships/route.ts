...
export async function GET(request: Request) {
  try {
    ...
    const { data: customers, error: customerError } = await supabase
      .from('clients')
      .select('*')
      .or(`name.ilike.%${customerName}%,first_name.ilike.%${customerName.split(' ')[0]}%`)
      .limit(5);
    ...
    const membershipResults = await Promise.all(customers.map(async (customer) => {
    ...
    return NextResponse.json({
      ...
      bookingTableColumns: sampleBooking?.[0] ? Object.keys(sampleBooking[0]) : 'No bookings found'
    });
