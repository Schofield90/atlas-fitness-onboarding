...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      alternativeQueries: {
        selectStar: { count: selectStarData?.length || 0, data: selectStarData },
        single: { data: singleData }
      },
      diagnosis: {
        userExistsInDb: !!(adminData && adminData.length > 0),
        rlsBlockingAccess: !!(adminData && adminData.length > 0 && (!rlsData || rlsData.length === 0)),
        recommendation: !!(adminData && adminData.length > 0 && (!rlsData || rlsData.length === 0)) 
