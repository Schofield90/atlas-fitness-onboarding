...
export async function GET(request: Request) {
  try {
    ...
    const { data: directCount, error: countError } = await adminSupabase
      ...
      .eq('class_session_id', classId || '00000000-0000-0000-0000-000000000000');
    ...
    return NextResponse.json({
      ...
      diagnosis: {
        rlsBlockingBookings: (adminBookings?.length || 0) > (regularBookings?.length || 0),
        bookingsExist: (adminBookings?.length || 0) > 0,
        apiCanSeeBookings: (classWithBookingsRegular?.bookings?.length || 0) > 0
      }
    });
  } catch (error: any) {
