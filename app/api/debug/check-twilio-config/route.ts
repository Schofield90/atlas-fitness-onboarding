...
export async function GET(request: NextRequest) {
  try {
    ...
    const config = {
      ...
      TWILIO_SMS_FROM: {
        ...
        format: process.env.TWILIO_SMS_FROM ? 
          (process.env.TWILIO_SMS_FROM.startsWith('+') ? 'CORRECT (starts with +)' : 'INCORRECT (missing + prefix)') : 
          'N/A'
      },
      TWILIO_WHATSAPP_FROM: {
        ...
        format: process.env.TWILIO_WHATSAPP_FROM ? 
          (process.env.TWILIO_WHATSAPP_FROM.startsWith('whatsapp:') ? 'CORRECT (has whatsapp: prefix)' : 'INCORRECT (missing whatsapp: prefix)') : 
          'N/A'
      },
      NEXT_PUBLIC_APP_URL: {
    ...
    const features = {
      sms: config.TWILIO_ACCOUNT_SID.present && 
           config.TWILIO_AUTH_TOKEN.present && 
           config.TWILIO_SMS_FROM.present,
      whatsapp: config.TWILIO_ACCOUNT_SID.present && 
                config.TWILIO_AUTH_TOKEN.present && 
                config.TWILIO_WHATSAPP_FROM.present,
      calling: config.TWILIO_ACCOUNT_SID.present && 
               config.TWILIO_AUTH_TOKEN.present && 
               config.TWILIO_SMS_FROM.present &&
    ...
    if (!config.TWILIO_ACCOUNT_SID.present) {
      recommendations.push('Add TWILIO_ACCOUNT_SID to Vercel environment variables')
    }
    
    if (!config.TWILIO_AUTH_TOKEN.present) {
      recommendations.push('Add TWILIO_AUTH_TOKEN to Vercel environment variables')
    }
    
    if (!config.TWILIO_SMS_FROM.present) {
      recommendations.push('Add TWILIO_SMS_FROM with your Twilio phone number (e.g., +447777777777)')
    } else if (!config.TWILIO_SMS_FROM.value.startsWith('+')) {
      recommendations.push('TWILIO_SMS_FROM must start with + and country code (e.g., +447777777777)')
    }
    
    if (!config.NEXT_PUBLIC_APP_URL.present) {
      recommendations.push('Add NEXT_PUBLIC_APP_URL (e.g., https://atlas-fitness-onboarding.vercel.app)')
    }
    ...
    if (config.TWILIO_ACCOUNT_SID.present && config.TWILIO_AUTH_TOKEN.present) {
      try {
        ...
        if (error.code === 20003) {
          recommendations.push('Twilio authentication failed - check your Account SID and Auth Token')
        }
      }
    }
