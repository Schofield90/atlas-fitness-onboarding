...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const tableChecks = await adminSupabase.rpc('to_json', {
        query: `
          SELECT 
            table_name,
            EXISTS (
    ...
    try {
      const userId = diagnostics.checks.auth.details?.userId
      const orgId = diagnostics.checks.auth.details?.organizationId || '63589490-8f55-4157-bd3a-e141594b748e'
      ...
      if (userId) {
        ...
        if (regularInsert?.id) {
          await supabase.from('calendar_events').delete().eq('id', regularInsert.id)
        }
        if (adminInsert?.id) {
          await adminSupabase.from('calendar_events').delete().eq('id', adminInsert.id)
        }
    ...
    if (!diagnostics.checks.auth.details?.hasOrganizationId) {
      recs.push({
        ...
        solution: 'Run: UPDATE auth.users SET raw_user_meta_data = raw_user_meta_data || \'{"organization_id": "63589490-8f55-4157-bd3a-e141594b748e"}\'::jsonb WHERE id = \'' + diagnostics.checks.auth.details?.userId + '\''
      })
    }
    ...
    if (diagnostics.checks.permissions.details?.membershipError) {
      recs.push({
        ...
        solution: 'Run: INSERT INTO user_organizations (user_id, organization_id, role) VALUES (\'' + diagnostics.checks.auth.details?.userId + '\', \'63589490-8f55-4157-bd3a-e141594b748e\', \'owner\')'
      })
    }

    if (diagnostics.checks.testInsert.details?.regularClientError && !diagnostics.checks.testInsert.details?.adminClientError) {
      recs.push({
        priority: 'high',
