...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: orgEvents, error: orgError } = await supabase
      .from('calendar_events')
      .select('*')
      .eq('organization_id', membership?.organization_id || user.user_metadata?.organization_id)
    ...
    return NextResponse.json({
      debug: {
        userId: user.id,
        organizationId: membership?.organization_id || user.user_metadata?.organization_id,
      ...
      weeklyEvents: weekEvents?.map(e => ({
        ...
        title: e.title,
        startTime: e.start_time,
        dayOfWeek: new Date(e.start_time).toLocaleDateString('en-US', { weekday: 'long' }),
