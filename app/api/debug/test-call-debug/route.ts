...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      recommendations: getRecommendations(orgCheck, twilioConfig, credentialsUsed, twilioError, callError)
    })
...
function getRecommendations(orgCheck: any, twilioConfig: any, credentialsUsed: any, twilioError: any, callError: any): string[] {
  const recommendations = []
  
  if (!orgCheck.twilioPhoneNumber) {
    recommendations.push('Organization does not have a Twilio phone number set. Run the migration.')
  }
  
  if (!twilioConfig.hasAccountSid || !twilioConfig.hasAuthToken) {
    recommendations.push('Twilio credentials are not configured in environment variables.')
  }
  ...
  if (callError) {
    if (callError.code === 21215) {
      recommendations.push('Invalid phone number format. Ensure numbers include country code (e.g., +447123456789)')
    } else if (callError.code === 21210) {
      recommendations.push(`The from phone number (${credentialsUsed.fromNumber}) is not verified or owned by your Twilio account.`)
    } else {
