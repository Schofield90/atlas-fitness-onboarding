    ...
    <DashboardLayout userData={null}>
      <div className="min-h-screen bg-gray-900">
        <div className="container mx-auto px-6 py-8 max-w-4xl">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-white mb-2">Price Migration Tool</h1>
            <p className="text-gray-400">
              Migrate membership plan prices from legacy 'price' field to 'price_pennies' field
            </p>
          </div>
          ...
          <div className="bg-blue-900/20 border border-blue-800 rounded-lg p-4 mb-6">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-gray-300">
                <p className="mb-2">
                  This tool will migrate prices from the old decimal format to the new pennies format:
                </p>
                <ul className="list-disc ml-5 space-y-1 text-gray-400">
                  ...
                  <li>Only plans with price set but price_pennies empty will be migrated</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-4 mb-8">
            ...
            <button
              ...
              disabled={loading || !status || status.summary.needs_migration === 0}
          ...
          {status && (
            <div className="space-y-6">
              ...
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="bg-gray-700/50 rounded-lg p-3">
                    <div className="text-3xl font-bold text-white">{status.summary.total_plans}</div>
                    <div className="text-sm text-gray-400">Total Plans</div>
                  </div>
                  <div className="bg-gray-700/50 rounded-lg p-3">
                    <div className="text-3xl font-bold text-blue-400">{status.summary.plans_with_price}</div>
                    <div className="text-sm text-gray-400">With Price Field</div>
                  </div>
                  <div className="bg-gray-700/50 rounded-lg p-3">
                    <div className="text-3xl font-bold text-green-400">{status.summary.plans_with_price_pennies}</div>
                    <div className="text-sm text-gray-400">With Price Pennies</div>
                  </div>
                  <div className="bg-gray-700/50 rounded-lg p-3">
                    <div className="text-3xl font-bold text-orange-400">{status.summary.needs_migration}</div>
              ...
              {/* Plans List */}
              <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
                <h2 className="text-xl font-semibold text-white mb-4">Membership Plans</h2>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-400">
                              price: {plan.price !== null ? `£${plan.price}` : 'null'} | 
                              price_pennies: {plan.price_pennies !== null ? `${plan.price_pennies}p` : 'null'}
                            </div>
                          </div>
                        </div>
          ...
          {migrationResult && (
            <div className="mt-6 bg-gray-800 rounded-lg p-6 border border-gray-700">
              <h2 className="text-xl font-semibold text-white mb-4">Migration Result</h2>
              ...
              {migrationResult.error ? (
                <div className="bg-red-900/20 border border-red-800 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-red-300 font-medium">{migrationResult.error}</p>
                      {migrationResult.details && (
                        <p className="text-red-400 text-sm mt-1">{migrationResult.details}</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <>
                  <div className="bg-green-900/20 border border-green-800 rounded-lg p-4 mb-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-green-300 font-medium">{migrationResult.message}</p>
                        <p className="text-green-400 text-sm mt-1">
                          Checked: {migrationResult.checked} plans | Migrated: {migrationResult.migrated} plans
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="mt-8 text-center text-gray-500 text-sm">
            <p>This is a one-time migration tool. Once all plans are migrated, this tool is no longer needed.</p>
            <p className="mt-1">The application now uses price_pennies for all price storage and display.</p>
          </div>
        </div>
      </div>
    </DashboardLayout>
