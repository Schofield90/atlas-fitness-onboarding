...
export default function OrganizationHeader({ organization, metrics }: OrganizationHeaderProps) {
      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <div className="flex items-center gap-2 mt-3">
            <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(organization.subscription_status || 'inactive')}`}>
            ...
            {organization.trial_ends_at && new Date(organization.trial_ends_at) > new Date() && (
              <span className="px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800">
                Trial ends {formatDistanceToNow(new Date(organization.trial_ends_at), { addSuffix: true })}
              </span>
            )}
          </div>
        </div>
          ...
          <div>
            ...
            <p className="text-lg font-semibold">
              {metrics.last_activity 
                ? formatDistanceToNow(new Date(metrics.last_activity), { addSuffix: true })
                : 'Never'
              }
            </p>
          </div>
