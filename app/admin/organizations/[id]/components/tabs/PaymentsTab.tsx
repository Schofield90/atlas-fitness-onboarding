...
export default function PaymentsTab({ organizationId }: PaymentsTabProps) {
  ...
  const fetchPaymentData = async () => {
    try {
      const res = await fetch(`/api/admin/organizations/${organizationId}/payments`)
      if (res.ok) {
  ...
  return (
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <p className="text-2xl font-bold">£{stats.revenue30d?.toLocaleString() || 0}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <p className="text-sm text-gray-500">Platform Fees (30d)</p>
            <p className="text-2xl font-bold">£{stats.fees30d?.toLocaleString() || 0}</p>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
      ...
      {/* Connected Accounts */}
      <div>
        <h3 className="text-lg font-medium text-gray-900 mb-4">Payment Processors</h3>
                  ...
                  {connectedAccounts?.stripe_account_id ? (
                    <>
                      ...
                      {connectedAccounts.stripe_charges_enabled && (
                        <span className="ml-2 text-green-600">✓ Charges enabled</span>
                      )}
                    </>
                  ) : (
                    'Not connected'
      ...
      {/* Recent Charges */}
      <div>
        <h3 className="text-lg font-medium text-gray-900 mb-4">Recent Gym Charges</h3>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Processor</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {recentCharges.map((charge) => (
                  <tr key={charge.id}>
                    ...
                    <td className="px-4 py-2 text-sm">{charge.client_name || 'Unknown'}</td>
                    <td className="px-4 py-2 text-sm">£{(charge.amount_cents / 100).toFixed(2)}</td>
                    <td className="px-4 py-2 text-sm">£{(charge.platform_fee_cents / 100).toFixed(2)}</td>
                    <td className="px-4 py-2 text-sm capitalize">{charge.processor}</td>
