...
export default function PaymentProcessorStats() {
  ...
  return (
    <div className="bg-white shadow rounded-lg p-6">
      <h3 className="text-lg font-medium text-gray-900 mb-6">Payment Processor Performance</h3>
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className={`text-sm font-medium ${
                (stats?.stripe.successRate || 0) >= 95 ? 'text-green-600' :
                (stats?.stripe.successRate || 0) >= 90 ? 'text-yellow-600' :
                'text-red-600'
              }`}>
            ...
            <div className="pt-3 border-t">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-medium">
                  £{stats?.stripe.connected ? Math.round(stats.stripe.volume30d / stats.stripe.connected) : 0}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="border rounded-lg p-4">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              <span className="text-sm text-gray-500">Success Rate</span>
              <span className={`text-sm font-medium ${
                (stats?.gocardless.successRate || 0) >= 95 ? 'text-green-600' :
                (stats?.gocardless.successRate || 0) >= 90 ? 'text-yellow-600' :
                'text-red-600'
              }`}>
            ...
            <div className="pt-3 border-t">
              <div className="flex justify-between items-center">
                <span className="text-xs text-gray-500">Avg Transaction</span>
                <span className="text-sm font-medium">
                  £{stats?.gocardless.connected ? Math.round(stats.gocardless.volume30d / stats.gocardless.connected) : 0}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 text-center">
          <div>
            <p className="text-2xl font-bold text-gray-900">
              £{((stats?.stripe.volume30d || 0) + (stats?.gocardless.volume30d || 0)).toLocaleString()}
            </p>
            <p className="text-sm text-gray-500">Total Volume (30d)</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-gray-900">
              £{((stats?.stripe.fees30d || 0) + (stats?.gocardless.fees30d || 0)).toLocaleString()}
            </p>
            <p className="text-sm text-gray-500">Total Platform Fees (30d)</p>
          </div>
          <div>
            <p className="text-2xl font-bold text-gray-900">
              {((stats?.stripe.connected || 0) + (stats?.gocardless.connected || 0))}
            </p>
