...
import { Package, Plus, Edit, Trash2, Check, X, TrendingUp, Users, Mail, Brain } from 'lucide-react';
              ...
              >
                ...
                <div className="space-y-2 mb-6">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-gray-300">
                      {plan.max_email_sends_per_month ? `${plan.max_email_sends_per_month} emails/mo` : 'Unlimited emails'}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <Brain className="h-4 w-4 text-gray-400" />
                    <span className="text-gray-300">
                      {plan.max_ai_credits_per_month ? `${plan.max_ai_credits_per_month} AI credits/mo` : 'Unlimited AI'}
                    </span>
                  </div>
                </div>
        ...
        {showForm && (
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {/* Limits */}
              <div className="border-t border-gray-700 pt-6">
                <h3 className="text-lg font-semibold text-white mb-4">Usage Limits (Leave empty for unlimited)</h3>
                <div className="grid md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">
                      Max Members
                    ...
                    <input
                      ...
                      onChange={e => setFormData({ ...formData, max_members: e.target.value ? parseInt(e.target.value) : null })}
                      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setFormData({ ...formData, max_staff: e.target.value ? parseInt(e.target.value) : null })}
                      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setFormData({ ...formData, max_email_sends_per_month: e.target.value ? parseInt(e.target.value) : null })}
                      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setFormData({ ...formData, max_sms_sends_per_month: e.target.value ? parseInt(e.target.value) : null })}
                      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setFormData({ ...formData, max_ai_credits_per_month: e.target.value ? parseInt(e.target.value) : null })}
                      className="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white"
                    />
                  </div>
                </div>
              ...
              <div className="border-t border-gray-700 pt-6">
                ...
                {/* Core Features */}
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-gray-400 mb-2">Core Features</h4>
                  <div className="grid md:grid-cols-3 gap-3">
                    {['lead_management', 'booking_system', 'class_scheduling', 'member_portal', 'staff_management'].map(feature => (
                      <label key={feature} className="flex items-center gap-2 text-gray-300">
                        <input
                          type="checkbox"
                          checked={formData.features?.core?.[feature] || false}
                          onChange={e => updateFeature('core', feature, e.target.checked)}
                ...
                <div className="mb-4">
                  ...
                  <div className="grid md:grid-cols-3 gap-3">
                    {['email', 'sms', 'whatsapp', 'voice_calls', 'dedicated_email_server'].map(feature => (
                      <label key={feature} className="flex items-center gap-2 text-gray-300">
                        <input
                          type="checkbox"
                          checked={formData.features?.communication?.[feature] || false}
                          onChange={e => updateFeature('communication', feature, e.target.checked)}
                          className="rounded text-orange-500"
                        />
                        {feature.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                      </label>
                ...
                {/* AI Features */}
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-gray-400 mb-2">AI Features</h4>
                  <div className="grid md:grid-cols-3 gap-3">
                    {['lead_scoring', 'content_generation', 'chat_assistant', 'workout_generation', 'nutrition_planning'].map(feature => (
                      <label key={feature} className="flex items-center gap-2 text-gray-300">
                        <input
                          type="checkbox"
                          checked={formData.features?.ai?.[feature] || false}
                          onChange={e => updateFeature('ai', feature, e.target.checked)}
                          className="rounded text-orange-500"
                        />
                        {feature.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}
                      </label>
