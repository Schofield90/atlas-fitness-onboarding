...
export default function MigrateAllPage() {
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      setStatus({ error: error instanceof Error ? error.message : 'Failed to check status' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const runMigration = async () => {
    if (!confirm('This will migrate ALL membership plans in the database. Continue?')) {
      return
    }
      ...
      <div className="min-h-screen bg-gray-900 p-8">
        <div className="max-w-4xl mx-auto">
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-4 gap-4">
                  <div className="bg-gray-700 p-3 rounded">
                    <div className="text-2xl font-bold text-white">{status.summary?.total_plans || 0}</div>
                    <div className="text-sm text-gray-400">Total Plans</div>
                  </div>
                  <div className="bg-gray-700 p-3 rounded">
                    <div className="text-2xl font-bold text-blue-400">{status.summary?.plans_with_price || 0}</div>
                    <div className="text-sm text-gray-400">With Price</div>
                  </div>
                  <div className="bg-gray-700 p-3 rounded">
                    <div className="text-2xl font-bold text-green-400">{status.summary?.plans_with_price_pennies || 0}</div>
                    <div className="text-sm text-gray-400">With Price Pennies</div>
                  </div>
                  <div className="bg-gray-700 p-3 rounded">
                    <div className="text-2xl font-bold text-orange-400">{status.summary?.needs_migration || 0}</div>
          ...
          {status?.plans && status.plans.length > 0 && (
            <div className="bg-gray-800 rounded-lg p-6 mb-6 border border-gray-700">
              <h2 className="text-xl font-semibold text-white mb-4">All Membership Plans</h2>
              <div className="space-y-2 max-h-96 overflow-y-auto">
                {status.plans.map((plan: any) => (
                  <div key={plan.id} className="flex items-center justify-between p-3 bg-gray-700/50 rounded">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400">
                          Org: {plan.organization_id?.slice(0, 8)}... | 
                          Price: {plan.price ?? 'null'} | 
                          Pennies: {plan.price_pennies ?? 'null'}
                        </div>
                      </div>
                    </div>
                    {plan.needs_migration && (
              ...
              {result.error ? (
                ...
                <div className="space-y-4">
                  <div className="bg-green-900/20 border border-green-800 rounded p-4">
                    <div className="text-green-400 font-medium">{result.message}</div>
                    <div className="text-sm text-green-500 mt-1">
                      Checked: {result.checked} | Attempted: {result.attempted} | Success: {result.migrated}
                    </div>
                  </div>
