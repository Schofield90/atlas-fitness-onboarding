...
describe("Membership Functionality", () => {
  ...
  describe("AddMembershipModal", () => {
    ...
    test("displays membership plan selection", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Select a membership plan"),
        ).toBeInTheDocument();
      });
    });
    ...
    test("prevents duplicate membership creation", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/already has an active membership/i),
        ).toBeInTheDocument();
      });
    });
    ...
    test("handles API errors gracefully", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/failed to add membership/i),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Membership Business Logic", () => {
    test("calculates prorated amount correctly", () => {
      ...
      const proratedAmount = Math.round(
        (monthlyPrice / daysInMonth) * daysRemaining,
      );
    ...
    test("validates membership overlap", () => {
      ...
      const hasOverlap =
        new Date(newStartDate) <= new Date(existingMembership.end_date);
    ...
    test("checks membership expiry", () => {
      ...
      const isExpired =
        new Date() > new Date(membership.end_date) &&
