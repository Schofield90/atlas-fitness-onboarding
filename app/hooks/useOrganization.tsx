  ...
  const fetchOrganization = async () => {
    try {
      ...
      const { data: orgData, error: orgError } = await supabase
        .rpc('get_user_organization_id', { user_uuid: currentUser.id })
      ...
      // Fetch full organization details with staff info
      const { data: orgDetails, error: detailsError } = await supabase
        ...
        `)
        .eq('id', orgData)
        .eq('organization_staff.user_id', currentUser.id)
        .single()
      ...
      if (err.code && err.code !== 'PGRST116') { // Not found error
