...
export default function FieldMappingInterface({
  ...
  const [defaultLeadSource, setDefaultLeadSource] =
    useState("Facebook Lead Form");
  ...
  const loadFieldMappings = async () => {
    try {
      ...
      const response = await fetch(
        `/api/integrations/facebook/field-mappings?formId=${formId}`,
      );
      const data = await response.json();

      if (data.mappings) {
        // Use existing mappings
        // Note: data.mappings is the StoredFieldMappings object
        ...
        setDefaultLeadSource(
          data.mappings.default_lead_source || "Facebook Lead Form",
        );
      } else {
  ...
  const refreshFormQuestions = async () => {
    try {
      ...
      const response = await fetch(
        "/api/integrations/facebook/refresh-form-questions",
        {
          method: "POST",
  ...
  const autoDetectMappings = async () => {
    try {
      ...
      const response = await fetch(
        "/api/integrations/facebook/auto-detect-mappings",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ formId }),
  ...
  const removeMapping = (fieldName: string) => {
    setMappings((prev) =>
      prev.filter((m) => m.facebook_field_name !== fieldName),
    );
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(
        "/api/integrations/facebook/field-mappings",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (result.success) {
        ...
        setValidationErrors(
          result.validation_errors || ["Failed to save mappings"],
        );
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        ...
        <p className="text-gray-600">
          Map Facebook form fields to your CRM fields for:{" "}
        ...
        {formFields.length === 0 ? (
          ...
          <div className="space-y-3">
            {mappings.map((mapping) => (
              <div
                key={mapping.facebook_field_name}
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="font-medium">
                    {mapping.facebook_field_label}
                  </div>
                  <div className="text-xs text-gray-400">
                    {FACEBOOK_FIELD_TYPES[mapping.facebook_field_type] ||
                      mapping.facebook_field_type}
                  </div>
                </div>
                ...
                <div className="flex-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateMapping(mapping.facebook_field_name, e.target.value)
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    <option value="">-- Select Field --</option>
                    {STANDARD_CRM_FIELDS.filter(
                      (field) =>
                        !mappedCrmFields.includes(field.value) ||
                        field.value === mapping.crm_field,
                    ).map((field) => (
            ...
            {unmappedFields.length > 0 && (
              <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-700 mb-2">
                  {unmappedFields.length} unmapped field
                  {unmappedFields.length > 1 ? "s" : ""}
                </p>
                <select
                  onChange={(e) => {
                    const field = unmappedFields.find(
                ...
                >
                  ...
                  {unmappedFields.map((field) => (
                    <option key={field.key} value={field.key}>
                      {field.label} (
                      {FACEBOOK_FIELD_TYPES[field.type] || field.type})
                    </option>
                  ))}
                </select>
              </div>
        ...
        <div className="space-y-3">
            ...
            >
              ...
              <div className="flex-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    updateCustomMapping(customMapping.id, {
                      facebook_field_name: e.target.value,
                    })
                  }
          ...
          {customMappings.length === 0 && (
            <p className="text-gray-500 text-sm">
              No custom fields configured. Click "Add Custom Field" to map
              Facebook fields to custom CRM fields.
            </p>
          )}
        </div>
      </div>
