...
export default function RecurrenceModal({ isOpen, onClose, onSave, classSession }: RecurrenceModalProps) {
  const [formData, setFormData] = useState<RecurrenceData>({
    frequency: 'weekly',
    interval: 1,
    daysOfWeek: classSession ? [new Date(classSession.start_time).getDay()] : [1],
    endType: 'never',
    endDate: '',
    occurrences: 10,
    rrule: ''
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.endType === 'date' && formData.endDate) {
      ...
      const startDate = classSession ? new Date(classSession.start_time) : new Date();
      if (endDate <= startDate) {
        newErrors.push('End date must be after the class start date');
      }
    }

    if (formData.endType === 'count' && (!formData.occurrences || formData.occurrences < 1)) {
  ...
  const getRecurrencePreview = (): string => {
    const { frequency, interval, daysOfWeek, endType, endDate, occurrences } = formData;
    ...
    if (frequency === 'daily') {
      ...
      const dayLabels = daysOfWeek?.map(day => WEEKDAYS.find(w => w.value === day)?.label).join(', ') || '';
      preview = interval === 1 
        ? `Weekly on ${dayLabels}`
      ...
      <div className="bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-white">Set Up Recurring Classes</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center">
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, endDate: e.target.value, endType: 'date' })}
              ...
              <label className="flex items-center">
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, occurrences: parseInt(e.target.value) || 1, endType: 'count' })}
          ...
          <div className="bg-gray-900 rounded-lg p-4 border border-gray-600">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-orange-400">Preview</span>
            </div>
