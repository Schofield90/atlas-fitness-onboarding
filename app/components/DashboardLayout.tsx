...
export default function DashboardLayout({
            ...
            try {
              if (newMsg.lead_id) {
                ...
                const senderName =
                  ...
                  lead?.email ||
                  "Unknown contact";
                const preview = (newMsg.body || newMsg.message || "")
                  .toString()
                  .slice(0, 80);
  ...
  const loadSystemPreference = () => {
    const savedMode = localStorage.getItem("systemMode") as SystemMode;
    if (savedMode && (savedMode === "crm" || savedMode === "booking")) {
  ...
  const handleSystemModeChange = (mode: SystemMode) => {
    setSystemMode(mode);
  ...
  const crmNavigation = [
      ...
      icon: (
        ...
        >
          ...
          <path
            ...
            strokeLinejoin="round"
            strokeWidth={2}
            d="M15 12a2 2 0 11-4 0 2 2 0 014 0z"
          />
        </svg>
      ),
    },
    {
      name: "Calendar & Booking Links",
      href: "/booking",
      icon: (
        <svg
          className="w-5 h-5"
    ...
    },
    {
      ...
      showBadge: !isFeatureEnabled("campaignsActions"),
    },
    ...
    },
    {
      ...
      showBadge: !isFeatureEnabled("surveysActions"),
    },
    {
  ...
  // Booking Navigation - GoTeamUp Style
      ...
      icon: (
        ...
        >
          <path
            ...
            strokeLinejoin="round"
            strokeWidth={2}
            d="M9 14L5 18m0 0l4-4m-4 4l4-4m11-1V7a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2h14a2 2 0 002-2z"
          />
        </svg>
      ),
    },
  ...
  const getFilteredNavigation = (nav: any[]) => {
    if (!staffPermissions) return nav;

    return nav.filter((item) => {
      // Check if staff has permission to view this section
      if (
        staffPermissions[item.name.toLowerCase().replace(/\s+/g, "_")] === false
      ) {
        return false;
      }
      return true;
  ...
  const navigation =
    systemMode === "crm"
      ? getFilteredNavigation(crmNavigation)
      : getFilteredNavigation(bookingNavigation);

  const trialEnds = userData?.trialEnds ? new Date(userData.trialEnds) : null;
  const daysLeft =
    mounted && trialEnds
      ? Math.ceil(
          (trialEnds.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24),
        )
      : 0;
      ...
      >
        ...
        <div className="p-4 border-b border-gray-700">
          <div className="flex items-center justify-between mb-4">
            <h1
              className={`${sidebarOpen ? "block" : "hidden"} text-xl font-bold text-orange-500`}
            >
              Gymleadhub
            </h1>
            <button
          ...
          {/* System Mode Toggle */}
          <div
            className={`${sidebarOpen ? "block" : "hidden"} bg-gray-700 rounded-lg p-1`}
          >
            <div className="grid grid-cols-2 gap-1">
              <button
                onClick={() => handleSystemModeChange("crm")}
              ...
              <button
                onClick={() => handleSystemModeChange("booking")}
                className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                  systemMode === "booking"
          ...
          {navigation.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`${sidebarOpen ? "block" : "hidden"} flex items-center`}
                >
                  {item.name}
                  {item.showBadge && sidebarOpen && <ComingSoonBadge />}
                </span>
              </Link>
            );
          })}
        ...
        {/* User Menu & Trial Info */}
        <div className="p-4 border-t border-gray-700">
          {mounted && trialEnds && daysLeft > 0 && (
            <div
              className={`${sidebarOpen ? "block" : "hidden"} mb-4 p-3 bg-orange-900 bg-opacity-50 rounded-lg`}
            >
          ...
          <Link
            href="/settings"
            className={`w-full flex items-center px-2 py-2 mb-2 text-sm font-medium rounded-md text-gray-300 hover:bg-gray-700 hover:text-white transition-colors`}
          >
            <svg
        ...
        <div className="bg-gray-800 border-b border-gray-700 px-6 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              >
                <Mail
                  className={`h-5 w-5 ${unreadMessagesCount > 0 ? "text-red-500" : "text-gray-400"}`}
                />
            ...
            <div className="bg-orange-600 border-b border-orange-700">
              <div className="px-6 py-4">
                <div className="flex items-center justify-between">
                  ...
                  <Link
                    href="/settings"
                    className="bg-white text-orange-600 px-6 py-2.5 rounded-lg hover:bg-orange-50 transition-colors font-semibold shadow-lg"
                  >
                    Upgrade Now
                  </Link>
                </div>
