...
export default function DashboardLayout({ children, userData }: DashboardLayoutProps) {
  ...
  const crmNavigation = [
    ...
    {
      name: 'Sales Calls & Consultations',
      href: '/booking',
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  const getFilteredNavigation = (nav: any[]) => {
    ...
    return nav.filter(item => {
      ...
      if (staffPermissions[item.name.toLowerCase().replace(/\s+/g, '_')] === false) {
        return false
      }
      return true
    })
  ...
  const navigation = systemMode === 'crm' 
      ...
      <div className={`${sidebarOpen ? 'w-64' : 'w-16'} bg-gray-800 transition-all duration-300 flex flex-col`}>
        ...
        <div className="p-4 border-b border-gray-700">
          <div className="flex items-center justify-between mb-4">
            <h1 className={`${sidebarOpen ? 'block' : 'hidden'} text-xl font-bold text-orange-500`}>
              Gymleadhub
            </h1>
            <button
          ...
          {/* System Mode Toggle */}
          <div className={`${sidebarOpen ? 'block' : 'hidden'} bg-gray-700 rounded-lg p-1`}>
          ...
          {navigation.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={(e) => {
                  // Prevent default navigation for certain routes in booking mode
                  if (systemMode === 'booking' && item.href === '/customers') {
                    e.preventDefault()
              ...
              >
                ...
                <span className={`${sidebarOpen ? 'block' : 'hidden'} flex items-center`}>
                  {item.name}
        ...
        <div className="p-4 border-t border-gray-700">
          {mounted && trialEnds && daysLeft > 0 && (
            <div className={`${sidebarOpen ? 'block' : 'hidden'} mb-4 p-3 bg-orange-900 bg-opacity-50 rounded-lg`}>
              <p className="text-sm text-orange-300">
                Trial ends in {daysLeft} days
              </p>
              <Link href="/billing" className="text-xs text-orange-400 hover:text-orange-300 underline">
                Upgrade now
              </Link>
            </div>
          )}
