...
const TeamChatNotificationContext = createContext<TeamChatNotificationContextType | undefined>(undefined);

export function useTeamChatNotifications() {
  const context = useContext(TeamChatNotificationContext);
  if (!context) {
    throw new Error('useTeamChatNotifications must be used within a TeamChatNotificationProvider');
  }
  return context;
}
...
export default function TeamChatNotificationProvider({ children }: TeamChatNotificationProviderProps) {
        ...
        async (payload) => {
          ...
          if (message) {
            const notification: TeamChatNotification = {
              ...
              user_name: (message as any).user.full_name || (message as any).user.email,
        ...
        async (payload) => {
          ...
          if (details) {
            const notification: TeamChatNotification = {
              ...
              user_name: (details as any).user.full_name || (details as any).user.email,
  ...
  const toggleNotifications = () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('team-chat-notifications-enabled', JSON.stringify(newValue));
    }
  };
