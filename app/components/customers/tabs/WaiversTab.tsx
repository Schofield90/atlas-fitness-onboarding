...
import { FileText, Download, Eye, Clock, CheckCircle, AlertCircle, Upload, Plus, Send, Mail, RotateCcw, X, Users, Edit } from 'lucide-react'
...
export default function WaiversTab({ customerId }: WaiversTabProps) {
  ...
  const [showWaiverModal, setShowWaiverModal] = useState<CustomerWaiver | null>(null)
  ...
  const fetchCustomerWaivers = async () => {
    try {
      const response = await fetch(`/api/waivers/customer-waivers?customer_id=${customerId}`)
  ...
  const sendWaiverEmail = async (customerWaiverId: string, isReminder: boolean = false) => {
    try {
  ...
  const markWaiverSigned = async (customerWaiverId: string) => {
    if (!confirm('Are you sure you want to manually mark this waiver as signed?')) {
      return
    }
    ...
    try {
      ...
      const response = await fetch(`/api/waivers/customer-waivers/${customerWaiverId}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-800 rounded-lg p-6">
        <h4 className="text-md font-semibold text-white mb-4">Customer Waivers</h4>
        
        {customerWaivers.length === 0 ? (
          <div className="text-center py-8">
            <FileText className="h-8 w-8 text-gray-600 mx-auto mb-2" />
            <p className="text-gray-400">No waivers assigned to this customer</p>
            <button
              onClick={() => setShowAssignModal(true)}
          ...
          <div className="space-y-4">
            {customerWaivers.map((customerWaiver) => (
              <div key={customerWaiver.id} className="p-4 bg-gray-700 rounded-lg">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      {getStatusIcon(customerWaiver.status, customerWaiver.expires_at)}
                      <h5 className="font-medium text-white">{customerWaiver.waiver.title}</h5>
                      <span className={`px-2 py-0.5 rounded-full text-xs ${getStatusColor(customerWaiver.status, customerWaiver.expires_at)}`}>
                        {getStatusText(customerWaiver.status, customerWaiver.expires_at)}
                      </span>
                    </div>
                    
                    <div className="grid grid-cols-2 gap-4 text-sm text-gray-400 mb-3">
                      <div>
                        <span className={`inline-block px-2 py-0.5 rounded text-xs ${getTypeColor(customerWaiver.waiver.waiver_type)} mr-2`}>
                      ...
                      <div>
                        <span>Assigned: {formatBritishDate(customerWaiver.assigned_at)}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm text-gray-400 mb-3">
                      <div>
                        {customerWaiver.sent_at ? (
                          <span>Sent: {formatBritishDate(customerWaiver.sent_at)}</span>
                        ) : (
                      ...
                      <div>
                        {customerWaiver.opened_at && (
                          <span>Opened: {formatBritishDate(customerWaiver.opened_at)}</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm text-gray-400">
                      <div>
                        {customerWaiver.signed_at ? (
                          <span className="text-green-400">Signed: {formatBritishDate(customerWaiver.signed_at)}</span>
                        ) : customerWaiver.expires_at ? (
                          <span>Expires: {formatBritishDate(customerWaiver.expires_at)}</span>
                        ) : null}
                      </div>
                      <div>
                        {customerWaiver.reminder_count > 0 && (
                          <span>Reminders: {customerWaiver.reminder_count}</span>
                        )}
                      </div>
                    </div>
                    ...
                    {customerWaiver.witness_name && (
                      <div className="mt-2 text-sm text-gray-400">
                        ...
                        {customerWaiver.witness_email && <span> ({customerWaiver.witness_email})</span>}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    ...
                    {customerWaiver.status === 'pending' && (
                      <>
                        {/* Send/Resend Email */}
                        <button
                          onClick={() => sendWaiverEmail(customerWaiver.id, !!customerWaiver.sent_at)}
                          ...
                          title={customerWaiver.sent_at ? "Send reminder" : "Send waiver email"}
                        >
                    ...
                    {customerWaiver.status === 'signed' && customerWaiver.signature_data && (
                      <button
                        className="p-2 text-gray-400 hover:text-white hover:bg-gray-600 rounded-lg"
            ...
            {availableWaivers.filter(w => w.is_active).length === 0 ? (
              ...
              <div className="space-y-3">
                {availableWaivers
                  ...
                  .filter(w => !customerWaivers.some(cw => cw.waiver_id === w.id))
                  .map((waiver) => (
                    <div
                      key={waiver.id}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          <h4 className="font-medium text-white mb-1">{waiver.title}</h4>
                          <div className="flex items-center gap-2 text-sm text-gray-400">
                            <span className={`px-2 py-0.5 rounded text-xs ${getTypeColor(waiver.waiver_type)}`}>
      ...
      {showWaiverModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg p-6 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold text-white">{showWaiverModal.waiver.title}</h3>
              <button
            ...
            <div className="bg-gray-700 rounded-lg p-4 mb-4">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <span className={`ml-2 px-2 py-0.5 rounded text-xs ${getStatusColor(showWaiverModal.status, showWaiverModal.expires_at)}`}>
                    {getStatusText(showWaiverModal.status, showWaiverModal.expires_at)}
                  </span>
                </div>
                <div>
                  <span className="text-gray-400">Type:</span>
                  <span className="ml-2 text-white">{formatWaiverType(showWaiverModal.waiver.waiver_type)}</span>
                </div>
                {showWaiverModal.signed_at && (
                  <div>
                    <span className="text-gray-400">Signed:</span>
                    <span className="ml-2 text-white">{formatBritishDate(showWaiverModal.signed_at)}</span>
                  </div>
                )}
                {showWaiverModal.expires_at && (
                  <div>
                    <span className="text-gray-400">Expires:</span>
                    <span className="ml-2 text-white">{formatBritishDate(showWaiverModal.expires_at)}</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="mt-4 bg-gray-700 rounded-lg p-4">
                ...
                {showWaiverModal.witness_name && (
                  <div className="mt-2 text-sm text-gray-400">
                    ...
                    {showWaiverModal.witness_email && <span> ({showWaiverModal.witness_email})</span>}
                  </div>
                )}
              </div>
            )}
