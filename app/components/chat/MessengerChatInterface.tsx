...
export default function MessengerChatInterface({
            ...
            setMessages((prev) =>
              prev.map((msg) =>
                msg.id === payload.new.id
                  ? (payload.new as MessengerMessage)
                  : msg,
              ),
            );
          }
  ...
  const updateWindowStatus = (lastInboundAt?: string) => {
    ...
    if (hoursElapsed >= 24) {
      setIsWithinWindow(false);
      setWindowExpiresIn(
        `Window expired ${Math.floor(hoursElapsed - 24)} hours ago`,
      );
    } else {
      setIsWithinWindow(true);
      const hoursRemaining = Math.floor(24 - hoursElapsed);
      const minutesRemaining = Math.floor(
        (24 - hoursElapsed - hoursRemaining) * 60,
      );
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span
                className={isWithinWindow ? "text-green-600" : "text-red-600"}
              >
                {windowExpiresIn}
              </span>
            </div>
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="text-sm text-yellow-800">
              ...
              <p className="text-xs mt-1">
                You can only send messages within 24 hours of the customer's
                last message. Wait for the customer to message again to reopen
                the window.
              </p>
            </div>
          </div>
      ...
      <div className="border-t p-4">
        <div className="flex items-center gap-2">
          ...
          <input
            ...
            placeholder={
              isWithinWindow ? "Type a message..." : "Messaging window expired"
            }
