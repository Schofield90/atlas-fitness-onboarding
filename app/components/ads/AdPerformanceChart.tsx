...
export function AdPerformanceChart({ accountId, days }: AdPerformanceChartProps) {
  ...
  const fetchChartData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/ads/chart-data?account_id=${accountId}&days=${days}`);
      if (response.ok) {
            ...
            <div key={index} className="flex items-center justify-between space-x-4">
              ...
              <span className="text-white font-medium">
                {entry.dataKey === 'spend' ? formatCurrency(entry.value) :
                 entry.dataKey === 'ctr' ? formatPercentage(entry.value) :
        ...
        {selectedMetrics.ctr && (
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-400">
              {formatPercentage(data.reduce((sum, d, _, arr) => sum + d.ctr, 0) / data.length)}
            </div>
