...
import { 
  Mail, Search, Plus, Sparkles, FileText, Eye, Edit, Variable, 
  ChevronDown, Send, Bold, Italic, Underline, Link, List, 
  CheckCircle, XCircle, Loader2, Image, Paperclip, BarChart3,
  Clock, Users, TestTube, Settings, Zap, Target
} from 'lucide-react'
...
export default function EnhancedEmailActionConfig({ config, onChange, organizationId }: EmailActionConfigProps) {
  ...
  const [activeTab, setActiveTab] = useState<'basic' | 'abtest' | 'delivery' | 'tracking' | 'attachments'>('basic')

  // A/B Testing Configuration
  const [abTestConfig, setABTestConfig] = useState<ABTestConfig>(config.abTestConfig || {
    enabled: false,
    variants: [],
  ...
  const [deliveryConfig, setDeliveryConfig] = useState<DeliveryOptimization>(config.deliveryConfig || {
    sendTime: 'immediate',
  ...
  const [trackingConfig, setTrackingConfig] = useState<TrackingConfig>(config.trackingConfig || {
    openTracking: true,
    clickTracking: true,
  ...
  const availableVariables = [
    ...
    { key: '{{organization_name}}', label: 'Organization Name', category: 'Organization' },
    ...
    { key: '{{next_appointment}}', label: 'Next Appointment', category: 'Schedule' },
    { key: '{{last_interaction}}', label: 'Last Interaction', category: 'Activity' }
  ]
  ...
  }, [emailMode, customEmail, selectedTemplate, abTestConfig, deliveryConfig, trackingConfig, attachments])
...
P.S. As a new member, you'll receive 20% off your first month – but this exclusive offer expires in 3 days!`
  ...
  const updateABTestVariant = (variantId: string, field: string, value: string | number) => {
    setABTestConfig(prev => ({
      ...prev,
            ...
            {showRichEditor && (
              <div className="mb-2 p-2 bg-gray-50 rounded-md border border-gray-200">
                <div className="flex items-center space-x-1">
                  ...
                  <button type="button" className="p-1 rounded hover:bg-gray-200" title="Insert Link">
                    <Link className="w-4 h-4" />
                  </button>
                  <button type="button" className="p-1 rounded hover:bg-gray-200" title="Bullet List">
                    <List className="w-4 h-4" />
                  </button>
                  <button type="button" className="p-1 rounded hover:bg-gray-200" title="Insert Image">
            ...
            <div className="p-3 bg-gray-50 rounded-lg">
              ...
              <div className="space-y-3">
                {Object.entries(
                  ...
                  <div key={category}>
                    <h5 className="text-xs font-medium text-gray-600 uppercase tracking-wide mb-1">{category}</h5>
                    <div className="grid grid-cols-2 gap-1">
                      {vars.map((variable) => (
                        ...
                        >
                          <code className="text-blue-600 text-xs">{variable.key}</code>
                          <span className="text-gray-600 ml-1 text-xs">- {variable.label}</span>
                        </button>
                      ))}
                    </div>
                  </div>
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange({ ...config, replyToEmail: e.target.value })}
  ...
  const renderABTestConfig = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-medium text-gray-700">A/B Test Configuration</h3>
          <p className="text-xs text-gray-500">Test different versions to optimize performance</p>
        </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => setABTestConfig(prev => ({ ...prev, testMetric: e.target.value as any }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setABTestConfig(prev => ({ ...prev, testDuration: parseInt(e.target.value) }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                type="button"
                onClick={() => setABTestConfig(prev => ({ ...prev, winnerSelection: 'automatic' }))}
              ...
              <button
                type="button"
                onClick={() => setABTestConfig(prev => ({ ...prev, winnerSelection: 'manual' }))}
                className={`px-4 py-2 rounded-lg border text-sm ${
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <h4 className="text-sm font-medium text-gray-700">Test Variants</h4>
              <button
                type="button"
              ...
              {abTestConfig.variants.map((variant, index) => (
                <div key={variant.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <input
                        ...
                        onChange={(e) => updateABTestVariant(variant.id, 'weight', parseInt(e.target.value))}
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      value={variant.body}
                      onChange={(e) => updateABTestVariant(variant.id, 'body', e.target.value)}
  ...
  const renderDeliveryConfig = () => (
    <div className="space-y-6">
      <h3 className="text-sm font-medium text-gray-700">Delivery Optimization</h3>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <button
              key={value}
              type="button"
              onClick={() => setDeliveryConfig(prev => ({ ...prev, sendTime: value as any }))}
              className={`px-4 py-2 rounded-lg border text-sm ${
      ...
      {deliveryConfig.sendTime === 'scheduled' && (
        <div>
          ...
          <input
            ...
            onChange={(e) => setDeliveryConfig(prev => ({ ...prev, scheduledTime: e.target.value }))}
            className="w-full px-3 py-2 border border-gray-300 rounded-md"
          />
        </div>
      )}
      ...
      <label className="flex items-center space-x-2">
        <input
          ...
          onChange={(e) => setDeliveryConfig(prev => ({ ...prev, timeZoneOptimization: e.target.checked }))}
          className="rounded border-gray-300 text-blue-600"
        />
        <span className="text-sm text-gray-700">Optimize send time by recipient's time zone</span>
      </label>

      {/* Frequency Capping */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <h4 className="text-sm font-medium text-gray-700">Frequency Capping</h4>
          <label className="flex items-center">
            <input
              type="checkbox"
              ...
              onChange={(e) => setDeliveryConfig(prev => ({ 
                ...prev, 
                frequencyCapping: { ...prev.frequencyCapping, enabled: e.target.checked }
              }))}
              className="rounded border-gray-300 text-blue-600"
            />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setDeliveryConfig(prev => ({ 
                  ...prev, 
                  frequencyCapping: { ...prev.frequencyCapping, maxEmailsPerDay: parseInt(e.target.value) }
                }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setDeliveryConfig(prev => ({ 
                  ...prev, 
                  frequencyCapping: { ...prev.frequencyCapping, maxEmailsPerWeek: parseInt(e.target.value) }
                }))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
              />
            </div>
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => setDeliveryConfig(prev => ({ 
            ...prev, 
            suppressionLists: Array.from(e.target.selectedOptions, option => option.value)
          }))}
          className="w-full px-3 py-2 border border-gray-300 rounded-md"
          size={3}
        >
  ...
  const renderTrackingConfig = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        {[
          { key: 'openTracking', label: 'Track email opens', description: 'Know when recipients open your email' },
          { key: 'clickTracking', label: 'Track link clicks', description: 'Track which links recipients click' },
          { key: 'unsubscribeTracking', label: 'Track unsubscribes', description: 'Monitor unsubscribe rates' },
          { key: 'conversionTracking', label: 'Track conversions', description: 'Track goal completions from email' }
        ].map(({ key, label, description }) => (
          <label key={key} className="flex items-start space-x-3">
            <input
              type="checkbox"
              checked={(trackingConfig as any)[key]}
              onChange={(e) => setTrackingConfig(prev => ({ ...prev, [key]: e.target.checked }))}
      ...
      {/* UTM Parameters */}
      <div>
        <h4 className="text-sm font-medium text-gray-600 mb-3">UTM Parameters</h4>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Campaign
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) => setTrackingConfig(prev => ({ ...prev, customTrackingPixel: e.target.value }))}
  ...
  const renderAttachmentsConfig = () => (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
        <h4 className="text-sm font-medium text-blue-900 mb-2">Attachment Guidelines</h4>
