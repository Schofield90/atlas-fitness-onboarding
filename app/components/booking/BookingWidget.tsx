...
export default function BookingWidget({
  ...
  const fetchAvailability = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/booking-by-slug/availability?slug=${slug}&${params}`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const { booking_link, appointment_types, organization, assigned_staff } =
    details;
    ...
    >
      ...
      {booking_link.style_settings.custom_css && (
        <style
          dangerouslySetInnerHTML={{
            __html: booking_link.style_settings.custom_css,
          }}
        />
      )}
      ...
      <div className="max-w-2xl mx-auto p-6">
        ...
        <div className="flex items-center justify-center mb-8">
          {["service", "staff", "datetime", "form", "confirmation"].map(
            (stepName, index) => (
              <div key={stepName} className="flex items-center">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
                    step === stepName
                      ? "text-white"
                      : ["service", "staff", "datetime", "form"].indexOf(step) >
                          index
                  ...
                  style={{
                    backgroundColor:
                      step === stepName ? primaryColor : undefined,
                  }}
                >
                  {["service", "staff", "datetime", "form"].indexOf(step) >
                  index ? (
        ...
        {step === "datetime" && (
          <DateTimeSelectionStep
            ...
            onBack={() =>
              setStep(assigned_staff.length > 1 ? "staff" : "service")
            }
            primaryColor={primaryColor}
          />
        )}
        ...
        {appointmentTypes.map((type) => (
          <button
            ...
            style={{
              ...
              backgroundColor:
                selected?.id === type.id ? `${primaryColor}10` : undefined,
            }}
          >
      ...
      <div className="grid gap-4">
        {staff.map((member) => (
          <button
            ...
            style={{
              borderColor:
                selected?.id === member.id ? primaryColor : undefined,
              backgroundColor:
                selected?.id === member.id ? `${primaryColor}10` : undefined,
            }}
          >
            <div className="flex items-center gap-4">
              {member.avatar_url ? (
            ...
            {getDateSlots(selectedDate).map((slot, index) => (
              <button
                ...
                style={{
                  ...
                  backgroundColor:
                    selectedSlot === slot ? primaryColor : undefined,
                }}
              >
    ...
    return (
      allFields.every(
        (field) =>
          !field.required ||
          (formData[field.name] &&
            formData[field.name].toString().trim().length > 0),
      ) && formData.consent
    );
  };
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-6">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                {(() => {
                  const date = new Date(selectedSlot.start_time);
          ...
          <div key={field.name}>
            ...
            {field.type === "textarea" ? (
              <textarea
                ...
                onChange={(e) =>
                  onFormChange({ ...formData, [field.name]: e.target.value })
                }
      ...
      <div className="border-t pt-4">
        <label className="flex items-start gap-3">
          <input
            ...
            onChange={(e) =>
              onFormChange({ ...formData, consent: e.target.checked })
            }
            className="mt-1"
            required
          />
    ...
    <div className="space-y-6">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <p className="text-yellow-800 text-sm">
          <strong>Payment Integration:</strong> Stripe payment processing will
          be implemented in the next phase. For now, bookings will be created
          without payment.
        </p>
      </div>
      ...
      {result?.success ? (
        <>
          ...
          <div>
            ...
            <p className="text-gray-600">{customMessage || result.message}</p>
