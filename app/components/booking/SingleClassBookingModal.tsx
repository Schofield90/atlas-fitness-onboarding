...
export default function SingleClassBookingModal({
  ...
  const canBookClass = () => {
    ...
    if (hoursUntilClass < cutoffHours) {
      return { canBook: false, reason: `Booking closes ${cutoffHours} hours before class` }
    }
  ...
  const handleBookClass = async () => {
    try {
      ...
      const bookingData = {
        ...
        status: 'confirmed',
        payment_status: selectedMethod.type === 'card' && classSchedule.price_pennies > 0 ? 'pending' : 'succeeded',
        payment_amount_pennies: selectedMethod.type === 'card' ? classSchedule.price_pennies : 0,
        special_requirements: specialRequirements || null,
        metadata: selectedMethod.type === 'package' ? { package_id: selectedMethod.id } : {}
      }
      ...
      if (selectedMethod.type === 'package') {
        const { error: packageError } = await supabase
          ...
          .update({
            ...
            classes_used: supabase.rpc('increment_classes_used', { pkg_id: selectedMethod.id })
          })
            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-800 rounded-lg p-4">
                ...
                {classSchedule.class_type.description && (
                  <p className="text-gray-300 mb-4">{classSchedule.class_type.description}</p>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="flex items-center gap-2 text-gray-300">
                    ...
                    {formatBritishDateTime(classSchedule.start_time).split(' ')[1]} - {formatBritishDateTime(classSchedule.end_time).split(' ')[1]}
                  </div>
                  <div className="flex items-center gap-2 text-gray-300">
                    <Users className="h-4 w-4" />
                    {classSchedule.current_bookings}/{classSchedule.max_capacity} booked
                  </div>
              ...
              {customer && (
                <div className="bg-gray-800 rounded-lg p-4">
                  <h4 className="text-md font-medium text-white mb-2">Booking for:</h4>
              ...
              {!canBook && (
                <div className="bg-red-900 border border-red-700 rounded-lg p-4 flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-red-300 font-medium">Cannot Book Class</h4>
              ...
              <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
                ...
                <p className="text-gray-400 text-sm">
                  Bookings can be cancelled up to {classSchedule.cancellation_cutoff_hours || 24} hours before the class starts.
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          {method.remaining !== undefined && method.remaining <= 3 && (
                            <p className="text-yellow-400 text-xs mt-1">
                              Warning: Only {method.remaining} classes remaining
                            </p>
                          )}
                        </div>
                      </div>
            ...
            <div className="text-center space-y-6">
              ...
              <div className="bg-gray-800 rounded-lg p-4 text-left">
                ...
                <div className="space-y-2 text-sm text-gray-300">
                  ...
                  <div>Time: {formatBritishDateTime(classSchedule.start_time).split(' ')[1]} - {formatBritishDateTime(classSchedule.end_time).split(' ')[1]}</div>
                  {classSchedule.room_location && <div>Location: {classSchedule.room_location}</div>}
                </div>
              </div>
            </div>
          )}
