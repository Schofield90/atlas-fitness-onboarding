...
const SelectedClassDetails: React.FC<SelectedClassDetailsProps> = ({
  selectedClass,
}) => {
  if (!selectedClass) {
    return (
      <div
        ...
        data-testid="selected-class-panel"
      >
  ...
  const start = selectedClass.startTime
    ...
    : null;
  const end =
    start && selectedClass.duration
      ? new Date(start.getTime() + selectedClass.duration * 60000)
      : null;
  ...
  const timeString =
    start && end
      ...
      : selectedClass.time || "";
  const bookings =
    typeof selectedClass.bookings === "number" ? selectedClass.bookings : 0;
  const capacity =
    typeof selectedClass.capacity === "number" ? selectedClass.capacity : 0;
  ...
  return (
    ...
    >
      <div className="space-y-6">
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p
                ...
                data-testid="selected-class-time"
              >
                {[dateString, timeString].filter(Boolean).join(" • ")}
              </p>
            </div>
            ...
            <CardContent>
              <div className="flex items-center gap-3">
                <div className="text-2xl">
                  {selectedClass.instructor?.avatar ?? ""}
                </div>
                <div className="flex-1">
                  <h4 className="font-medium text-white">
                    {selectedClass.instructor?.name ?? selectedClass.instructor}
                  </h4>
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex items-center gap-1">
                      <Star className="w-3 h-3 text-yellow-400 fill-current" />
                      <span className="text-xs text-white opacity-70">
                        {selectedClass.instructor?.rating ?? ""}
                      </span>
                    </div>
              ...
              {selectedClass.waitlist > 0 && (
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="warning">
                    {selectedClass.waitlist} waiting
                  </Badge>
                </div>
              )}
            </div>
        ...
        {/* Equipment */}
        {Array.isArray(selectedClass.equipment) &&
              ...
              <div className="space-y-3">
                {selectedClass.attendees.map((attendee, index) => (
                  ...
                  >
                    <div>
                      <p className="text-sm font-medium text-white">
                        {attendee.name || attendee.client?.name || "Unknown"}
                      </p>
        ...
        <div className="space-y-3">
          ...
          <Button
            ...
            onClick={() => {
              ...
              alert(
                "Compose message modal would open to send to all 4 attendees",
              );
            }}
          >
