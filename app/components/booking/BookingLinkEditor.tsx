...
type TabType = 'details' | 'availability' | 'form' | 'notifications' | 'customization'
...
export default function BookingLinkEditor({ bookingLinkId, onSave, onCancel }: BookingLinkEditorProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : 'Failed to save booking link')
    } finally {
      setSaving(false)
    }
  }
  ...
  const handlePreview = async () => {
    try {
      const response = await fetch(`/api/booking-links/${bookingLinkId}/preview`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData)
      })
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <code className="text-orange-400 bg-gray-900 px-2 py-1 rounded">
                {typeof window !== 'undefined' ? window.location.origin : 'https://your-domain.com'}/book/{formData.slug}
              </code>
            </div>
            <Button
              variant="outline"
      ...
      <div className="bg-gray-800 rounded-lg p-6">
        ...
        {activeTab === 'customization' && <CustomizationTab formData={formData} setFormData={setFormData} />}
      </div>
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto border border-gray-600 rounded p-3">
                {appointmentTypes.map((type) => (
                  <label key={type.id} className="flex items-center">
                    <input
                      type="checkbox"
                      checked={formData.appointment_type_ids?.includes(type.id) || false}
                      onChange={(e) => {
                        ...
                        if (e.target.checked) {
                          ...
                          setFormData({
                            ...formData,
                            appointment_type_ids: currentIds.filter(id => id !== type.id)
                          })
                        }
                      }}
                      className="mr-3"
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-500">
                        ...
                        {type.max_capacity > 1 && ` • Max ${type.max_capacity} people`}
                      </div>
                    </div>
                  </label>
                ))}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, max_days_in_advance: parseInt(e.target.value) })}
                className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white"
                min="1"
                max="365"
              />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white"
              placeholder={
                formData.meeting_location?.type === 'in_person' ? 'Gym address or room' :
                formData.meeting_location?.type === 'video_call' ? 'Zoom link or meeting room' :
                formData.meeting_location?.type === 'phone' ? 'Phone number' :
...
const FormTab = ({ formData, setFormData }: {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Default Fields */}
      <div>
        <h4 className="text-md font-semibold text-gray-300 mb-3">Default Fields</h4>
        ...
        {customFields.length === 0 ? (
          ...
          <div className="space-y-3">
            {customFields.map((field, index) => (
              <div key={field.id} className="p-3 bg-gray-700 rounded flex items-center gap-4">
      ...
      <div className="space-y-4">
        <h4 className="text-md font-semibold text-gray-300">Confirmation Settings</h4>
...
const NotificationsTab = ({ formData, setFormData }: {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Email Notifications */}
      <div className="space-y-4">
        <h4 className="text-md font-semibold text-gray-300">Email Notifications</h4>
      ...
      {/* SMS Notifications */}
      <div className="space-y-4">
        <h4 className="text-md font-semibold text-gray-300">SMS Notifications</h4>
      ...
      {/* Reminder Schedule */}
      <div className="space-y-4">
        <h4 className="text-md font-semibold text-gray-300">Reminder Schedule</h4>
        ...
        <div className="space-y-2">
          {reminderOptions.map((option) => (
            <label key={option.value} className="flex items-center">
              <input
                type="checkbox"
                checked={formData.notification_settings?.reminder_schedules?.includes(option.value) || false}
                onChange={(e) => {
                  const currentSchedules = formData.notification_settings?.reminder_schedules || []
      ...
      <div className="space-y-4">
        <h4 className="text-md font-semibold text-gray-300">Cancellation Policy</h4>
