...
const TWILIO_FAQS: FAQ[] = [
  {
    ...
    answer: "Go to twilio.com and click 'Sign up free'. You'll get $15 in free credits to start. Verify your email and phone number to activate your account.",
    category: "setup"
  },
  {
    question: "Where do I find my Account SID and Auth Token?",
    answer: "Log into your Twilio Console at console.twilio.com. Your Account SID and Auth Token are displayed on the main dashboard under 'Account Info'.",
    category: "setup"
  },
  {
    question: "How do I buy a phone number?",
    answer: "In your Twilio Console, go to Phone Numbers > Manage > Buy a number. Choose your country, select SMS and Voice capabilities, then purchase the number.",
    category: "setup"
  },
  {
    question: "What if my connection test fails?",
    answer: "Check that your Account SID and Auth Token are correct. Ensure you have sufficient credits in your account and that your account is verified.",
    category: "troubleshooting"
  },
  {
    question: "Can I send WhatsApp messages through Twilio?",
    answer: "Yes! Twilio supports WhatsApp Business API. You'll need to apply for WhatsApp Business API access through Twilio.",
    category: "features"
  },
  {
    question: "How much does it cost to send SMS?",
    answer: "Costs vary by country. In the US, SMS typically costs $0.0075 per message. Check Twilio's pricing page for your region.",
    category: "features"
  }
]
...
export default function TwilioAIHelper() {
  ...
  useEffect(() => {
    if (messages.length === 0) {
      ...
      const welcomeMessage: Message = {
        id: 'welcome',
        content: "Hi! I'm your Twilio setup assistant. I can help you with account creation, finding credentials, troubleshooting connections, and more. What would you like to know?",
        role: 'assistant',
        timestamp: new Date()
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage: Message = {
        id: (Date.now() + 1).toString(),
        content: 'I apologize, but I encountered an error. Please try again or check the FAQ section for common questions.',
        role: 'assistant',
        timestamp: new Date()
      }
  ...
  const generateTwilioResponse = (query: string): string => {
    ...
    if (lowercaseQuery.includes('account sid') || lowercaseQuery.includes('auth token')) {
    ...
    if (lowercaseQuery.includes('phone number') || lowercaseQuery.includes('buy number')) {
    ...
    if (lowercaseQuery.includes('webhook') || lowercaseQuery.includes('callback')) {
    ...
    if (lowercaseQuery.includes('test') || lowercaseQuery.includes('connection') || lowercaseQuery.includes('fail')) {
    ...
    if (lowercaseQuery.includes('cost') || lowercaseQuery.includes('price') || lowercaseQuery.includes('billing')) {
